Develop CMB with Git
=========================

This page documents how to develop CMB through [Git][].
...
Here we document procedures used by the CMB development community.
...
3.  Follow the [download instructions](download.md#clone) to create a
    local clone of the main CMB repository.  Optionally configure
...
4.  Run the [developer setup script][] to prepare your CMB work tree and
...
5.  (Optional but highly recommended.)
    [Register](https://open.cdash.org/register.php) with the CMB project
    ...
    "Subscribe to this project" on the right of CMB.
...
CMB development uses a [branchy workflow][] based on topic branches.
...
[CMB GitLab][] repository page and using the "Fork" button in the upper right.

[CMB GitLab]: https://gitlab.kitware.com/cmb/cmb
...
It should be of the form:
    ...
    https://gitlab.kitware.com/<username>/cmb/merge_requests/new

Follow these steps:

1.  In the "**Source branch**" box select the `<username>/cmb` repository
    and the `my-topic` branch.

2.  In the "**Target branch**" box select the `cmb/cmb` repository and
...
deems that it needs more work, they can add the `triage:merge-needs-work` label.
...
For a list of all builders, see:
  ...
  * [cmb-expected](https://buildbot.kitware.com/builders?category=cmb-expected)
  * [cmb-superbuild](https://buildbot.kitware.com/builders?category=cmb-superbuild)
  * [cmb-experimental](https://buildbot.kitware.com/builders?category=cmb-experimental)
...
Contribute VTK or ParaView Changes
----------------------

If you have any VTK or ParaView changes, then you are required to get your changes
incorporated into VTK using [VTK's development workflow][] and/or into ParaView using [ParaView's development workflow][]. 
