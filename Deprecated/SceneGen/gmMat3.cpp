...
std::istream & operator >> ( std::istream& is, gmMatrix3& M )
{
  is >> M[0][0] >> M[0][1] >> M[0][2];
  is >> M[1][0] >> M[1][1] >> M[1][2];
  is >> M[2][0] >> M[2][1] >> M[2][2];
  return is;
}

std::ostream & operator << ( std::ostream& os, const gmMatrix3& M )
{
