...
{
#ifdef TRILIBRARY
  /* rather than exit we need to jump to the end */
  /* this way as a library triangle always returns instead of taking */
  /* down the application */

  /* Added in support for error type */
...
//*************************************************
//*** BCW - added by Barry C White (09-04-2007) ***
//*************************************************

#ifdef ANSI_DECLARATORS
void newTriangulateIO( struct triangulateio *tio )
#else /* not ANSI_DECLARATORS */
void newTriangulateIO( tio )
...
#ifdef ANSI_DECLARATORS
void deleteTriangulateIO( struct triangulateio *tio )
#else /* not ANSI_DECLARATORS */
void deleteTriangulateIO( tio )
struct triangulateio *tio;
#endif /* not ANSI_DECLARATORS */

{
  if (tio->pointlist != NULL) trifree((VOID *)tio->pointlist);
  tio->pointlist = NULL;
  if (tio->pointattributelist != NULL) trifree((VOID *)tio->pointattributelist);
  tio->pointattributelist = NULL;
  if (tio->pointmarkerlist != NULL) trifree((VOID *)tio->pointmarkerlist);
  tio->pointmarkerlist = NULL;
  tio->numberofpoints = 0;
  tio->numberofpointattributes = 0;

  if (tio->trianglelist != NULL) trifree((VOID *)tio->trianglelist);
  tio->trianglelist = NULL;
  if (tio->triangleattributelist != NULL) trifree((VOID *)tio->triangleattributelist);
  tio->triangleattributelist = NULL;
  if (tio->trianglearealist != NULL) trifree((VOID *)tio->trianglearealist);
  tio->trianglearealist = NULL;
  if (tio->neighborlist != NULL) trifree((VOID *)tio->neighborlist);
  tio->neighborlist = NULL;
  tio->numberoftriangles = 0;
  tio->numberofcorners = 0;
  tio->numberoftriangleattributes = 0;

  if (tio->segmentlist != NULL) trifree((VOID *)tio->segmentlist);
  tio->segmentlist = NULL;
  if (tio->segmentmarkerlist != NULL) trifree((VOID *)tio->segmentmarkerlist);
  tio->segmentmarkerlist = NULL;
  tio->numberofsegments = 0;

  if (tio->holelist != NULL) trifree((VOID *)tio->holelist);
  tio->holelist = NULL;
  tio->numberofholes = 0;

  if (tio->regionlist != NULL) trifree((VOID *)tio->regionlist);
  tio->regionlist = NULL;
  tio->numberofregions = 0;

  if (tio->edgelist != NULL) trifree((VOID *)tio->edgelist);
  tio->edgelist = NULL;
  if (tio->edgemarkerlist != NULL) trifree((VOID *)tio->edgemarkerlist);
  tio->edgemarkerlist = NULL;
  if (tio->normlist != NULL) trifree((VOID *)tio->normlist);
  tio->normlist = NULL;
  tio->numberofedges = 0;
}
//*************************************************
...
#ifndef TRILIBRARY /* not TRILIBRARY */
  FILE *polyfile;
#else /* TRILIBRARY */
  //setup tri library
  if( setjmp(longJumpBuf) )
    {
    //we just jumped here from a longjump
    //so now goto the end of the triangulate call
    //since something failed
    goto TRI_EXIT_GOTO;
    }
#endif
#ifndef NO_TIMER
