...
#include "qtCMBGeologyBuilderContextMenuBehavior.h"
...
///////////////////////////////////////////////////////////////////////////
// pqCMBGeologyBuilderMainWindowCore::vtkInternal

/// Private implementation details for pqCMBGeologyBuilderMainWindowCore
class pqCMBGeologyBuilderMainWindowCore::vtkInternal
{
public:
  ...
  QPointer<qtCMBGeologyBuilderContextMenuBehavior> RenderWindowMenuBehavior;
  ...
  QPointer<pqDisplayColorWidget> BHColorByWidget;
  QPointer<pqDisplayColorWidget> CSColorByWidget;
  QPointer<QLabel> BHRadiusLabel;
  QPointer<QDoubleSpinBox> BHRadiusBox;
};


///////////////////////////////////////////////////////////////////////////
// pqCMBGeologyBuilderMainWindowCore

pqCMBGeologyBuilderMainWindowCore::pqCMBGeologyBuilderMainWindowCore(QWidget* parent_widget) :
  ...
  Internal(new vtkInternal(parent_widget))
{
  ...
  this->ProgramKey = qtCMBProjectServerManager::GeologyBuilder;
  this->setObjectName("pqCMBGeologyBuilderMainWindowCore");
...
//-----------------------------------------------------------------------------
pqCMBGeologyBuilderMainWindowCore::~pqCMBGeologyBuilderMainWindowCore()
{
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::buildRenderWindowContextMenuBehavior(QObject * /*parent_widget*/)
{
  ...
  this->Internal->RenderWindowMenuBehavior =
    new qtCMBGeologyBuilderContextMenuBehavior(this->Tree, this->parent());
}

//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::setupGeoToolbar(QToolBar* geoToolbar)
{
  //this->Internal->BHColorByLabel=new QLabel("Color all boreholes by:", this);
  this->Internal->BHColorByWidget=new pqDisplayColorWidget(geoToolbar);
  this->Internal->BHColorByWidget->setObjectName("bhColorByWidget");
  this->Internal->BHColorByWidget->setToolTip("Color all boreholes by cell array:");
  QComboBox* components = this->Internal->BHColorByWidget->findChild<QComboBox*>("Components");
  if(components)
    {
    components->hide();
    }

  this->Internal->BHRadiusLabel=new QLabel(" Borehole radius:", geoToolbar);
  this->Internal->BHRadiusBox = new QDoubleSpinBox(geoToolbar);
  this->Internal->BHRadiusBox->setObjectName("bhRadiusBox");
  this->Internal->BHRadiusBox->setMinimum(0.1);
  this->Internal->BHRadiusBox->setMaximum(VTK_DOUBLE_MAX);
  this->Internal->BHRadiusBox->setValue(0.5);
  this->Internal->BHRadiusBox->setSingleStep(0.1);
  this->Internal->BHRadiusBox->setSizePolicy(QSizePolicy::Fixed,
    QSizePolicy::Fixed);
  this->Internal->BHRadiusBox->setFixedWidth(60);

  //this->Internal->CSColorByLabel=new QLabel("Color all cross-sections by:", this);
  this->Internal->CSColorByWidget=new pqDisplayColorWidget(geoToolbar);
  this->Internal->CSColorByWidget->setObjectName("csColorByWidget");
  this->Internal->CSColorByWidget->setToolTip("Color all cross-sections by cell array:");
  components = this->Internal->CSColorByWidget->findChild<QComboBox*>("Components");
  if(components)
    {
    components->hide();
    }

  geoToolbar->addWidget(this->Internal->BHColorByWidget);
  geoToolbar->addWidget(this->Internal->BHRadiusLabel);
  geoToolbar->addWidget(this->Internal->BHRadiusBox);
  geoToolbar->addWidget(this->Internal->CSColorByWidget);

  QObject::connect(
    this->Internal->BHColorByWidget, SIGNAL(arraySelectionChanged()),
    this, SLOT(colorBoreHoles()));
  QObject::connect(
    this->Internal->CSColorByWidget, SIGNAL(arraySelectionChanged()),
    this, SLOT(colorCrossSections()));

  QObject::connect(
    this->Internal->BHRadiusBox, SIGNAL(valueChanged(double)),
    this, SLOT(onBoreholeRadiusChanged(double)));

}
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::onBoreholeRadiusChanged(double radius)
{
  this->Tree->setBoreHolesRadius(radius);
}

//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::setupBoxWidget()
{
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::onBHEditColorMap()
{
  if(pqDataRepresentation* bhRep=this->Tree->getABoreHoleRepresentation())
    {
    pqCMBColorMapDialog editor(bhRep, pqCoreUtilities::mainWidget());
    editor.exec();
    }
}
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::onCSEditColorMap()
{
  if(pqDataRepresentation* csRep=this->Tree->getACrossSectionRepresentation())
    {
    pqCMBColorMapDialog editor(csRep, pqCoreUtilities::mainWidget());
    editor.exec();
    }
}

//----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::processScene(const QString& filename,
                                             pqPipelineSource* source)
{
  ...
  else if (finfo.completeSuffix().toLower() == "bor")
    {
    this->Internal->OutputFileName = filename;
    this->processBoreholeInfo(filename,source);
    }
...
//----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::processSceneInfo(const QString& filename,
                                            pqPipelineSource* source)
{
  // force read
...
//----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::processMapInfo(const QString& /*filename*/,
                                            pqPipelineSource* source)
{
...
//----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::processBoreholeInfo(const QString& filename,
                                            pqPipelineSource* source)
{
  int result=this->Tree->createBorFileObjects(filename, source);
  if(result > 0)
    {
    this->Internal->BHColorByWidget->setRepresentation(
      this->Tree->getABoreHoleRepresentation());
    this->Internal->CSColorByWidget->setRepresentation(
      this->Tree->getACrossSectionRepresentation());
...
//----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::processOSDLInfo(const QString& /*filename*/,
                                                 pqPipelineSource* source)
{
  pqApplicationCore* core = pqApplicationCore::instance();
  pqObjectBuilder* builder = core->getObjectBuilder();
  ...
  pqCMBSceneNode *scene = this->Tree->createRoot("Geology");
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::updateWidgetPanel(
  pqCMBSceneObjectBase* widgetObject)
{
  ...
  QList< pq3DWidget* > userWidgets = pWidget->findChildren<pq3DWidget*>(widgetName);

  bool found = false;
  for(int i=0; i<userWidgets.count(); i++)
    {
    if(!found && userWidgets.value(i) == selWidget)
      {
      found = true;
      }
    else
      {
      userWidgets.value(i)->setVisible(0);
      }
    }
  if(!found)
    {
    selWidget->setParent(pWidget);
    this->getAppearanceEditorContainer()->layout()->
      addWidget(selWidget);
    }

  if(selWidget->widgetVisible())
    {
    selWidget->select();
    selWidget->setVisible(true);
    selWidget->setEnabled(true);
    }
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::unselectNode(pqCMBSceneNode *node,
                                              bool updateBox)
{
  pqCMBSceneObjectBase *obj;
  if (node->isTypeNode())
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::RemoveBoxWidgetPropertyLinks()
{
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::prepSelectedObject(pqCMBSceneObjectBase *obj)
{
  if(!obj)
    {
    return;
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::selectNode(pqCMBSceneNode *node,
                                            bool updateBox)
{
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::updateBoxWidget()
{
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::zoomOnSelection()
{
  QList<pqCMBSceneNode *>::iterator iter;
  vtkBoundingBox bb;
  for (iter = this->SelectedLeaves.begin();
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::setSelectionMode(int mode)
{
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::onSaveAsData()
{
  if (this->Tree->isEmpty())
    {
    this->Internal->OutputFileName.clear();
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::onPackageScene()
{
  if (this->Tree->isEmpty())
    {
    this->Internal->OutputFileName.clear();
...
//-----------------------------------------------------------------------------
QString pqCMBGeologyBuilderMainWindowCore::getOutputFileName() const
{
  return this->Internal->OutputFileName;
}

//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::onSaveData()
{
  if (this->Tree->isEmpty())
    {
    this->Internal->OutputFileName.clear();
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::saveData(const QString& filename)
{
  if (this->Tree->isEmpty())
    {
    return;
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::onCloseData()
{
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::onCreateOmicronInput()
{
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::onExport2DBCSFile()
{
  // dialog to get filename and necessary parameters
  // get input filename...
  pqFileDialog file_dialog(this->getActiveServer(),
...
//-----------------------------------------------------------------------------
double pqCMBGeologyBuilderMainWindowCore::computeVolumeContraint(double volumeFactor)
{
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::createSurfaceMesh()
{
  ...
  this->CreateProperLiDARForSurfaceMesh(selectedNames,transformFilters);
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::CreateProperLiDARForSurfaceMesh( const QStringList &selectedNames,
   QList<pqPipelineSource*> &transformFilters)
{
  pqObjectBuilder* builder = pqApplicationCore::instance()->getObjectBuilder();
  pqCMBSceneNode *node;
...
//-----------------------------------------------------------------------------
double pqCMBGeologyBuilderMainWindowCore::getMinVOIBounds()
{
  int voiNodeIndex = this->Internal->MesherOptionDlg->getCurrentVOINameIndex();
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::createOmicronInput(QString *filename,
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::onSpawnSurfaceMesher()
{
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::onSpawnModelMesher()
{
  if (!setupModelMesherDialog())
    {
    return;
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::checkForChooseMesher(int mesherIndex)
{
...
//-----------------------------------------------------------------------------
int pqCMBGeologyBuilderMainWindowCore::selectSurfaceMesher()
{
  QString newMesher;
...
//-----------------------------------------------------------------------------
int pqCMBGeologyBuilderMainWindowCore::setupSurfaceMesherDialog()
{
  SceneObjectNodeIterator nodeIter( this->Tree->getRoot() );
...
//-----------------------------------------------------------------------------
int pqCMBGeologyBuilderMainWindowCore::setupModelMesherDialog()
{
  SceneObjectNodeIterator nodeIter( this->Tree->getRoot() );
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::previewMesherOutput()
{
...
//----------------------------------------------------------------------------
bool pqCMBGeologyBuilderMainWindowCore::exportSolidsCMBModel(
  QList<pqOutputPort*>& inputs, const QString& cmbFileName, bool is2D)
{
  ...
  pqPipelineSource *appendSolids = builder->createFilter( "filters",
    "AppendSolids", namedInputs, this->getActiveServer());
  ...
  // Version of the CMBModel that will live on the client... allowing us to add
  // extra info (color/username) to the cmodel after we "build" it
  vtkDiscreteModel *clientModel = vtkDiscreteModel::New();

  vtkSMProxyManager* pxm = vtkSMProxyManager::GetProxyManager();
  vtkSMProxy *modelWrapper = pxm->NewProxy("CMBModelGroup", "CMBModelWrapper");

  // master polydata normals (make sure each region has normals pointing out)
  // RSB!!! Doing some extra work here, at least some of the time, because
  // solids creating during TIN stitching are oriented the right wa already.
  vtkSMSourceProxy *masterPolyDataNormals = vtkSMSourceProxy::SafeDownCast(
    pxm->NewProxy("filters", "MasterPolyDataNormals"));
  pqSMAdaptor::setInputProperty(masterPolyDataNormals->GetProperty("Input"),
                                appendSolids->getProxy(), 0);
  masterPolyDataNormals->UpdateVTKObjects();

  vtkSMSourceProxy *modelMasterPoly;
  if(is2D)
    {
    //Extract region edges for 2D model
    vtkSMSourceProxy *extractEdges = vtkSMSourceProxy::SafeDownCast(
      pxm->NewProxy("filters", "ExtractRegionEdges"));
    pqSMAdaptor::setInputProperty(extractEdges->GetProperty("Input"),
      masterPolyDataNormals, 0);
    pqSMAdaptor::setElementProperty(
      extractEdges->GetProperty("RegionArrayName"),
      vtkCMBParserBase::GetShellTagName());
    extractEdges->UpdateVTKObjects();
    modelMasterPoly = extractEdges;
    }
  else
    {
    //Merge duplicate cells filter
    vtkSMSourceProxy *mergeDuplicateCells = vtkSMSourceProxy::SafeDownCast(
    ...
    modelMasterPoly = mergeDuplicateCells;
    }

  vtkSmartPointer<vtkCMBModelBuilderClient> builderOperator =
  vtkSmartPointer<vtkCMBModelBuilderClient>::New();
  bool buildResult = builderOperator->Operate(clientModel, modelWrapper,
                                              modelMasterPoly);
  if(buildResult)
    {
    // now add color and username information
    vtkModelItemIterator* entDomains = is2D ? clientModel->NewIterator(vtkModelFaceType) :
      clientModel->NewIterator(vtkModelRegionType);
    const char* strDomainName = is2D ? "Face%d" : "Region%d";
    for(entDomains->Begin(); !entDomains->IsAtEnd(); entDomains->Next())
      {
      int entIndex;
      vtkModelEntity* entity =
        vtkModelEntity::SafeDownCast(entDomains->GetCurrentItem());
      std::string temp = vtkModelUserName::GetUserName(entity);
      if (sscanf(temp.c_str(), strDomainName, &entIndex) > 0)
        {
        vtkSmartPointer<vtkModelEntityOperatorClient> entityOperator =
          vtkSmartPointer<vtkModelEntityOperatorClient>::New();
        entityOperator->SetItemType( entity->GetType() );
        entityOperator->SetId( entity->GetUniquePersistentId() );
        entityOperator->SetUserName( this->Internal->NodeInfo[entIndex-1].UserName.c_str() );
        entityOperator->SetRGBA(this->Internal->NodeInfo[entIndex-1].Color);
        entityOperator->Operate(clientModel, modelWrapper);
        }
      else
        {
    ...
    writerOperator->SetFileName( cmbFileName.toAscii().constData() );
    buildResult = writerOperator->Operate(clientModel, modelWrapper);
    }
  // cleanup
  builder->destroy(appendSolids);
  masterPolyDataNormals->Delete();
  modelMasterPoly->Delete();
  modelWrapper->Delete();
  clientModel->Reset();
  clientModel->Delete();
  return buildResult;
...
//----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::exportSelectedSolids()
{
...
//----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::exportSelectedPolygons()
{
  // see what we have for inputs (must be at least 1 solid); use NodeInfo to
  // store solid name and color info which is added after running the builder
  QList<pqOutputPort*> inputs;
...
//----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::exportCMBFile()
{
  emit enableExternalProcesses(true);
  ...
  if(solidMeshes.size()>0)
    ...
    if(!solidMeshOperator->GetOperateSucceeded())
      {
      QMessageBox::critical(NULL, "GeologyBuilder",
...
//----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::rejectMesh()
{
  emit enableExternalProcesses(true);
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::addOmicronModelFaceData(vtkPolyData *outputPD)
{
...
//-----------------------------------------------------------------------------
bool pqCMBGeologyBuilderMainWindowCore::clientTransformSource(pqPipelineSource* serverSource,
...
//----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::onRubberBandSelect(bool checked)
{
  if (checked)
    {
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::closeData()
{
  this->ColorNode = NULL;
  if (this->Tree->isEmpty())
    {
...
void pqCMBGeologyBuilderMainWindowCore::onReaderCreated(pqPipelineSource* reader,
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::resetCenterOfRotationToCenterOfCurrentData()
{
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::onServerCreationFinished(pqServer *server)
{
  if(this->Tree)
    {
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::setpqCMBSceneTree(pqCMBSceneTree * tree)
{
  this->Tree = tree;
  this->Tree->setCurrentView(this->activeRenderView());
  this->Tree->setCurrentServer(this->getActiveServer());
...
void pqCMBGeologyBuilderMainWindowCore::boxWidgetInteraction()
{
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::newScene()
{
  if (this->checkForPreviewDialog())
    {
    return;
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::updateNodeColor()
{
  if (!this->ColorNode)
    {
    return;
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::updateBoxInteraction()
{
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::AddLineSegmentObjectsToModel(
...
//-----------------------------------------------------------------------------
bool pqCMBGeologyBuilderMainWindowCore::AddContourObjectsToModel(
  ...
  while((n = iter.next()))
    ...
    if(arcInfo && arcInfo->GetNumberOfPoints() && arcInfo->IsClosedLoop())
      ...
      if(coords->GetNumberOfTuples() > 1)
        ...
        model->BuildModelFace(edges.size(), &edges[0], &edgeDirections[0], material);
        }
      }
    }
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::stitchTINs()
{
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::onPreviewAccepted()
{
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::onPreviewRejected()
{
  if (this->PreviewMeshOutput)
    {
    this->rejectMesh();
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::clearColorNode()
{
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::onSelectGlyph(bool checked)
{
  if (checked)
    {
    int isShiftKeyDown = this->activeRenderView()->
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::selectGlyph(QList<pqCMBGlyphObject*> &glyphList)
{
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::updateBoxWidgetForGlyph()
{
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::clearSelectedGlyphList()
{
  QMap<pqCMBGlyphObject*, QList<vtkIdType> >::iterator iter;
  // Check to see if there are any moveable leaf nodes
  for (iter = this->Internal->selGlyphs.begin();
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::updateBoxInteractionForGlyph(
...
//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::colorBoreHoles()
{
  QPair<int, QString> val = this->Internal->BHColorByWidget->arraySelection();
  int association = val.first;
  const QString &arrayName = val.second;

  this->colorObjects(association, arrayName.toStdString().c_str(),
                     pqCMBSceneObjectBase::GeoBoreHole);
}

//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::colorCrossSections()
{
  QPair<int, QString> val = this->Internal->CSColorByWidget->arraySelection();
  int association = val.first;
  const QString &arrayName = val.second;

  this->colorObjects(association, arrayName.toStdString().c_str(),
                     pqCMBSceneObjectBase::GeoCrossSection);
}

//-----------------------------------------------------------------------------
void pqCMBGeologyBuilderMainWindowCore::colorObjects(
  ...
  pqCMBSceneObjectBase::enumObjectType enObjType)
{
  ...
  pqCMBSceneObjectBase *obj;
  SceneObjectNodeIterator iter(this->Tree->getRoot());
  iter.addObjectTypeFilter(enObjType);

  while((node = iter.next()))
    {
    obj = node->getDataObject();
    if(obj && obj->getRepresentation())
      {
      RepresentationHelperFunctions::CMB_COLOR_REP_BY_ARRAY(
        obj->getRepresentation()->getProxy(), arrayName, association);
      }
    }
}
