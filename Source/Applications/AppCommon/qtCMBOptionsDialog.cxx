...
int qtCMBOptionsDialogModel::rowCount(const QModelIndex& parentIndex) const
{
  ...
  {
    item = reinterpret_cast<qtCMBOptionsDialogModelItem*>(parentIndex.internalPointer());
  }
...
QModelIndex qtCMBOptionsDialogModel::index(
...
QModelIndex qtCMBOptionsDialogModel::getIndex(qtCMBOptionsDialogModelItem* item) const
{
  if (item && item->Parent)
  {
  ...
  : QDialog(widgetParent)
{
  ...
  this->connect(this->Form->ApplyButton, SIGNAL(clicked()), this, SLOT(applyChanges()));
  this->connect(this->Form->ResetButton, SIGNAL(clicked()), this, SLOT(resetChanges()));
  this->connect(this->Form->CloseButton, SIGNAL(clicked()), this, SLOT(accept()));
  this->connect(this->Form->RestoreDefaults, SIGNAL(clicked()), this, SLOT(restoreDefaults()));
}
...
void qtCMBOptionsDialog::addOptions(const QString& path, qtCMBOptionsPage* options)
{
  ...
  if (options->isApplyUsed())
  {
    ...
    this->connect(options, SIGNAL(changesAvailable()), this, SLOT(enableButtons()));
  }
  ...
  while (iter != this->Form->Pages.end())
  {
    if (*iter == options)
    {
      ...
      if (this->Form->Model->removeIndex(index))
      {
        ...
        while (parentIndex.isValid() && this->Form->Model->rowCount(parentIndex) == 0)
        {
          index = parentIndex;
