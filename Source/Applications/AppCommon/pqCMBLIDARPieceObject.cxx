/*=========================================================================
  ...
  Program:   CMB
  Module:    pqCMBLIDARPieceObject.cxx
  ...
  if (this->UseElevationFilter)
    ...
    if (!this->Highlight)
      ...
      RepresentationHelperFunctions::CMB_COLOR_REP_BY_ARRAY(
        this->Representation->getProxy(),
        "Elevation", vtkDataObject::POINT, false);
      }
    }
...
//-----------------------------------------------------------------------------
pqCMBLIDARPieceObject *pqCMBLIDARPieceObject::createObject(pqPipelineSource* source,
                                                         double *bounds,
                                                         pqServer * /*server*/,
                                                         pqRenderView *view,
                                                         bool updateRep)
{
  ...
  obj->ThresholdSource = builder->createFilter("filters","PointThresholdFilter",obj->DiggerSource);
  obj->ElevationSource = builder->createFilter("filters","LIDARElevationFilter", obj->ThresholdSource);
  ...
  vtkSMPropertyHelper(obj->Representation->getProxy(),
    "AmbientColor").Get(obj->OriginalColor, 3);
  ...
  vtkSMPropertyHelper(obj->Representation->getProxy(), "MapScalars").Set(0);
    RepresentationHelperFunctions::CMB_COLOR_REP_BY_ARRAY(
      obj->Representation->getProxy(),
      "Color", vtkDataObject::POINT, false);
...
void pqCMBLIDARPieceObject::getBounds(double bounds[6]) const
{
  vtkSMRepresentationProxy::SafeDownCast(
    this->Representation->getProxy())->
    GetRepresentedDataInformation()->GetBounds(bounds);
}
...
{
   vtkSMSourceProxy::SafeDownCast(this->getElevationSource() ?
    this->getElevationSource()->getProxy() : this->getSource()->getProxy())
  ...
  if (this->Highlight != highlight)
    ...
    if(highlight)
      ...
      RepresentationHelperFunctions::CMB_COLOR_REP_BY_ARRAY(rep->getProxy(),
        NULL, vtkDataObject::POINT, false);
      vtkSMPropertyHelper(rep->getProxy(), "DiffuseColor").Set(
...
//----------------------------------------------------------------------------
bool pqCMBLIDARPieceObject::isObjectUpToDate(bool clipEnabled,
...
//----------------------------------------------------------------------------
bool pqCMBLIDARPieceObject::isClipUpToDate(bool clipEnabled,
                                          vtkBoundingBox &clipBBox)
{
  // is our clipping state up-to-date
  if (clipEnabled != this->ClipState ||
    (clipEnabled && (!this->areClippingBoundsEqual(clipBBox) ||
...
bool pqCMBLIDARPieceObject::isClipTransformationUnchanged()
{
  ...
  if (position[0] != this->ClipPosition[0] ||
    position[1] != this->ClipPosition[1] ||
    position[2] != this->ClipPosition[2] ||
    scale[0] != this->ClipScale[0] ||
    scale[1] != this->ClipScale[1] ||
    scale[2] != this->ClipScale[2] ||
    orientation[0] != this->ClipOrientation[0] ||
    orientation[1] != this->ClipOrientation[1] ||
    orientation[2] != this->ClipOrientation[2] ||
    ((origin[0] != this->ClipOrigin[0] ||
    origin[1] != this->ClipOrigin[1] ||
...
{
  vtkSMPropertyHelper(this->Representation->getProxy(),
                      "Position").Get(this->ClipPosition, 3);
}
...
{
  vtkSMPropertyHelper(this->Representation->getProxy(),
                      "Orientation").Get(this->ClipOrientation, 3);
}
...
bool pqCMBLIDARPieceObject::isThresholdTransformationUnchanged()
{
  ...
  if (position[0] != this->ThresholdPosition[0] ||
    position[1] != this->ThresholdPosition[1] ||
    position[2] != this->ThresholdPosition[2] ||
    scale[0] != this->ThresholdScale[0] ||
    scale[1] != this->ThresholdScale[1] ||
    scale[2] != this->ThresholdScale[2] ||
    ...
    ((origin[0] != this->ThresholdOrigin[0] ||
    origin[1] != this->ThresholdOrigin[1] ||
...
{
  vtkSMPropertyHelper(this->Representation->getProxy(),
                      "Position").Get(this->ThresholdPosition, 3);
}
...
{
  vtkSMPropertyHelper(this->Representation->getProxy(),
                      "Orientation").Get(this->ThresholdOrientation, 3);
}
...
{
  vtkSMPropertyHelper(this->Representation->getProxy(),
                      "Scale").Get(this->ThresholdScale, 3);
}
...
{
  vtkSMPropertyHelper(this->Representation->getProxy(),
                      "Origin").Get(this->ThresholdOrigin, 3);
}
...
void pqCMBLIDARPieceObject::updateThresholdUseFilter(int idx, int useFilter)
{
  this->updateProxyProperty(
    this->getThresholdSource(), "SetUseFilter", idx, useFilter);
}
...
{
  this->updateProxyProperty(
    this->getContourSource(), "ClipApplyPolygon", idx, useFilter);
}
...
{
  this->updateProxyProperty(
    this->getContourSource(), "InsideOut", idx, invert);
}
...
{
  this->updateProxyProperty(
    this->getContourSource(), "ClipAsROI", idx, roi);
}
  ...
  pqPipelineSource* source, const char* name, int idx, int val)
{
  ...
  pqSMAdaptor::setMultipleElementProperty(
    smSource->GetProperty(name), 0, idx);
  pqSMAdaptor::setMultipleElementProperty(
    smSource->GetProperty(name), 1, val);
...
{
  vtkSMSourceProxy* currentThresholds = vtkSMSourceProxy::SafeDownCast(
    this->getThresholdSource()->getProxy());
...
{
  vtkSMSourceProxy* thresholdSourceProxy = vtkSMSourceProxy::SafeDownCast(
    this->getThresholdSource()->getProxy());
...
void pqCMBLIDARPieceObject::removeThreshold()
{
  ...
  pqSMAdaptor::setElementProperty(
    thresholdSourceProxy->GetProperty("SetActiveFilterIndex"), 0);
...
bool pqCMBLIDARPieceObject::hasActiveFilters()
{
  ...
  if ( pqSMAdaptor::getElementProperty(this->getThresholdSource()->getProxy()->
      GetProperty("NumberOfActiveThresholdSets")).toInt() ||
      pqSMAdaptor::getElementProperty(this->getContourSource()->getProxy()->
      GetProperty("NumberOfActivePolygons")).toInt())
    {
    return true;
    }
  return false;
...
void pqCMBLIDARPieceObject::setActiveContourGroup(int groupId)
{
  ...
  pqSMAdaptor::setElementProperty(
                                  contourProxy->GetProperty("ActiveGroupIndex"), groupId);
