...
struct qtCMBProjectServerManager::ProjectDetails
{
  ...
  void init()
  {
    ...
    ProgramFileRegExps[0] =
      QRegExp(".*.(?:pts|bin|pts.bin|las)$", Qt::CaseInsensitive, QRegExp::RegExp2);
    ProgramFileRegExps[1] =
      QRegExp(".*.(?:sg|osd.txt|map)$", Qt::CaseInsensitive, QRegExp::RegExp2);
    ProgramFileRegExps[2] =
      QRegExp(".*.(?:cmb|vtk|vtu|2dm|3dm|sol|stl|map)$", Qt::CaseInsensitive, QRegExp::RegExp2);
    ProgramFileRegExps[3] = QRegExp(".*.(?:simb.xml)$", Qt::CaseInsensitive, QRegExp::RegExp2);
  }
...
void qtCMBProjectServerManager::serverOpenProject(QString const& project)
{
  ...
  if (pxm)
  {
    ...
    pqSMAdaptor::setElementProperty(
      readerProxy->GetProperty("ProjectFile"), project.toStdString().c_str());
  ...
  qtCMBProjectServerManager::PROGRAM const& program)
{
  ...
  {
    this->ProjectManagerProxy.reset(pxm->NewProxy("utilities", "ProjectManager"));
  ...
  vtkSMPropertyHelper helperPD(this->ProjectManagerProxy.data(), "ActiveDefaultProgramDirectory");
  ...
  qtCMBProjectServerManager::PROGRAM const& program, QString const& directory)
{
  ...
  vtkSMPropertyHelper helper2(this->ProjectManagerProxy.data(), "SetActiveProgramDirectory");
...
void qtCMBProjectServerManager::pullFromServer()
{
  ...
  for (int i = 0; i < NUM_PROGRAMS; ++i)
  {
    ...
    vtkSMPropertyHelper helper4(this->ProjectManagerProxy.data(), "ActiveProgramDirectory");
