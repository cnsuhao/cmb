...
void vtkPythonAppInitPrependPathWindows(const std::string& SELF_DIR)
{
  ...
  build_dir_site_packages = SELF_DIR + "/../lib/site-packages";
#endif
  bool is_build_dir = vtksys::SystemTools::FileExists(build_dir_site_packages.c_str());
  if (is_build_dir)
  {
...
void vtkPythonAppInitPrependPathOsX(const std::string& SELF_DIR)
{
  bool is_app = false;
  {
    ...
    std::string contents_dir = vtksys::SystemTools::CollapseFullPath((SELF_DIR + "/..").c_str());
  ...
  bool is_unix_style_install =
    vtksys::SystemTools::FileExists((lib_dir + "/paraview-" PARAVIEW_VERSION).c_str());
  if (is_build_dir)
  {
...
void vtkPythonAppInitPrependPathLinux(const std::string& SELF_DIR)
{
  ...
  //    directory.
  bool is_build_dir = vtksys::SystemTools::FileExists((SELF_DIR + "/../CMBConfig.cmake").c_str());
  if (is_build_dir)
  {
    vtkPythonAppInitPrependPythonPath(SELF_DIR + "/../lib");
