...
int pqCMBSceneV1Reader::process(vtkXMLDataElement* root)
{
  ...
  this->Progress = new QProgressDialog("", "Abort Load", 0, num, this->Tree->getWidget());
...
pqCMBSceneObjectBase* pqCMBSceneV1Reader::processContour(vtkXMLDataElement* elem)
{
  ...
  */
  obj = this->Tree->getArcWidgetManager()->createLegacyV1Contour(
    positionNormal, positionOffset, closedLoop, nodePositions, SelIndices);
  ...
  vtkXMLDataElement* elem, int surfaceType)
{
  ...
  {
    fobj = new pqCMBFacetedObject(this->FileNames[index].c_str(), this->Tree->getCurrentServer(),
...
void pqCMBSceneV1Reader::processObject(vtkXMLDataElement* elem)
{
  ...
  int stype;
  pqCMBSceneObjectBase::enumObjectType otype = static_cast<pqCMBSceneObjectBase::enumObjectType>(
