...
vtkSMSourceProxy* pqCMBTexturedObject::getSelectionInput() const
{
  ...
  proxy = vtkSMSourceProxy::SafeDownCast(this->ElevationFilter->getProxy());
...
void pqCMBTexturedObject::duplicateInternals(pqCMBSceneObjectBase* o)
{
  ...
  if (this->TextureImageSource)
  {
    nobj->setTextureMap(this->TextureFileName.toStdString().c_str(),
      this->NumberOfRegistrationPoints, this->RegistrationPoints);
  }
...
void pqCMBTexturedObject::prepTexturedObject(pqServer* /*server*/, pqRenderView* view)
{
  ...
  this->setRepresentation(builder->createDataRepresentation(this->ElevationFilter->getOutputPort(0),
...
void pqCMBTexturedObject::showElevation(bool flag)
{
  ...
  {
    vtkSMPropertyHelper(this->ElevationFilter->getProxy(), "CreateElevation").Set(1);
    double bounds[6], p[3];
    p[0] = p[1] = 0.0;
    this->getDataBounds(bounds);
    p[2] = bounds[4];
    vtkSMPropertyHelper(this->ElevationFilter->getProxy(), "LowPoint").Set(p, 3);
    p[2] = bounds[5];
    vtkSMPropertyHelper(this->ElevationFilter->getProxy(), "HighPoint").Set(p, 3);
  }
  else
  {
    vtkSMPropertyHelper(this->ElevationFilter->getProxy(), "CreateElevation").Set(0);
  ...
  vtkSMPropertyHelper(this->getRepresentation()->getProxy(), "MapScalars").Set(this->ShowElevation);
  RepresentationHelperFunctions::CMB_COLOR_REP_BY_ARRAY(this->getRepresentation()->getProxy(),
...
void pqCMBTexturedObject::unsetTextureMap()
{
  ...
  if (this->TextureImageSource)
  {
    pqApplicationCore* core = pqApplicationCore::instance();
    pqObjectBuilder* builder = core->getObjectBuilder();
    vtkSMPropertyHelper(this->getRepresentation()->getProxy(), "LargeTextureInput")
      .Set(static_cast<vtkSMProxy*>(0));
...
{
  if (this->hasTexture() && this->TextureImageSource && this->TextureFileName.compare(filename))
  {
  ...
  if (!this->TextureImageSource)
  {
    this->TextureImageSource = RepresentationHelperFunctions::ReadTextureImage(
      pqApplicationCore::instance()->getObjectBuilder(), this->Source->getServer(), filename);
    this->TextureFileName = filename;
    vtkSMPropertyHelper(this->getRepresentation()->getProxy(), "LargeTextureInput")
      .Set(this->TextureImageSource->getProxy());
    this->getRepresentation()->getProxy()->UpdateProperty("LargeTextureInput");
    ...
    ev[0] = extents[0];
    ev[1] = extents[1];
    vtkSMPropertyHelper(this->RegisterTextureFilter->getProxy(), "SRange").Set(ev, 2);
    ev[0] = extents[2];
    ev[1] = extents[3];
    vtkSMPropertyHelper(this->RegisterTextureFilter->getProxy(), "TRange").Set(ev, 2);
  }

  if (numberOfRegistrationPoints == 2)
  {
    vtkSMPropertyHelper(this->RegisterTextureFilter->getProxy(), "TwoPointRegistration")
      .Set(this->RegistrationPoints, 8);
  }
  else
  {
    vtkSMPropertyHelper(this->RegisterTextureFilter->getProxy(), "ThreePointRegistration")
      .Set(this->RegistrationPoints, 12);
  }

  vtkSMPropertyHelper(this->RegisterTextureFilter->getProxy(), "GenerateCoordinates").Set(1);
  this->RegisterTextureFilter->getProxy()->UpdateVTKObjects();
  vtkSMSourceProxy::SafeDownCast(this->RegisterTextureFilter->getProxy())->UpdatePipeline();

  this->getRepresentation()->getProxy()->UpdateVTKObjects();

  if (this->TexturePointIntensityFilter)
  {
    pqApplicationCore::instance()->getObjectBuilder()->destroy(this->TexturePointIntensityFilter);
    this->TexturePointIntensityFilter = 0;
  }
}
...
double pqCMBTexturedObject::getTextureIntensityAtPoint(double pt[3])
{
  if (!this->TexturePointIntensityFilter)
  {
    ...
    this->TexturePointIntensityFilter = builder->createFilter("filters",
      "TexturePointIntensityFilter", namedInputs, this->RegisterTextureFilter->getServer());
  }
