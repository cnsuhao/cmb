...
{
  this->Constraints[0] = this->Constraints[1] = this->Constraints[2] =
    this->Constraints[3] = this->Constraints[4] = this->Constraints[5] =
    this->Constraints[6] = this->Constraints[7] = this->Constraints[8] =
    this->Constraints[9] = false;
}

//-----------------------------------------------------------------------------
void pqCMBSceneObjectBase::DoRepresentationCallback(vtkObject* /*vtk_obj*/, unsigned long /*event*/,
...
int pqCMBSceneObjectBase::getPickable()
{
  if (this->Representation)
    {
    return pqSMAdaptor::getElementProperty(this->Representation->
      getProxy()->GetProperty("Pickable")).toInt();
    }
  return 0;
}
//-----------------------------------------------------------------------------
...
void pqCMBSceneObjectBase::setRepresentation(pqDataRepresentation *rep)
{
  if ( this->Representation )
    ...
    oldProxy->GetProperty("Orientation")->RemoveObservers(
      this->Internal->OrientationTag);
    oldProxy->GetProperty("Position")->RemoveObservers(
      this->Internal->PositionTag);
    oldProxy->GetProperty("Scale")->RemoveObservers(
      this->Internal->ScaleTag);
    }
  ...
  if(this->Representation)
    ...
    if ( srcProxy )
      {
      this->Internal->OrientationTag =
        srcProxy->GetProperty("Orientation")->AddObserver(
        vtkCommand::ModifiedEvent, this->Internal->CallBack);
      this->Internal->PositionTag =
        srcProxy->GetProperty("Position")->AddObserver(
        vtkCommand::ModifiedEvent, this->Internal->CallBack);
      this->Internal->ScaleTag =
        srcProxy->GetProperty("Scale")->AddObserver(
        vtkCommand::ModifiedEvent, this->Internal->CallBack);
      }
    }
}
...
void pqCMBSceneObjectBase::duplicateInternals(pqCMBSceneObjectBase *nobj)
{
  ...
  destProxy->GetProperty("Orientation")->
    Copy(srcProxy->GetProperty("Orientation"));

  destProxy->GetProperty("Position")->
    Copy(srcProxy->GetProperty("Position"));

  destProxy->GetProperty("Scale")->
    Copy(srcProxy->GetProperty("Scale"));

  destProxy->GetProperty("DiffuseColor")->
    Copy(srcProxy->GetProperty("DiffuseColor"));
}
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
pqCMBSceneObjectBase::enumObjectType
pqCMBSceneObjectBase::convertStringToType(const char *typeName)
{
...
//-----------------------------------------------------------------------------
std::string pqCMBSceneObjectBase::convertSurfaceTypeToString(pqCMBSceneObjectBase::enumSurfaceType t)
{
  switch(t)
    {
...
//-----------------------------------------------------------------------------
pqCMBSceneObjectBase::enumSurfaceType
pqCMBSceneObjectBase::convertStringToSurfaceType(const char *typeName)
{
  std::string name = typeName;
...
void pqCMBSceneObjectBase::getColor(double color[4]) const
{
  ...
  color[3] = vtkSMPropertyHelper(this->Representation->getProxy(),
    "Opacity").GetAsDouble();
}
...
{
  GetRepresentationTransformedBounds(this->Internal->Transform,
    this->Representation, inBB);
}
...
//-----------------------------------------------------------------------------
void pqCMBSceneObjectBase::updateVectorProperty(const char* name,
  double *v,const int &size, const bool &updateRep)
{
...
void pqCMBSceneObjectBase::getAreaStats(double* areaStats)
{
  ...
  for (int i = 0; i < 3; i++)
    {
    areaStats[i] = pqSMAdaptor::getMultipleElementProperty(fproxy->GetProperty("AreaStats"),i).toDouble();
    }
}
//-----------------------------------------------------------------------------
void pqCMBSceneObjectBase::getGeometryBounds(double* geoBounds) const
{
  ...
  for (int i = 0; i < 6; i++)
    {
    geoBounds[i] = pqSMAdaptor::getMultipleElementProperty(fproxy->GetProperty("GeometryBounds"),i).toDouble();
    }
}
//-----------------------------------------------------------------------------
void pqCMBSceneObjectBase::getPolySideStats(double* polySide)
{
  ...
  for (int i = 0; i < 3; i++)
    {
    polySide[i] = pqSMAdaptor::getMultipleElementProperty(fproxy->GetProperty("PolygonalSideStats"),i).toDouble();
    }
}
//-----------------------------------------------------------------------------
...
void pqCMBSceneObjectBase::updatePolyDataStats()
{
  ...
  if (!this->PolyDataStatsFilter)
    ...
    pqApplicationCore* core = pqApplicationCore::instance();
    pqObjectBuilder* builder = core->getObjectBuilder();
    this->PolyDataStatsFilter = builder->createFilter("filters",
                                                "PolyDataStatsFilter",
                                                this->Source);
    }
...
void pqCMBSceneObjectBase::getClosestPoint(const double p[3], double cp[3])
{
  ...
  if (!this->ClosestPointFilter)
    ...
    pqApplicationCore* core = pqApplicationCore::instance();
    pqObjectBuilder* builder = core->getObjectBuilder();
    this->ClosestPointFilter = builder->createFilter("filters",
                                                     "ClosestPointFilter",
                                                     this->Source);
    }
  ...
  fproxy->UpdatePropertyInformation();
  double *data =
    vtkSMDoubleVectorProperty::SafeDownCast(fproxy->GetProperty("ClosestPoint"))
    ->GetElements();
  cp[0] = data[0];
  cp[1] = data[1];
  cp[2] = data[2];
...
void pqCMBSceneObjectBase::setLODMode(int mode, bool updateRep)
{
  vtkSMIntVectorProperty::SafeDownCast(this->Representation->
                                       getProxy()->GetProperty("SuppressLOD"))
    ->SetElements1(mode);
  if (updateRep)
    {
    this->Representation->getProxy()->UpdateVTKObjects();
...
//-----------------------------------------------------------------------------
void pqCMBSceneObjectBase::applyTransform(double scaleDelta[3],
