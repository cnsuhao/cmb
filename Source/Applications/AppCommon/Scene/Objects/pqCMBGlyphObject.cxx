...
//-----------------------------------------------------------------------------
pqCMBGlyphObject::pqCMBGlyphObject() : pqCMBSceneObjectBase()
{
  this->SurfaceType = pqCMBSceneObjectBase::Other;
}
//-----------------------------------------------------------------------------
pqCMBGlyphObject::pqCMBGlyphObject(pqPipelineSource *glyphSource,
                                         pqRenderView *view,
                                         pqServer *server,
                                         ...
                                         bool updateRep)
{
  ...
  vtkSMCMBGlyphPointSourceProxy::
    SafeDownCast(this->Source->getProxy())->ReadFromFile(pointsFilename);
...
//-----------------------------------------------------------------------------
pqCMBGlyphObject::pqCMBGlyphObject(const char *glyphFilename,
                                         pqServer *server,
                                         pqRenderView *view,
                                         bool updateRep)
{
...
void pqCMBGlyphObject::initialize(pqRenderView *view, pqServer * /*server*/, bool updateRep)
{
  ...
  RepresentationHelperFunctions::CMB_COLOR_REP_BY_ARRAY(
    repProxy, "Color", vtkDataObject::POINT);
  ...
  vtkSMPropertyHelper(repProxy,
    "SelectScaleArray").Set("Scaling");
  vtkSMPropertyHelper(repProxy,
    "SelectOrientationVectors").Set("Orientation");
...
pqPipelineSource *pqCMBGlyphObject::duplicateGlyphPipelineSource(pqServer *server)
{
  ...
  vtkSMDataSourceProxy::SafeDownCast(pdSource->getProxy())->CopyData(
    vtkSMSourceProxy::SafeDownCast(this->GlyphSource->getProxy()));
  return pdSource;
}
//-----------------------------------------------------------------------------
pqCMBSceneObjectBase *pqCMBGlyphObject::duplicate(pqServer *server,
                                                    pqRenderView *view,
                                                    bool updateRep)
{
  pqApplicationCore* core = pqApplicationCore::instance();
  ...
  pqCMBGlyphObject *nobj = new pqCMBGlyphObject(pdSource, view, server,
                                                      this->GlyphFileName.c_str());
...
void pqCMBGlyphObject::getAveragePoint(double *pa) const
{
  ...
  for (i = 0; i < n; i++)
    {
    const_cast<vtkSMCMBGlyphPointSourceProxy*>
      (this->SourceProxy)->GetPoint(i, p);
    pa[0] += p[0];
    pa[1] += p[1];
    pa[2] += p[2];
    }
...
{
  const_cast<vtkSMCMBGlyphPointSourceProxy*>
    (this->SourceProxy)->GetOrientation(i, p);
}
...
{
  const_cast<vtkSMCMBGlyphPointSourceProxy*>
    (this->SourceProxy)->SetOrientation(i, p);
}
...
void pqCMBGlyphObject::writePointsFile() const
{
  if (this->PointsFileName != "")
    {
    const_cast<vtkSMCMBGlyphPointSourceProxy*>
      (this->SourceProxy)->WriteToFile(this->PointsFileName.c_str());
    }
}
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void pqCMBGlyphObject::applyTransform(double scaleDelta[3],
                                         double orientationDelta[3],
                                         double translationDelta[3])
{
  this->SourceProxy->
    ApplyTransform(orientationDelta, translationDelta, scaleDelta);
  this->getRepresentation()->getProxy()->UpdateVTKObjects();
}
//-----------------------------------------------------------------------------
