...
class qtCMBPlacementConstraintWidgetInternal : public Ui::qtObjectPlacementConstraint
{
public:
  ...
  , parentNode(node)
{
  ...
  QObject::connect(this->Internal->GlyphPointsFileBrowserButton, SIGNAL(clicked()), this,
    SLOT(displayGlyphPlaybackFileBrowser()));
  QObject::connect(this->Internal->GlyphPlaybackOption, SIGNAL(currentIndexChanged(int)), this,
...
void qtCMBPlacementConstraintWidget::setupConstraintTable()
{
  ...
  table->setHorizontalHeaderLabels(QStringList() << tr("Use") << tr("Constraint") << tr("Outside"));

  table->setSelectionMode(QAbstractItemView::SingleSelection);
  table->setSelectionBehavior(QAbstractItemView::SelectRows);
  table->verticalHeader()->hide();

  QObject::connect(table, SIGNAL(itemSelectionChanged()), this, SLOT(onTableSelectionChanged()),
    Qt::QueuedConnection);
...
const QMap<pqCMBSceneNode*, int>& qtCMBPlacementConstraintWidget::getSelectedConstraints() const
{
  ...
  for (int row = 0; row < table->rowCount(); row++)
  {
    pqCMBSceneNode* node = static_cast<pqCMBSceneNode*>(
      this->Internal->tableWidget->item(row, UseCol)->data(Qt::UserRole).value<void*>());
...
void qtCMBPlacementConstraintWidget::displayGlyphPlaybackFileBrowser()
{
  ...
  pqFileDialog file_dialog(
    tree->getCurrentServer(), this, tr("Open/Save As File:"), QString(), filters);
  ...
  QObject::connect(&file_dialog, SIGNAL(filesSelected(const QList<QStringList>&)), this,
    SLOT(filesSelected(const QList<QStringList>&)));
...
void qtCMBPlacementConstraintWidget::checkGlyphPlaybackFile(int option)
{
  if (option == 0)
  {
    ...
    {
      if (QMessageBox::No == QMessageBox::warning(this, this->windowTitle(),
                               QString(tr("%1 already exists. \nDo you want to replace it?"))
                                 .arg(this->Internal->GlyphPointsFileNameText->text()),
  ...
  else if (option == 1)
  {
    ...
    if (!this->fileExists(this->Internal->GlyphPointsFileNameText->text()))
    {
      QMessageBox::warning(this, this->windowTitle(),
        QString(tr("%1 does not exist. \nSelect a valid glyph playback file."))
          .arg(this->Internal->GlyphPointsFileNameText->text()));
      displayGlyphPlaybackFileBrowser();
    }
  }
