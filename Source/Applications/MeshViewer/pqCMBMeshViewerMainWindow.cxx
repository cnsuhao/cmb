...
pqCMBMeshViewerMainWindow::~pqCMBMeshViewerMainWindow()
{
  ...
  {
    this->getThisCore()->deleteContourWidget(this->Internal->CurrentContour);
  }
  if (this->Internal->BoxWidget)
  {
    vtkSMProxyManager* pxm = vtkSMProxyManager::GetProxyManager();
    pxm->UnRegisterProxy("3d_widgets_prototypes",
      pxm->GetProxyName("3d_widgets_prototypes", this->Internal->BoxWidget),
  ...
  if (this->Internal->PlaneWidget)
  {
    vtkSMProxyManager* pxm = vtkSMProxyManager::GetProxyManager();
    pxm->UnRegisterProxy("3d_widgets_prototypes",
      pxm->GetProxyName("3d_widgets_prototypes", this->Internal->PlaneWidget),
...
void pqCMBMeshViewerMainWindow::initializeApplication()
{
  ...
  QObject::connect(mainWinCore->pvSelectionManager(), SIGNAL(selectionChanged(pqOutputPort*)), this,
    SLOT(updateSelection()));

  QObject::connect(mainWinCore, SIGNAL(newMeshLoaded()), this, SLOT(onMeshLoaded()));
  QObject::connect(mainWinCore, SIGNAL(meshModified()), this, SLOT(onMeshModified()));
  QObject::connect(mainWinCore, SIGNAL(filterPropertiesChanged(bool)), this,
    SLOT(onFilterPropertiesModified(bool)));

  this->getMainDialog()->toolBar_Selection->insertAction(
    this->getMainDialog()->action_Select, this->getMainDialog()->action_SelectThrough);
  this->getMainDialog()->toolBar_Selection->insertAction(
    this->getMainDialog()->action_Select, this->getMainDialog()->action_SelectPoints);

  QObject::connect(this->getMainDialog()->action_SelectThrough, SIGNAL(triggered(bool)),
    mainWinCore, SLOT(onFrustumSelect(bool)));
  QObject::connect(this->getMainDialog()->action_SelectPoints, SIGNAL(triggered(bool)), mainWinCore,
    SLOT(onRubberBandSelectPoints(bool)));
  ...
  this->getMainDialog()->toolBar_Selection->insertAction(
    this->getMainDialog()->action_SelectThrough, this->getMainDialog()->actionExtract_Subset);
  this->getMainDialog()->toolBar_Selection->insertAction(
    this->getMainDialog()->action_SelectThrough,
  ...
  QObject::connect(this->getMainDialog()->actionExtract_Subset, SIGNAL(triggered()), this,
    SLOT(extractSelection()));
  this->getMainDialog()->actionExtract_Subset->setEnabled(0);

  QObject::connect(this->getMainDialog()->action_Select, SIGNAL(triggered(bool)), mainWinCore,
    SLOT(onRubberBandSelectCell(bool)));
  QObject::connect(
    this->getMainDialog()->action_FindData, SIGNAL(triggered()), this, SLOT(showQueryDialog()));
  QObject::connect(this->getMainDialog()->actionSelect_Contour_Surface_Point, SIGNAL(triggered()),
    this, SLOT(onDefineContourWidget()));
  QObject::connect(this->getMainDialog()->actionMove_Selected_Contour_Points, SIGNAL(triggered()),
    this, SLOT(onInvokeBoxWidget()));
  QObject::connect(this->getMainDialog()->actionPush_Selected_Contour_Points, SIGNAL(triggered()),
    this, SLOT(onInvokePlaneWidget()));
  QObject::connect(this->getMainDialog()->actionSelect_Contour_Through_Cells, SIGNAL(triggered()),
    this, SLOT(onDefineContourWidget()));
  QObject::connect(this->getMainDialog()->actionSelect_Contour_Surface_Cell, SIGNAL(triggered()),
    this, SLOT(onDefineContourWidget()));

  QObject::connect(this->getMainDialog()->actionEdit_Conical_Region, SIGNAL(triggered()), this,
    SLOT(onEditConeSelection()));
  QObject::connect(this->getMainDialog()->actionSelect_Cone_Cells, SIGNAL(triggered()), this,
    SLOT(onStartConeSelection()));
  ...
  QObject::connect(this->Internal->MeshViewerPanel->getGUIPanel()->pushButton_Apply,
    SIGNAL(clicked()), this, SLOT(applyFilters()));
  QObject::connect(this->Internal->MeshViewerPanel->getGUIPanel()->pushButton_Reset,
    SIGNAL(clicked()), mainWinCore, SLOT(resetFilterPanels()));

  mainWinCore->setupAppearanceEditor(
    this->Internal->MeshViewerPanel->getGUIPanel()->appearanceFrame);
  mainWinCore->setupInspectorPanel(this->Internal->MeshViewerPanel->getGUIPanel()->tab_Mesh);
  mainWinCore->setupSelectionPanel(this->Internal->MeshViewerPanel->getGUIPanel()->frame_Histogram);
  ...
  QObject::connect(this->Internal->SelectionMenu, SIGNAL(currentIndexChanged(int)), this,
    SLOT(setSelectionMode()));
  ...
  QObject::connect(this->Internal->MeshViewerPanel->getGUIPanel()->comboBox_Histogram,
    SIGNAL(currentIndexChanged(int)), mainWinCore, SLOT(setHistogramMode(int)));
  ...
  QObject::connect(this->Internal->MeshViewerPanel->getGUIPanel()->pushButton_ChangeMaterial,
    SIGNAL(clicked()), this, SLOT(onChangeSelectionMaterialId()));
  QObject::connect(this->Internal->MeshViewerPanel->getGUIPanel()->MaterialIdEntry,
    SIGNAL(textChanged(const QString&)), this, SLOT(updateChangeMaterialButton()));

  QObject::connect(this->Internal->MeshViewerPanel->getGUIPanel()->pushButton_InvertSelection,
    SIGNAL(clicked()), this, SLOT(invertSelection()));
  this->Internal->MeshViewerPanel->getGUIPanel()->pushButton_InvertSelection->setEnabled(0);

  QTreeWidget* treeWidget = this->inputTreeWidget();

  QObject::connect(treeWidget, SIGNAL(itemClicked(QTreeWidgetItem*, int)), this,
    SLOT(onInputClicked(QTreeWidgetItem*, int)), Qt::QueuedConnection);
  QObject::connect(treeWidget, SIGNAL(itemSelectionChanged()), this,
    SLOT(onInputSelectionChanged()), Qt::QueuedConnection);
  QObject::connect(treeWidget, SIGNAL(itemChanged(QTreeWidgetItem*, int)), this,
    SLOT(onInputNameChanged(QTreeWidgetItem*, int)), Qt::QueuedConnection);

  QObject::connect(this->Internal->MeshViewerPanel->getGUIPanel()->checkBoxShowCell,
    SIGNAL(toggled(bool)), this, SLOT(showCellLabel(bool)));
  QObject::connect(this->Internal->MeshViewerPanel->getGUIPanel()->checkBoxShowPoint,
    SIGNAL(toggled(bool)), this, SLOT(showPointLabel(bool)));
  QObject::connect(this->Internal->MeshViewerPanel->getGUIPanel()->pushButton_ClearSelection,
    SIGNAL(clicked()), this, SLOT(onClearSelection()));
  QObject::connect(this->Internal->MeshViewerPanel->getGUIPanel()->pushButton_SelectAll,
    SIGNAL(clicked()), this, SLOT(onSelectAll()));

  QObject::connect(this->Internal->MeshViewerPanel->activeInputAction(), SIGNAL(triggered()), this,
    SLOT(onInputChanged()));
  QObject::connect(this->Internal->MeshViewerPanel->exportSubsetAction(), SIGNAL(triggered()), this,
    SLOT(onExportSubset()));
  QObject::connect(this->Internal->MeshViewerPanel->deleteInputAction(), SIGNAL(triggered()), this,
    SLOT(onRemoveInputSource()));
  ...
  QShortcut* eShortcut = new QShortcut(QKeySequence(tr("E")), this->centralWidget());
  eShortcut->setContext(Qt::WidgetShortcut);
  QObject::connect(eShortcut, SIGNAL(activated()), this, SLOT(onExtractShortcutActivated()));
  QShortcut* aShortcut = new QShortcut(QKeySequence(tr("A")), this->centralWidget());
  eShortcut->setContext(Qt::WidgetShortcut);
  QObject::connect(aShortcut, SIGNAL(activated()), this, SLOT(onSetActiveShortcutActivated()));
  QObject::connect(this->Internal->MeshViewerPanel->getGUIPanel()->contourAllIn, SIGNAL(clicked()),
    this, SLOT(onShapeSelectionOptionClicked()));
  QObject::connect(this->Internal->MeshViewerPanel->getGUIPanel()->contourPartiallyIn,
    SIGNAL(clicked()), this, SLOT(onShapeSelectionOptionClicked()));
  ...
  QObject::connect(this->Internal->MeshViewerPanel->getGUIPanel()->radioButtonShowHistogram,
    SIGNAL(toggled(bool)), this, SLOT(onSwitchHistogramAndSpreadsheet(bool)));
  ...
  QObject::connect(this->Internal->MeshViewerPanel->getGUIPanel()->pushButton_ApplySmoothing,
    SIGNAL(clicked()), this, SLOT(onApplySmoothing()));
  this->updateEnableState();
...
{
  vtkSMPropertyHelper(
    this->getThisCore()->activeRepresentation()->getProxy(), "SelectionPointFieldDataArrayName")
    .Set("Mesh Node ID");
  vtkSMPropertyHelper(
    this->getThisCore()->activeRepresentation()->getProxy(), "SelectionPointLabelVisibility")
    .Set(showLocal);
...
{
  vtkSMPropertyHelper(
    this->getThisCore()->activeRepresentation()->getProxy(), "SelectionCellFieldDataArrayName")
    .Set("Mesh Cell ID");
  vtkSMPropertyHelper(
    this->getThisCore()->activeRepresentation()->getProxy(), "SelectionCellLabelVisibility")
    .Set(showLocal);
  this->getThisCore()->activeRepresentation()->getProxy()->UpdateVTKObjects();
  this->getThisCore()->activeRenderView()->render();
}
...
void pqCMBMeshViewerMainWindow::setupMenuActions()
{
  ...
  this->getMainDialog()->menu_File->insertAction(
    this->getMainDialog()->action_Close, this->Internal->MeshViewerPanel->exportSubsetAction());

  // Add actions to "Edit" menu.
  this->getMainDialog()->menuEdit->addAction(this->getMainDialog()->action_FindData);
  this->getMainDialog()->menuEdit->insertAction(this->getMainDialog()->action_FindData,
    this->getMainDialog()->actionSelect_Contour_Surface_Point);
  this->getMainDialog()->menuEdit->insertAction(this->getMainDialog()->action_FindData,
    this->getMainDialog()->actionSelect_Contour_Surface_Cell);
  ...
  this->getMainDialog()->menuEdit->insertAction(
    this->getMainDialog()->action_FindData, this->getMainDialog()->action_SelectThrough);
  this->getMainDialog()->menuEdit->insertAction(this->getMainDialog()->action_FindData,
    this->getMainDialog()->actionSelect_Contour_Through_Cells);
  this->getMainDialog()->menuEdit->insertAction(
    this->getMainDialog()->action_FindData, this->getMainDialog()->actionExtract_Subset);
  this->getMainDialog()->menuEdit->insertAction(
...
void pqCMBMeshViewerMainWindow::showQueryDialog()
{
  ...
  QObject::connect(&dialog, SIGNAL(extractSelection()), this, SLOT(extractSelection()));
  ...
  pqSelectionManager* selManager = pqPVApplicationCore::instance()->selectionManager();
  if (selManager)
  {
    QObject::connect(
      &dialog, SIGNAL(showing(pqOutputPort*)), selManager, SLOT(select(pqOutputPort*)));
  }
  dialog.show();
  QEventLoop loop;
...
void pqCMBMeshViewerMainWindow::onMeshLoaded()
{
  ...
  if (this->getThisCore()->isMeshLoaded())
  {
    ...
    this->Internal->InformationWidget->setOutputPort(activeSource->getOutputPort(0));
    ...
    this->addInputItem(parentItem, this->getThisCore()->fullMeshRepresentation(), true);
    this->Internal->MeshViewerPanel->getGUIPanel()->comboBox_Histogram->setCurrentIndex(1);
...
void pqCMBMeshViewerMainWindow::onHelpAbout()
{
  ...
  dialog->setWindowTitle(QApplication::translate("Mesh Viewer AboutDialog", "About Mesh Viewer", 0
#if QT_VERSION < 0x050000
    ,
...
void pqCMBMeshViewerMainWindow::updateSelection()
{
  ...
  {
    QMessageBox::warning(
      this, "Selection Warning", "The active mesh must be visible to select mesh elements!");
    return;
  }

  // We need some logic here to handle different type of selections
  bool isShapeSel = this->getMainDialog()->actionSelect_Contour_Through_Cells->isChecked() ||
  ...
  vtkSMProxy* repr = this->getThisCore()->spreadSheetViewModel()->activeRepresentationProxy();

  // For 3D mesh, the move/push point actions should only be enabled
  // when the selection is NOT select-through type selection.
  bool bEnableMovePtsActions =
    (!this->getThisCore()->is2DMesh() &&
  ...
  this->getMainDialog()->actionPush_Selected_Contour_Points->setEnabled(bEnableMovePtsActions);
  this->getMainDialog()->actionMove_Selected_Contour_Points->setChecked(0);
  this->getMainDialog()->actionMove_Selected_Contour_Points->setEnabled(bEnableMovePtsActions);

  this->Internal->MeshViewerPanel->getGUIPanel()->pushButton_InvertSelection->setEnabled(
    hasSelection);
...
void pqCMBMeshViewerMainWindow::updateChangeMaterialButton()
{
  vtkSMSourceProxy* smSource =
    vtkSMSourceProxy::SafeDownCast(this->getThisCore()->activeSource()->getProxy());
  ...
  // line edit widget
  bool benable =
    enable && (this->Internal->MeshViewerPanel->getGUIPanel()->MaterialIdEntry->text() != "");

  this->getMainDialog()->actionExtract_Subset->setEnabled(enable);
  this->Internal->MeshViewerPanel->getGUIPanel()->pushButton_ChangeMaterial->setEnabled(benable);
  if (enable)
  {
    this->Internal->MeshViewerPanel->getGUIPanel()->tabWidget->setCurrentIndex(
      3); // show Selection tab
  }
}
...
void pqCMBMeshViewerMainWindow::onInputChanged()
{
  ...
  QTreeWidgetItem* previous = this->findItemFromSource(
    this->inputTreeWidget()->invisibleRootItem(), this->getThisCore()->currentInputSource());
  if (previous)
  {
...
void pqCMBMeshViewerMainWindow::extractSelection()
{
  ...
  pqDataRepresentation* subsetRep = this->getThisCore()->extractSelectionAsInput();
  if (subsetRep)
  {
    this->Internal->MeshViewerPanel->getGUIPanel()->tabWidget->setCurrentIndex(0);
  ...
  QTreeWidgetItem* parentItem, pqPipelineSource* currentS)
{
  ...
  {
    QTreeWidgetItem* theItem = this->findItemFromSource(parentItem->child(i), currentS);
    if (theItem)
    {
      return theItem;
    }
  ...
  QTreeWidgetItem* parentLocal, pqDataRepresentation* extractRep, bool select)
{
  ...
  {
    QString itemText = "Sel" + QString::number(this->Internal->NumberOfExtractedSubsets + 1);
...
{
  return item
    ? static_cast<pqDataRepresentation*>(item->data(TREE_INPUT_COL, Qt::UserRole).value<void*>())
    : NULL;
}
...
bool pqCMBMeshViewerMainWindow::isItemInActiveChain(QTreeWidgetItem* item)
{
  ...
  QTreeWidgetItem* meshItem = this->findItemFromSource(
    this->inputTreeWidget()->invisibleRootItem(), this->getThisCore()->meshSource());
...
void pqCMBMeshViewerMainWindow::onDefineContourWidget()
{
  ...
  QAction* const senderAction = qobject_cast<QAction*>(QObject::sender());
  if (!senderAction)
  {
    return;
  }
  ...
  QObject::connect(
    this->Internal->CurrentContour, SIGNAL(contourLoopClosed()), this, SLOT(onContourFinished()));
  QObject::connect(
    this->Internal->CurrentContour, SIGNAL(contourDone()), this, SLOT(onContourFinished()));
  QObject::connect(
    this->Internal->CurrentContour, SIGNAL(widgetEndInteraction()), this, SLOT(onContourChanged()));
  this->getMainDialog()->actionMove_Selected_Contour_Points->setChecked(0);
  this->getMainDialog()->actionMove_Selected_Contour_Points->setEnabled(0);
  this->getMainDialog()->actionPush_Selected_Contour_Points->setChecked(0);
  this->getMainDialog()->actionPush_Selected_Contour_Points->setEnabled(0);
...
void pqCMBMeshViewerMainWindow::onContourFinished()
{
  if (this->getMainDialog()->actionSelect_Contour_Surface_Point->isChecked() ||
    this->getMainDialog()->actionSelect_Contour_Surface_Cell->isChecked())
  {
    this->getThisCore()->contourSelectSurface(this->Internal->CurrentContour,
      this->getMainDialog()->actionSelect_Contour_Surface_Cell->isChecked(),
  ...
  else if (this->getMainDialog()->actionSelect_Contour_Through_Cells->isChecked())
  {
    ...
    this->getThisCore()->contourSelectThrough(this->Internal->CurrentContour, selectContourType);
  }
  this->updateSelection();
  this->getThisCore()->enableCameraInteractionModeChanges(true);
}
...
void pqCMBMeshViewerMainWindow::updateBoxWidget()
{
  if (this->Internal->BoxWidget)
  {
    ...
    {
      this->Internal->VTKBoxConnect = vtkSmartPointer<vtkEventQtSlotConnect>::New();
    }
...
void pqCMBMeshViewerMainWindow::updatePlaneWidget()
{
  if (this->Internal->PlaneWidget)
  {
    ...
    {
      this->Internal->VTKPlaneConnect = vtkSmartPointer<vtkEventQtSlotConnect>::New();
    }
    ...
    this->Internal->VTKPlaneConnect->Connect(this->Internal->PlaneWidget,
      vtkCommand::InteractionEvent, this, SLOT(onUpdatePlaneInteraction()));
    this->Internal->VTKPlaneConnect->Connect(this->Internal->PlaneWidget,
...
void pqCMBMeshViewerMainWindow::hidePlaneWidget()
{
  if (this->Internal->PlaneWidget)
  {
    pqSMAdaptor::setElementProperty(this->Internal->PlaneWidget->GetProperty("Visibility"), false);
    pqSMAdaptor::setElementProperty(this->Internal->PlaneWidget->GetProperty("Enabled"), false);
    ...
    (this->getMainDialog()->actionSelect_Cone_Cells->isChecked() ||
        (this->Internal->CurrentContour &&
          (this->getMainDialog()->actionSelect_Contour_Through_Cells->isChecked() ||
            this->getMainDialog()->actionSelect_Contour_Surface_Cell->isChecked()))))
  {
    this->getThisCore()->setShapeSelectionOption(this->Internal->CurrentContour,
      this->getMainDialog()->actionSelect_Contour_Through_Cells->isChecked(), selOption);
    this->updateSelection();
  }
}
...
void pqCMBMeshViewerMainWindow::onStartConeSelection(bool showDialog)
{
  ...
  bool coneStop = false;
  if (showDialog || this->getMainDialog()->actionSelect_Cone_Cells->isChecked())
  {
