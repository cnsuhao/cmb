...
void pqSimBuilderUIManager::setSMTKView(smtk::common::ViewPtr view, QWidget* parentWidget)
{
  ...
  if (!view)
  {
    QMessageBox::warning(parentWidget, tr("No Views Warning"),
      tr("There is no top view specified in the template file!"), QMessageBox::Ok);
    return;
  }
  ...
  if (qTopAttView)
  {
    QObject::connect(qTopAttView, SIGNAL(attColorChanged()), this, SIGNAL(attColorChanged()));
    QObject::connect(
  ...
  if (qTopExpressView)
  {
    QObject::connect(qTopExpressView,
      SIGNAL(onCreateFunctionWithExpression(QString&, double, double, int)), this,
  ...
  {
    smtk::extension::qtAttributeView* attSec = qobject_cast<smtk::extension::qtAttributeView*>(sec);
    if (attSec)
    {
      QObject::connect(attSec, SIGNAL(attColorChanged()), this, SIGNAL(attColorChanged()));
      QObject::connect(
  ...
  QString& funcExpr, double initVal, double deltaVal, int numValues)
{
  smtk::extension::qtSimpleExpressionView* const expressionView =
    qobject_cast<smtk::extension::qtSimpleExpressionView*>(QObject::sender());
  if (!expressionView)
  {
    return;
  }
  ...
  if (result)
  {
    ...
    expressionView->buildSimpleExpression(funcExpr, valuesText, numberOfComponents);
  }
}
