    ...
    if(colorMode == vtkModelMultiBlockSource::GetVolumeTagName())
   ...
   else if( colorMode == vtkModelMultiBlockSource::GetGroupTagName())
     {
      ...
      for(uit = modinfo->Info->GetUUID2BlockIdMap().begin();
        ...
        if(eref.hasColor()
          && pqCMBContextMenuHelper::getValidEntityColor(color, eref))
          {
...
void pqModelBuilderViewContextMenuBehavior::buildMenuFromSelections()
{
  ...
  int selNumBlocks = pqCMBContextMenuHelper::getSelectedRepBlocks(
    ...
    this->m_modelPanel->modelManager()->selectedMeshes(),
    this->m_selModelBlocks, this->m_selMeshBlocks);
...
*/
    ...
    if(!hasAnalysisMesh)
      ...
      if(!multipleModels && modit.key()->grp_annotations.size() > 2) // skip the "no group" entry
        ...
        for(git = modit.key()->grp_annotations.begin();
            git != modit.key()->grp_annotations.end(); ++git)
          {
          if(*git != "no group")
            {
            str_uuid = *git; // UUID
...
*/
    ...
    pqDataRepresentation* rep = hasAnalysisMesh ? meshit.key()->Representation :
                                modit.key()->Representation;
    QList<QVariant> rTypes = pqSMAdaptor::getEnumerationPropertyDomain(
      rep->getProxy()->GetProperty("Representation"));
    ...
    foreach (QVariant rtype, rTypes)
      ...
      if(strRepType == "Outline" ||
         strRepType == "3D Glyphs")
        continue;
...
void pqModelBuilderViewContextMenuBehavior::hideBlock()
{
  ...
  QMap<smtk::common::UUID,
    QPair<smtk::common::UUIDs, smtk::mesh::MeshSets> > blocksVis;
...
void pqModelBuilderViewContextMenuBehavior::showAllBlocks()
{
  ...
  this->showAllEntitiesAndMeshes(
    this->m_selModelBlocks.keys(),
    this->m_selMeshBlocks.keys());
}
  ...
  const QList<pqSMTKModelInfo*>& ents, const QList<pqSMTKMeshInfo*>& meshes)
{
  ...
  foreach(pqSMTKModelInfo* minfo, ents)
    ...
    pqCMBContextMenuHelper::getAllEntityIds(
      minfo, modelMan, blocksVis[minfo->SessionId].first);
    }
...
void pqModelBuilderViewContextMenuBehavior::showAllRepresentations()
{
  ...
  this->showAllEntitiesAndMeshes(
    this->m_modelPanel->modelManager()->allModels(),
...
//-----------------------------------------------------------------------------
void pqModelBuilderViewContextMenuBehavior::setSelectedBlocksColor(
  const QColor& color)
{
  // map of <sessionId, < Entities, Meshes> >
  QMap<smtk::common::UUID,
    QPair<smtk::common::UUIDs, smtk::mesh::MeshSets> > blocksColor;
...
void pqModelBuilderViewContextMenuBehavior::createGroup()
{
  ...
  foreach(pqSMTKModelInfo* minfo, this->m_selModelBlocks.keys())
    {
    pqCMBContextMenuHelper::startGroupOp(modMgr, minfo, "Create",
...
void pqModelBuilderViewContextMenuBehavior::addToGroup(QAction* action)
{
  ...
  pqCMBContextMenuHelper::startGroupOp(modMgr, minfo, "Modify",
