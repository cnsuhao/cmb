  ...
  , Internal(new vtkInternal(parent_widget))
{
  ...
  this->Tree = NULL;
  this->ColorNode = NULL;
  this->BoxTranslation[0] = this->BoxTranslation[1] = this->BoxTranslation[2] = 0.0;
  this->BoxOrientation[0] = this->BoxOrientation[1] = this->BoxOrientation[2] = 0.0;
  this->BoxScale[0] = this->BoxScale[1] = this->BoxScale[2] = 1.0;
  ...
  this->Internal->TINStitcherDlg = new qtCMBTINStitcherDialog(NULL);
...
void pqCMBSceneBuilderMainWindowCore::setupBoxWidget()
{
  ...
  vtkSMPropertyHelper(this->Internal->BoxWidget, "PlaceFactor").Set(1.05);

  this->Internal->BoxWidget->UpdateVTKObjects();

  pqSMAdaptor::addProxyProperty(
    this->activeRenderView()->getProxy()->GetProperty("Representations"),
    this->Internal->BoxWidget);

  this->activeRenderView()->getProxy()->UpdateVTKObjects();
  this->Internal->VTKBoxConnect->Disconnect();
  this->Internal->VTKBoxConnect->Connect(
    this->Internal->BoxWidget, vtkCommand::InteractionEvent, this, SLOT(updateBoxInteraction()));
  ...
  const QString& filename, pqPipelineSource* source)
{
  ...
  {
    if (filename.compare(this->Internal->CurrentSceneFileName, Qt::CaseInsensitive) == 0)
    {
      return;
    }
    else
  ...
  const QString& /*filename*/, pqPipelineSource* source)
{
  ...
  for (int i = 0; i < numArcs; i++)
  {
    ...
    vtkSMSourceProxy* sourceProxy = vtkSMSourceProxy::SafeDownCast(extract->getProxy());
    sourceProxy->UpdatePipeline();

    pqCMBArc* obj = new pqCMBArc(sourceProxy);
    this->Tree->createNode(
      arcIdArray[i].toString().toStdString().c_str(), this->Tree->getArcTypeNode(), obj, event);
  }
  ...
  const QString& /*filename*/, pqPipelineSource* source)
{
  ...
  vtkSMOutputPort* outputPort =
    vtkSMSourceProxy::SafeDownCast(source->getProxy())->GetOutputPort(static_cast<unsigned int>(0));
  ...
  {
    pqPipelineSource* extract = builder->createFilter("filters", "ExtractLeafBlock", source);
    ...
    if (strcmp(info->GetObjectType(), "RegionOfInterest"))
    {
      ...
      {
        obj = new pqCMBPoints(
          pdSource, this->activeRenderView(), this->getActiveServer(), info->GetObjectFileName());
      }
      else
      {
        obj = new pqCMBFacetedObject(
          pdSource, this->activeRenderView(), this->getActiveServer(), info->GetObjectFileName());
      }
      ...
      if (repr)
      {
        ...
        pqSMAdaptor::setMultipleElementProperty(repr->getProxy()->GetProperty("Position"), values);
        values.clear();
        values << scale[0] << scale[1] << scale[2];
        pqSMAdaptor::setMultipleElementProperty(repr->getProxy()->GetProperty("Scale"), values);
      }
    }
    else
    {
      ...
      obj = new pqCMBVOI(translation, bounds, this->getActiveServer(), this->activeRenderView());
      repr = obj->getRepresentation();

      QList<QVariant> values;
      values << orientation[0] << orientation[1] << orientation[2];
      pqSMAdaptor::setMultipleElementProperty(repr->getProxy()->GetProperty("Orientation"), values);
      values.clear();
      values << scale[0] << scale[1] << scale[2];
      pqSMAdaptor::setMultipleElementProperty(repr->getProxy()->GetProperty("Scale"), values);
    }
  ...
  const QList<pqCMBSceneNode*>* unselected, const QList<pqCMBSceneNode*>* newlySelected)
{
  ...
  {
    this->getAppearanceEditor()->setDisabled((this->SelectedLeaves.size() != 1) ? true : false);
  }
...
void pqCMBSceneBuilderMainWindowCore::updateWidgetPanel(pqCMBSceneObjectBase* widgetObject)
{
  ...
  foreach (qtInteractionWidget* iw, pWidget->findChildren<qtInteractionWidget*>())
  {
    if (iw != selUiWidget)
    {
  ...
  if (qtInteractionWidget* iw = qobject_cast<qtInteractionWidget*>(selUiWidget))
  {
...
void pqCMBSceneBuilderMainWindowCore::prepSelectedObject(pqCMBSceneObjectBase* obj)
{
  ...
  if (1 || obj->getType() == pqCMBSceneObjectBase::UniformGrid)
  {
    ...
    {
      pqSMAdaptor::setElementProperty(selSource->GetProperty("FieldType"), vtkSelectionNode::POINT);
    }
  ...
  else
  {
    pqSMAdaptor::setElementProperty(selRep->GetProperty("SelectionUseOutline"), 0);
    pqSMAdaptor::setElementProperty(
      selRep->GetProperty("SelectionRepresentation"), this->SelectionMode - 1);
  }
  ...
  if (!this->ColorNode)
  {
    ...
    this->Internal->VTKColorConnect->Connect(selRep->GetProperty("DiffuseColor"),
      vtkCommand::ModifiedEvent, this, SLOT(updateNodeColor()));
    this->Internal->VTKOpacityConnect->Connect(
      selRep->GetProperty("Opacity"), vtkCommand::ModifiedEvent, this, SLOT(updateNodeColor()));
  }
}
...
void pqCMBSceneBuilderMainWindowCore::updateBoxWidget()
{
  ...
  for (iter = this->SelectedLeaves.begin(); iter != this->SelectedLeaves.end(); ++iter)
  {
    ...
    if (obj && obj->getSource() && (!obj->isFullyConstrained()) && !obj->isDefaultConstrained())
    {
      ++numMoveableObj;
      moveable = obj;
      if (obj->getType() == pqCMBSceneObjectBase::Glyph)
  ...
  {
    pqSMAdaptor::setElementProperty(this->Internal->BoxWidget->GetProperty("Visibility"), false);
    pqSMAdaptor::setElementProperty(this->Internal->BoxWidget->GetProperty("Enabled"), false);
    this->Internal->BoxWidget->UpdateVTKObjects();
    return;
  }
  ...
  if ((!foundGlyph) && (numMoveableObj == 1))
  {
    ...
    this->Internal->BoxWidget->GetProperty("Position")->Copy(srcProxy->GetProperty("Position"));
    this->Internal->PositionLink = vtkSMPropertyLink::New();
    this->Internal->PositionLink->AddLinkedProperty(
      srcProxy, "Position", vtkSMLink::INPUT | vtkSMLink::OUTPUT);
    this->Internal->PositionLink->AddLinkedProperty(
      this->Internal->BoxWidget, "Position", vtkSMLink::INPUT | vtkSMLink::OUTPUT);

    this->Internal->BoxWidget->GetProperty("Rotation")->Copy(srcProxy->GetProperty("Orientation"));
    this->Internal->OrientationLink = vtkSMPropertyLink::New();
    this->Internal->OrientationLink->AddLinkedProperty(
      srcProxy, "Orientation", vtkSMLink::INPUT | vtkSMLink::OUTPUT);
    this->Internal->OrientationLink->AddLinkedProperty(
      this->Internal->BoxWidget, "Rotation", vtkSMLink::INPUT | vtkSMLink::OUTPUT);

    this->Internal->BoxWidget->GetProperty("Scale")->Copy(srcProxy->GetProperty("Scale"));
    this->Internal->ScaleLink = vtkSMPropertyLink::New();
    this->Internal->ScaleLink->AddLinkedProperty(
      srcProxy, "Scale", vtkSMLink::INPUT | vtkSMLink::OUTPUT);
    this->Internal->ScaleLink->AddLinkedProperty(
      this->Internal->BoxWidget, "Scale", vtkSMLink::INPUT | vtkSMLink::OUTPUT);
    ...
    if (bbValid)
    {
      ...
      pqSMAdaptor::setMultipleElementProperty(
        this->Internal->BoxWidget->GetProperty("PlaceWidget"), values);
  ...
  else // More then one object selected
  {
    ...
    if (bbValid)
    {
      ...
      this->BoxScale[0] = this->BoxScale[1] = this->BoxScale[2] = 1.0;
      QList<QVariant> values;
      values << (bounds[0] - this->BoxTranslation[0]) << (bounds[1] - this->BoxTranslation[0])
             << (bounds[2] - this->BoxTranslation[1]) << (bounds[3] - this->BoxTranslation[1])
             << (bounds[4] - this->BoxTranslation[2]) << (bounds[5] - this->BoxTranslation[2]);
      pqSMAdaptor::setMultipleElementProperty(
        this->Internal->BoxWidget->GetProperty("PlaceWidget"), values);
      vtkSMPropertyHelper(this->Internal->BoxWidget, "Position").Set(this->BoxTranslation, 3);
      double ori[3], scale[3];
      ori[0] = ori[1] = ori[2] = 0.0;
      scale[0] = scale[1] = scale[2] = 1.0;
      vtkSMPropertyHelper(this->Internal->BoxWidget, "Rotation").Set(ori, 3);
      vtkSMPropertyHelper(this->Internal->BoxWidget, "Scale").Set(scale, 3);
    }
  }

  pqSMAdaptor::setElementProperty(this->Internal->BoxWidget->GetProperty("Visibility"), bbValid);
  pqSMAdaptor::setElementProperty(this->Internal->BoxWidget->GetProperty("Enabled"), bbValid);
  this->Internal->BoxWidget->UpdateVTKObjects();
}
...
void pqCMBSceneBuilderMainWindowCore::setSelectionMode(int mode)
{
  ...
  for (iter = this->SelectedLeaves.begin(); iter != this->SelectedLeaves.end(); ++iter)
  {
    vtkSMProxy* selRep = (*iter)->getDataObject()->getRepresentation()->getProxy();
    if (selRep)
    {
...
void pqCMBSceneBuilderMainWindowCore::onSaveAsData()
{
  ...
  pqFileDialog file_dialog(
    this->getActiveServer(), this->parentWidget(), tr("Save Model:"), QString(), filters);
...
void pqCMBSceneBuilderMainWindowCore::onPackageScene()
{
  ...
  pqFileDialog file_dialog(
    this->getActiveServer(), this->parentWidget(), tr("Package Scene:"), QString());
...
void pqCMBSceneBuilderMainWindowCore::onCreateOmicronInput()
{
  ...
  // get input filename...
  pqFileDialog file_dialog(this->getActiveServer(), this->parentWidget(), "Save Omicron Input As:",
    QString(), "Omicron \"model\" Input files (*.dat)");
...
void pqCMBSceneBuilderMainWindowCore::onExport2DBCSFile()
{
  // dialog to get filename and necessary parameters
  // get input filename...
  pqFileDialog file_dialog(this->getActiveServer(), this->parentWidget(), "Save 2D BCS File As:",
    QString(), "2D BCS files (*.bcs)");
  ...
  pqCMBArc* contourObj =
    static_cast<pqCMBArc*>(contourTypeNode->getChildren().at(0)->getDataObject());
  if (!contourObj)
  {
    saveError = "No valid contour found !";
  }
  ...
  {
    vtkSmartPointer<vtkSGXMLBCSWriter> bcsWriter = vtkSmartPointer<vtkSGXMLBCSWriter>::New();
...
double pqCMBSceneBuilderMainWindowCore::computeVolumeContraint(double volumeFactor)
{
  ...
  // the scale
  double voiBounds[6];
  this->Internal->VOINodeForMesher->getDataObject()
    ->getRepresentation()
  ...
  double volume = (voiBounds[1] - voiBounds[0]) * scale[0] * (voiBounds[3] - voiBounds[2]) *
    scale[1] * (voiBounds[5] - voiBounds[4]) * scale[2];
...
void pqCMBSceneBuilderMainWindowCore::applyArcToSurface()
{
  ...
  pqPipelineSource* pdSource =
    builder->createSource("sources", "HydroModelPolySource", this->getActiveServer());
  vtkSMDataSourceProxy::SafeDownCast(pdSource->getProxy())
    ->CopyData(vtkSMSourceProxy::SafeDownCast(this->Internal->ArcDepress->getProxy()));
  pdSource->updatePipeline();
  //destroy the filters and readers
  ...
  if (this->Tree->getSelected().size() > 0)
  {
    parent = this->Tree->getSelected()[0]->getParent() != NULL
      ? this->Tree->getSelected()[0]->getParent()
      : this->Tree->getSelected()[0];
  }
  ...
  switch (this->Internal->ArcModManager->getSourceType())
  {
    ...
    {
      pqCMBPoints* obj =
        new pqCMBPoints(pdSource, this->activeRenderView(), this->getActiveServer(), "");
    ...
    {
      pqCMBFacetedObject* obj =
        new pqCMBFacetedObject(pdSource, this->activeRenderView(), this->getActiveServer(), "");
...
void pqCMBSceneBuilderMainWindowCore::arcDeformData(pqCMBSceneObjectBase::enumObjectType dt)
{
  ...
  QObject::connect(this->Internal->ArcModManager, SIGNAL(applyFunctions()), this,
    SLOT(applyArcToSurface()), Qt::UniqueConnection);
  ...
  this->Internal->ArcDepress =
    builder->createFilter("filters", "ArcDepressFilter", namedInputs, this->getActiveServer());
  {
...
void pqCMBSceneBuilderMainWindowCore::createSurfaceMesh()
{
  ...
  if (!validBounds)
  {
    QMessageBox::critical(
      this->parentWidget(), "Generating Surface Mesh", "Invalid Bounds on the VOI");
    return;
  }
  ...
  if (this->Internal->SurfaceMesherOptionDlg->getMeshVisibleArcSets())
  {
    ...
    while ((node = nodeIter.next()))
    {
      if (node->isVisible())
      {
        ...
        {
          namedInputs["Source"].push_back(node->getDataObject()->getSource()->getOutputPort(0));
        }
      }
    }
  }
  ...
  // Send each of the nodes down as a input to the filter that will
  // mesh the contour collection with the voi, and than make the TINs
  pqPipelineSource* extrudedContours = builder->createFilter(
    "filters", "CmbMeshTerrainWithArcs", namedInputs, this->getActiveServer());
  ...
  pqPipelineSource* pdSource =
    builder->createSource("sources", "HydroModelPolySource", this->getActiveServer());
  vtkSMDataSourceProxy::SafeDownCast(pdSource->getProxy())
    ->CopyData(vtkSMSourceProxy::SafeDownCast(extrudedContours->getProxy()));
  pdSource->updatePipeline();
  //destroy the filters and readers
  ...
  while ((node = nodeIter.next()))
  {
    ...
    if (selectedNames.contains(node->getName()))
    {
      ...
      pqPipelineSource* transformFilter =
        builder->createFilter("filters", "TransformFilter", dataObj->getSource());
  ...
  QString* filename, double /*vConstraintNotUsed*/)
{
  ...
  if (this->Tree->IsTemporaryPtsFileForMesherNeeded(surfaceNames))
  {
    ...
    if (tmpPtsFileIter != this->Internal->TemporaryPointsFileMap.end() && finfo.exists() &&
      tmpPtsFileIter->second.size() == surfaceNames.size())
    {
    ...
    if (tempPtsFileNotYetWritten)
    {
      ...
      while ((lidarNode = nodeIter.next()))
      {
        if (surfaceNames.contains(lidarNode->getName()))
        {
          ...
          obj = dynamic_cast<pqCMBPoints*>(lidarNode->getDataObject());
          if (obj && obj->getReaderSource() && obj->getPieceOnRatio() != 1)
          {
            // get data object full resolution
            nobj = new pqCMBPoints(this->Tree->getCurrentServer(), this->Tree->getCurrentView(),
              obj->getReaderSource(), obj->getPieceId(), 1, obj->getDoubleDataPrecision());
      ...
      pqPipelineSource* ptsWriter =
        builder->createFilter("writers", "LIDARWriter", namedInputs, this->getActiveServer());
      vtkSMPropertyHelper(ptsWriter->getProxy(), "WriteAsSinglePiece").Set(true);
      vtkSMPropertyHelper(ptsWriter->getProxy(), "FileName").Set(tmpPtsFileName.c_str());
    ...
    pieceComment =
      "This points file is formed using LIDAR pieces: " + surfaceNames[0].toStdString();
  ...
  {
    // come up with exec directory... omicron expects to be in pt data directory
    pqCMBPoints* obj = dynamic_cast<pqCMBPoints*>(lidarNode->getDataObject());
    this->setProcessExecDirectory(
      QString(vtksys::SystemTools::GetFilenamePath(obj->getFileName()).c_str()));

    writer->SetSurfaceFileName(vtksys::SystemTools::GetFilenameName(obj->getFileName()).c_str());
  ...
  while ((solidNode = nodeIter.next()) != 0)
  {
    ...
    if (solidNode->isVisible())
    {
      ...
      if (solidInVOI)
      {
        ...
        {
          if (dynamic_cast<pqCMBFacetedObject*>(solidNode->getDataObject())->getSurfaceType() !=
            pqCMBSceneObjectBase::Solid)
          {
            continue;
          }
        }
...
void pqCMBSceneBuilderMainWindowCore::onSpawnModelMesher()
{
  ...
  QString mesherOutputFileName =
    this->getProcessExecDirectory() + "/" + this->Internal->MesherOutputPrefix + ".vtk";
  QFileInfo info(mesherOutputFileName);
  if (info.exists() == true)
  {
    if (QMessageBox::question(this->parentWidget(), "Remove duplicate file?",
  ...
  QString execString;
  if (this->Internal->MesherOptionDlg->getCurrentMesherPath() == "Internal Mesher")
  {
  ...
  QObject::connect(this, SIGNAL(remusCompletedNormally(remus::proto::JobResult)), this,
    SLOT(previewMesherOutput()));
...
{
  QString newMesher;
  if (this->getExistingFileName(
        "All files (*);;Windows Executable (*.exe)", "Select surface mesher", newMesher) == true)
  {
...
int pqCMBSceneBuilderMainWindowCore::setupSurfaceMesherDialog()
{
  ...
  while ((surfaceNode = nodeIter.next()) != 0)
  {
    ...
    this->Internal->SurfaceMesherOptionDlg->insertSurfaceName(
      surfaceIndex++, surfaceNode->getName());
  }
  ...
  if ((this->Tree->getSelected().size() == 1) && this->Tree->getSelected()[0]->getDataObject() &&
    this->Tree->getSelected()[0]->getDataObject()->getType() == pqCMBSceneObjectBase::VOI)
  {
...
void pqCMBSceneBuilderMainWindowCore::previewMesherOutput()
{
  ...
  QString criticalMessage("The surface mesher was unable to generate a mesh based on your inputs.  "
                          "Please examine both the model and meshing settings for errors");
  QString warningMessage("The scene that you created had intersecting faces around one or multiple "
                         "objects that intersect the surface.\n\nSelect surface with edges in the "
                         "dropdown list and zoom in on the objects to look for crossed edges on "
                         "the object surface at the surface intersection line.\n\nTry turning or "
                         "slightly moving the object/objects that have crossed edges and remesh.");
  ...
  pqPipelineSource* reader =
    builder->createReader("sources", "LegacyVTKFileReader", fileList, this->getActiveServer());
...
void pqCMBSceneBuilderMainWindowCore::exportSelectedSolids()
{
  ...
  {
    QMessageBox::warning(this->parentWidget(), "No Solids or Contours Selected",
      "Must selected at least 1 solid or contour to export!");
    return;
  }
...
void pqCMBSceneBuilderMainWindowCore::exportSelectedPolygons()
{
  ...
  {
    QMessageBox::warning(
      this->parentWidget(), "No Polygons Selected", "Must selected at least 1 Polygon to export!");
    return;
  }

  // get output filename
  QString filters = "CMB Files(*.cmb)";
  pqFileDialog file_dialog(this->getActiveServer(), this->parentWidget(),
    tr("Export Polygon CMB Model:"), QString(), filters);
  file_dialog.setObjectName("CMBExportDialog");
  file_dialog.setFileMode(pqFileDialog::AnyFile);
  QString cmbFileName;
  if (file_dialog.exec() == QDialog::Accepted)
...
void pqCMBSceneBuilderMainWindowCore::exportCMBFile()
{
  ...
  vtkSmartPointer<vtkCompleteShells> fixHoles = vtkSmartPointer<vtkCompleteShells>::New();
  fixHoles->SetInputConnection(normals->GetOutputPort());

  vtkSmartPointer<vtkCMBModelBuilder> builder = vtkSmartPointer<vtkCMBModelBuilder>::New();
  ...
  vtkModelItemIterator* regions = wrapper->GetModel()->NewIterator(vtkModelRegionType);
  for (regions->Begin(); !regions->IsAtEnd(); regions->Next())
  {
    ...
    if (sscanf(temp.c_str(), "Region%d", &regionIndex) > 0)
    {
      ...
      // user name
      vtkModelUserName::SetUserName(
        region, this->Internal->NodeInfo[regionIndex - 1].UserName.c_str());

      // color
      region->SetColor(this->Internal->NodeInfo[regionIndex - 1].Color[0],
  ...
  if (solidMeshes.size() > 0)
  {
    ...
    foreach (vtkIdType regId, solidMeshes.keys())
    {
      ...
      if (this->clientTransformSource(
            solidMeshes[regId]->getSource(), solSource, transform.GetPointer()))
      {
        solidMeshOperator->AddSolidMesh(regId, vtkPolyData::SafeDownCast(solSource->GetSource()));
      }
    }
    ...
    {
      QMessageBox::critical(NULL, "SceneBuilder", "Failed to incorporate solid mesh to cmb model!");
      return;
    }
  }
...
void pqCMBSceneBuilderMainWindowCore::rejectMesh()
{
  ...
  QFile::remove(
    this->getProcessExecDirectory() + "/" + this->Internal->MesherOutputPrefix + ".vtk");
  QFile::remove(
    this->getProcessExecDirectory() + "/" + this->Internal->MesherOutputPrefix + "_Surf3D.vtk");
  QFile::remove(
    this->getProcessExecDirectory() + "/" + this->Internal->MesherOutputPrefix + "_Box_Shell.vtk");
  QFile::remove(
    this->getProcessExecDirectory() + "/" + this->Internal->MesherOutputPrefix + "_BCfaces.vtk");
  QFile::remove(
    this->getProcessExecDirectory() + "/" + this->Internal->MesherOutputPrefix + "_nodes.bc");
  QFile::remove(
    this->getProcessExecDirectory() + "/" + this->Internal->MesherOutputPrefix + "_faces.bc");
...
void pqCMBSceneBuilderMainWindowCore::addOmicronModelFaceData(vtkPolyData* outputPD)
{
  vtkIntArray* regionIDs = vtkIntArray::SafeDownCast(
    outputPD->GetCellData()->GetArray(vtkMultiBlockWrapper::GetShellTagName()));
  if (regionIDs)
  {
    // depending on whether BCS specified, create model faces either on shells or the BCS file
    QString bcFacesFile =
      this->getProcessExecDirectory() + "/" + this->Internal->MesherOutputPrefix + "_BCfaces.vtk";
    vtkSmartPointer<vtkPolyDataReader> faceReader = vtkSmartPointer<vtkPolyDataReader>::New();
    faceReader->SetFileName(bcFacesFile.toLatin1().constData());
    faceReader->Update();

    vtkIntArray* modelFaceIDs =
      vtkIntArray::SafeDownCast(faceReader->GetOutput()->GetFieldData()->GetArray("BCtags"));

    if (modelFaceIDs && regionIDs->GetNumberOfTuples() == modelFaceIDs->GetNumberOfTuples())
    {
      vtkSmartPointer<vtkIntArray> newModelFaceIDs = vtkSmartPointer<vtkIntArray>::New();
...
{
  vtkAlgorithm* inAlgorithm =
    vtkAlgorithm::SafeDownCast(serverSource->getProxy()->GetClientSideObject());
  if (!inAlgorithm)
  {
    return false;
  }
    ...
    matrix->Element[3][3] != 1)
  {
    ...
    clientSource->CopyData(vtkPolyData::SafeDownCast(transFilter->GetOutputDataObject(0)));
  }
  else
  {
    clientSource->CopyData(vtkPolyData::SafeDownCast(inAlgorithm->GetOutputDataObject(0)));
  }
  return true;
}
...
void pqCMBSceneBuilderMainWindowCore::onRubberBandSelect(bool checked)
{
  ...
  {
    int isShiftKeyDown =
      this->activeRenderView()->getRenderViewProxy()->GetInteractor()->GetShiftKey();
...
void pqCMBSceneBuilderMainWindowCore::setpqCMBSceneTree(pqCMBSceneTree* tree)
{
  ...
  Internal->ArcModManager =
    new pqCMBModifierArcManager(NULL, this->getActiveServer(), this->activeRenderView());

  //make sure the context menu has the new scene tree
  this->Internal->RenderWindowMenuBehavior->setTree(this->Tree);

  QObject::connect(this->Tree, SIGNAL(requestSceneUpdate()), this, SLOT(requestRender()));
  QObject::connect(this->Tree,
    SIGNAL(selectionUpdated(const QList<pqCMBSceneNode*>*, const QList<pqCMBSceneNode*>*)), this,
    SLOT(updateSelection(const QList<pqCMBSceneNode*>*, const QList<pqCMBSceneNode*>*)));
  QObject::connect(this->Tree, SIGNAL(requestTINStitch()), this, SLOT(stitchTINs()));
  QObject::connect(this->Tree, SIGNAL(requestSolidExport()), this, SLOT(exportSelectedSolids()));
  QObject::connect(
    this->Tree, SIGNAL(requestPolygonsExport()), this, SLOT(exportSelectedPolygons()));

  this->Internal->VTKBoxConnect->Connect(this->Internal->BoxWidget, vtkCommand::InteractionEvent,
    this->Tree, SLOT(collapseAllDataInfo()));

  this->Internal->MesherOptionDlg = new qtCMBSceneMesherDialog(this->Tree);
  this->Internal->SurfaceMesherOptionDlg = new qtCMBSceneSurfaceMesherDialog(this->Tree);
  this->Internal->ArcModOptionDlg = new qtCMBArcModifierInputDialog(this->Tree);
  QObject::connect(this->Internal->ArcModOptionDlg, SIGNAL(sourceTypeChanged()), this,
    SLOT(modifierInputSelectionType()));
...
void pqCMBSceneBuilderMainWindowCore::updateBoxInteraction()
{
  ...
  if (this->Internal->selGlyphs.count() == 0)
  {
    ...
    if (this->SelectedLeaves.size() < 2 &&
      (this->SelectedLeaves[0]->getDataObject()->getType() != pqCMBSceneObjectBase::Glyph))
  ...
  vtkSMPropertyHelper(this->Internal->BoxWidget, "Position").Get(newPos, 3);
  vtkSMPropertyHelper(this->Internal->BoxWidget, "Rotation").Get(newOri, 3);
  vtkSMPropertyHelper(this->Internal->BoxWidget, "Scale").Get(newScale, 3);
  int i;
  for (i = 0; i < 3; i++)
  {
  ...
  for (iter = this->SelectedLeaves.begin(); iter != this->SelectedLeaves.end(); ++iter)
  {
    ...
    if (dataObj->getType() == pqCMBSceneObjectBase::Line ||
      dataObj->getType() == pqCMBSceneObjectBase::Arc || dataObj->isFullyConstrained())
    {
      continue;
    }
  ...
  while ((n = iter.next()))
  {
    ...
    else
    {
      QMessageBox::warning(this->parentWidget(), "Line not added to model!",
        "The line is not completely enclosed by a single region and thus is not being added to the "
        "model.");
    }
  }
}
  ...
  while ((n = iter.next()))
  {
    ...
    if (arcInfo && arcInfo->GetNumberOfPoints() && arcInfo->IsClosedLoop())
    {
      ...
      { // assume that there is more than 1 point so that we can make a loop
        ...
        {
          std::vector<vtkIdType> sortedNodes(selectedNodes->GetPointer(0),
            selectedNodes->GetPointer(selectedNodes->GetNumberOfTuples()));
          ;
          // sort the ids since we are not sure
          std::sort(sortedNodes.begin(), sortedNodes.end());
          vtkModelVertex* firstVertex = model->BuildModelVertex(sortedNodes[0]);
          vtkModelVertex* prevVertex = firstVertex;
          for (size_t i = 1; i < sortedNodes.size(); i++)
          {
            vtkModelVertex* nextVertex = model->BuildModelVertex(sortedNodes[i]);
            vtkDiscreteModelEdge* edge =
              vtkDiscreteModelEdge::SafeDownCast(model->BuildModelEdge(prevVertex, nextVertex));
          ...
          vtkDiscreteModelEdge* edge =
            vtkDiscreteModelEdge::SafeDownCast(model->BuildModelEdge(prevVertex, firstVertex));
          vtkIdList* cellIds = vtkIdList::New();
          for (vtkIdType j = 0; j < sortedNodes[0]; j++)
          {
            cellIds->InsertNextId(j);
          }
          for (vtkIdType j = sortedNodes[sortedNodes.size() - 1]; j < Geometry->GetNumberOfCells();
               j++)
          {
            cellIds->InsertNextId(j);
          }
        ...
        { // a loop model edge with no vertices
          vtkDiscreteModelEdge* edge =
            vtkDiscreteModelEdge::SafeDownCast(model->BuildModelEdge(0, 0));
          vtkIdList* cellIds = vtkIdList::New();
        ...
        model->BuildModelFace(
          static_cast<int>(edges.size()), &edges[0], &edgeDirections[0], material);
      }
    }
  }
...
void pqCMBSceneBuilderMainWindowCore::stitchTINs()
{
  ...
  {
    QMessageBox::warning(
      NULL, "TIN Stitch", "Preview dialog already open; must close before stitching!");
    return;
  }
  ...
  this->TINStitcher =
    builder->createFilter("filters", "MultiLayerTINStitcher", namedInputs, this->getActiveServer());
  ...
  pqSMAdaptor::setElementProperty(this->TINStitcher->getProxy()->GetProperty("MinimumAngle"),
    this->Internal->TINStitcherDlg->getMinimumAngle());
  pqSMAdaptor::setElementProperty(this->TINStitcher->getProxy()->GetProperty("UseQuads"),
  ...
  pqSMAdaptor::setElementProperty(this->TINStitcher->getProxy()->GetProperty("Tolerance"),
    this->Internal->TINStitcherDlg->getTolerance());
  pqSMAdaptor::setElementProperty(this->TINStitcher->getProxy()->GetProperty("TIN Type"),
  ...
  vtkPVDataInformation* dataInfo = this->TINStitcher->getOutputPort(0)->getDataInformation();
...
void pqCMBSceneBuilderMainWindowCore::onPreviewAccepted()
{
  ...
  else // previewing stitched TIN
  {
    ...
    pqFileDialog file_dialog(
      this->getActiveServer(), this->parentWidget(), tr("Save Stitched TIN:"), QString(), filters);
    ...
    if (outputFileName.size() != 0)
    {
      ...
      vtkSMOutputPort* outputPort = vtkSMSourceProxy::SafeDownCast(this->TINStitcher->getProxy())
      ...
      for (int i = 0; i < numBlocks; i++)
      {
        pqPipelineSource* extract =
          builder->createFilter("filters", "ExtractLeafBlock", this->TINStitcher);
        ...
        pqPipelineSource* writer = builder->createFilter("writers", "DataSetWriter", pdSource);
        if (numBlocks > 1)
        {
          char fileNameWithIndex[256];
          sprintf(fileNameWithIndex, "%s/%s_%d.vtk", origFInfo.path().toStdString().c_str(),
            origFInfo.baseName().toStdString().c_str(), i);
          outputFileName = fileNameWithIndex;
        }
        pqSMAdaptor::setElementProperty(
          writer->getProxy()->GetProperty("FileName"), outputFileName.toStdString().c_str());
        writer->getProxy()->UpdateVTKObjects();
        vtkSMSourceProxy::SafeDownCast(writer->getProxy())->UpdatePipeline();
        builder->destroy(writer);

        pqCMBFacetedObject* fobj = new pqCMBFacetedObject(pdSource, this->activeRenderView(),
          this->getActiveServer(), outputFileName.toStdString().c_str());
        fobj->setSurfaceType(pqCMBSceneObjectBase::Solid);
        fobj->setUserDefinedType("-Solid");
        QFileInfo fInfo(outputFileName);
        pqCMBSceneNode* node = this->Tree->createNode(
          fInfo.baseName().toStdString().c_str(), this->Tree->getRoot(), fobj, event);
...
void pqCMBSceneBuilderMainWindowCore::selectGlyph(QList<pqCMBGlyphObject*>& glyphList)
{
  ...
  for (QList<pqCMBGlyphObject*>::iterator it = glyphList.begin(); it != glyphList.end(); ++it)
  {
...
void pqCMBSceneBuilderMainWindowCore::updateBoxWidgetForGlyph()
{
  ...
  // Check to see if there are any moveable leaf nodes
  for (iter = this->Internal->selGlyphs.begin(); iter != this->Internal->selGlyphs.end(); ++iter)
  {
    pqCMBGlyphObject* obj = iter.key();
    if (obj && obj->getSource() && (!obj->isFullyConstrained()) && !obj->isDefaultConstrained())
    {
      vtkSMCMBGlyphPointSourceProxy* glyphProxy =
        vtkSMCMBGlyphPointSourceProxy::SafeDownCast(obj->getSource()->getProxy());
      if (!glyphProxy)
      {
        continue;
      }
  ...
  pqSMAdaptor::setElementProperty(this->Internal->BoxWidget->GetProperty("Visibility"), true);
  ...
  double sdelta[3], double odelta[3], double tdelta[3])
{
  ...
  for (iter = this->Internal->selGlyphs.begin(); iter != this->Internal->selGlyphs.end(); ++iter)
  {
    ...
    if (obj && obj->getSource() && (!obj->isFullyConstrained()) && !obj->isDefaultConstrained())
    {
      ...
      foreach (vtkIdType selId, iter.value())
      {
        glyphProxy->ApplyTransform(selId, odelta, tdelta, sdelta);
      }
