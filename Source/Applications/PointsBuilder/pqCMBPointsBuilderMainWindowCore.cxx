  ...
  pqCMBCommonMainWindowCore(parent_widget), Internal(new vtkInternal(parent_widget))
{
  ...
  QObject::connect(
    this->Internal->FilterDlg, SIGNAL(OkPressed()),
...
//-----------------------------------------------------------------------------
void pqCMBPointsBuilderMainWindowCore::ImportLIDARFiles(const QStringList& files,
  pqPipelineSource* reader)
{
  ...
  if(this->Internal->CurrentTotalNumPoints<=0)
    ...
    QMessageBox::warning(this->parentWidget(),
      tr("File Open Warning"),"No points info found", QMessageBox::Ok);
    return;
    }
...
void pqCMBPointsBuilderMainWindowCore::abort()
{
  ...
  if (this->TerrainExtractionManager->getFullTerrainFilter())
    ...
    {
    source = vtkSMSourceProxy::SafeDownCast(
          this->Internal->CurrentWriter->getProxy());
    }
  ...
  if (res!=0 && this->ReaderManager->userRequestsDoubleData())
    ...
    if (this->ReaderManager->setOutputDataTypeToDouble())
      ...
      QList<pqCMBLIDARPieceObject*> filePieces =
        this->ReaderManager->getFilePieceObjects(filename,
          allPieces);
      this->ReaderManager->updatePieces(filename,filePieces, true,
        this->Internal->PieceMainTable, false, NULL);
      }
    }
...
void pqCMBPointsBuilderMainWindowCore::initThresholdTable()
{
  ...
  table->setHorizontalHeaderLabels(
    QStringList() << tr("Apply") << tr("Thresholds") );
  ...
  QObject::connect(table, SIGNAL(itemSelectionChanged()),
    this, SLOT(onThresholdSelectionChanged()));
  QObject::connect(table, SIGNAL(itemChanged(QTableWidgetItem*)),
    this, SLOT(onThresholdItemChanged(QTableWidgetItem*))/*, Qt::QueuedConnection*/);
}
...
void pqCMBPointsBuilderMainWindowCore::initPolygonsTree()
{
  ...
  this->Internal->ContourTree = new pqCMBLIDARContourTree(
    this->Internal->LIDARPanel->getGUIPanel()->frameContour);
  QObject::connect(this->Internal->ContourTree,
    SIGNAL(itemChanged(QList<QTreeWidgetItem*>, int , int)),
    this, SLOT(onPolygonItemChanged(QList<QTreeWidgetItem*>, int, int)));
  QObject::connect(this->Internal->ContourTree, SIGNAL(selectionChanged(QTreeWidgetItem*)),
    this, SLOT(onPolygonTableSelectionChanged(QTreeWidgetItem*))/*, Qt::QueuedConnection*/);
  QObject::connect(this->Internal->ContourTree, SIGNAL(onItemsDropped(QTreeWidgetItem*,
    int ,QList<QTreeWidgetItem*> )),
...
void pqCMBPointsBuilderMainWindowCore::setupElevationFilter(double minZ, double maxZ)
{
  ...
  this->Internal->LIDARPanel->getGUIPanel()->elevationMinSpinBox->setMinimum(
    VTK_FLOAT_MIN);
  this->Internal->LIDARPanel->getGUIPanel()->elevationMinSpinBox->setMaximum(
    maxZ);
  this->Internal->LIDARPanel->getGUIPanel()->elevationMaxSpinBox->setMinimum(
    minZ);
  this->Internal->LIDARPanel->getGUIPanel()->elevationMaxSpinBox->setMaximum(
    VTK_FLOAT_MAX);
...
void pqCMBPointsBuilderMainWindowCore::onAcceptToLoad()
{
  ...
  if (visiblePieces.count() > 0)
    ...
    if (!pqCMBLIDARSaveDialog::getFile(this->parentWidget(),this->getActiveServer(),
                                  (this->Internal->PieceMainTable->getWidget()->rowCount() >= 2),
                                  &filename, &saveAsSingle, &saveAsDisplayed))
      {
...
//-----------------------------------------------------------------------------
bool pqCMBPointsBuilderMainWindowCore::savePieces(
  QList<pqCMBLIDARPieceObject*> pieces, const QString& filename,
  bool saveAsSinglePiece, bool loadAsDisplayed, bool multiOutput)
{
  ...
  if(writerName.compare("DEMWriter") == 0)
    ...
    {
    writeResult =
      this->WritePieces(savePieces, writerName, filename, saveAsSinglePiece);
    }
...
//-----------------------------------------------------------------------------
bool pqCMBPointsBuilderMainWindowCore
::ExportDem( QList<pqPipelineSource*> pieces,
             const QString& writerName, const QString& fileName )
{
  ...
  QPointer<pqPipelineSource> filter = builder->createFilter("filters", "DemDataExtractor", pdSource);
  ...
  //set things up
  {
    ...
    pqSMAdaptor::setMultipleElementProperty(this->Internal->CurrentWriter->getProxy()->GetProperty("RasterSize"), values);
    pqSMAdaptor::setElementProperty(this->Internal->CurrentWriter->getProxy()->GetProperty("RadiusX"), rSpacing[0]);
    pqSMAdaptor::setElementProperty(this->Internal->CurrentWriter->getProxy()->GetProperty("RadiusY"), rSpacing[1]);
    pqSMAdaptor::setElementProperty(this->Internal->CurrentWriter->getProxy()->GetProperty("Zone"), zone);
    pqSMAdaptor::setElementProperty(this->Internal->CurrentWriter->getProxy()->GetProperty("IsNorth"), isNorth);
    pqSMAdaptor::setElementProperty(this->Internal->CurrentWriter->getProxy()->GetProperty("Scale"), scale);
  }
  bool result = this->WriteFile(fileName);
  if(distroy) builder->destroy(pdSource);
  return result;
}
  ...
  if(inputs.count() > 0)
    ...
    if (writerName.compare("LIDARWriter") == 0 )
      ...
      if (writeAsSinglePiece)
        {
        vtkSMPropertyHelper(this->Internal->CurrentWriter->getProxy(),
      ...
      this->Internal->CurrentWriter = builder->createFilter("writers",
        writerName.toStdString().c_str(), pdSource);
      bool result = this->WriteFile(fileName);
...
bool pqCMBPointsBuilderMainWindowCore::WriteFile(const QString& fileName)
{
  ...
  int aborted = pqSMAdaptor::getElementProperty(
    this->Internal->CurrentWriter->getProxy()->GetProperty("AbortExecute")).toInt();
  if(aborted)
    {
    return false;
    }
...
//-----------------------------------------------------------------------------
void pqCMBPointsBuilderMainWindowCore::onVTKConnectionChanged(
...
void pqCMBPointsBuilderMainWindowCore::setupControlPanel(QWidget* parent)
{
  ...
  QObject::connect(this->Internal->LIDARPanel->getGUIPanel()->updateButton, SIGNAL(clicked()),
  ...
  QObject::connect(
    this->Internal->LIDARPanel->getGUIPanel()->pushButtonAddFiles, SIGNAL(clicked()),
  ...
  QObject::connect(this->Internal->LIDARPanel->getGUIPanel()->minXClip, SIGNAL(valueChanged(double)),
    this, SLOT(onClippingBoxChanged()));
  QObject::connect(this->Internal->LIDARPanel->getGUIPanel()->maxXClip, SIGNAL(valueChanged(double)),
    this, SLOT(onClippingBoxChanged()));
  QObject::connect(this->Internal->LIDARPanel->getGUIPanel()->minYClip, SIGNAL(valueChanged(double)),
    this, SLOT(onClippingBoxChanged()));
  QObject::connect(this->Internal->LIDARPanel->getGUIPanel()->maxYClip, SIGNAL(valueChanged(double)),
    this, SLOT(onClippingBoxChanged()));

  QObject::connect(this->Internal->LIDARPanel->getGUIPanel()->applyTargetNumberOfPoints, SIGNAL(clicked()),
  ...
  QObject::connect(this->Internal->LIDARPanel->getGUIPanel()->advancedCheckBox, SIGNAL(stateChanged(int)),
    this, SLOT(onAdvancedCheckBox(int)));

  QObject::connect(this->Internal->PieceMainTable, SIGNAL(
  ...
  QObject::connect(this->Internal->PieceMainTable, SIGNAL(requestRender()),
    this, SLOT(onRequestRender()));
  QObject::connect(this, SIGNAL(requestingRender()),
    this, SLOT(onRequestRender()));
  ...
  QObject::connect(
    this->Internal->LIDARPanel->getGUIPanel()->addFilterButton, SIGNAL(clicked()),
    this, SLOT(onAddThreshold()));
  QObject::connect(
    this->Internal->LIDARPanel->getGUIPanel()->removeFilterButton, SIGNAL(clicked()),
  ...
  QObject::connect(
    this->Internal->LIDARPanel->getGUIPanel()->newGroupButton, SIGNAL(clicked()),
    this, SLOT(onAddContourGroup()));
  QObject::connect(
    this->Internal->LIDARPanel->getGUIPanel()->addContourButton, SIGNAL(clicked()),
    this, SLOT(onAddContourWidget()));
  QObject::connect(
    this->Internal->LIDARPanel->getGUIPanel()->removeContourButton, SIGNAL(clicked()),
    this, SLOT(onRemoveContour()));
  QObject::connect(
    this->Internal->LIDARPanel->getGUIPanel()->buttonUpdateContours, SIGNAL(clicked()),
    this, SLOT(onUpdateContours()));
  QObject::connect(
    this->Internal->LIDARPanel->getGUIPanel()->elevationMaxSpinBox,
    SIGNAL(valueChanged(double)),
    this, SLOT(onElevationMaxChanged(double)));

  QObject::connect(
    this->Internal->LIDARPanel->getGUIPanel()->elevationGroup, SIGNAL(toggled(bool)),
    this, SLOT(onElevationFilter(bool)));
  QObject::connect(
    this->Internal->LIDARPanel->getGUIPanel()->elevationUpdateButton, SIGNAL(clicked()),
    this, SLOT(onUpdateElevationFilter()));
  QObject::connect(
    this->Internal->LIDARPanel->getGUIPanel()->elevationMinSpinBox,
    SIGNAL(valueChanged(double)),
    this, SLOT(onElevationMinChanged(double)));
  QObject::connect(
    this->Internal->LIDARPanel->getGUIPanel()->elevationMaxSpinBox,
    SIGNAL(valueChanged(double)),
    this, SLOT(onElevationMaxChanged(double)));
  ...
  this->setupAppearanceEditor(
    this->Internal->LIDARPanel->getGUIPanel()->tabDisplay);

  this->ReaderManager = new pqCMBLIDARReaderManager(this,
  ...
  this->Internal->pqCMBModifierArcTable = new pqCMBModifierArcManager(this->Internal->LIDARPanel->getGUIPanel()->EditPoints,
                                                            this->getActiveServer(), this->activeRenderView());
  QObject::connect(this->Internal->pqCMBModifierArcTable, SIGNAL(requestRender()),
                   this, SLOT(onRequestRender()));
  QObject::connect(this->Internal->pqCMBModifierArcTable, SIGNAL(addingNewArc()),
                   this, SLOT(onModifierArcWidgetStart()));
  QObject::connect(this->Internal->pqCMBModifierArcTable, SIGNAL(modifyingArcDone()),
...
{
  double tmpOnRatio = this->Internal->LIDARPanel->getGUIPanel()->
    targetNumberOfPoints->text().toDouble() /  static_cast<double>(totalNumberOfPoints);
...
//-----------------------------------------------------------------------------
int pqCMBPointsBuilderMainWindowCore::calculateOnRatioForPiece(int onRatio,
...
bool pqCMBPointsBuilderMainWindowCore::isObjectUpToDate(pqCMBLIDARPieceObject* dataObj)
{
  ...
  return dataObj->isObjectUpToDate(this->Internal->LIDARPanel->getGUIPanel()->clipGroupBox->isChecked(),
    this->Internal->ClipBBox);
}
...
void pqCMBPointsBuilderMainWindowCore::setupProgressBar(QStatusBar* statusBar)
{
  ...
  QObject::connect(progress_manager, SIGNAL(progress(const QString&, int)),
                   this, SLOT(updateProgress(const QString&, int)), Qt::QueuedConnection);

  QObject::connect(progress_manager, SIGNAL(enableAbort(bool)),
                   this->Internal->ProgressBar,      SLOT(enableAbort(bool)));

  QObject::connect(this->Internal->ProgressBar, SIGNAL(abortPressed()),
...
void pqCMBPointsBuilderMainWindowCore::onClippingBoxChanged()
{
  if (this->Internal->OutlineSource)
    ...
    values << this->Internal->LIDARPanel->getGUIPanel()->minXClip->value() <<
      ...
      this->Internal->LIDARPanel->getGUIPanel()->maxYClip->value() <<
      this->Internal->DataBounds[4] - zPad << this->Internal->DataBounds[5] + zPad;
    pqSMAdaptor::setMultipleElementProperty(this->Internal->OutlineSource->
...
void pqCMBPointsBuilderMainWindowCore::clearSelection(bool updateFocusFlag/*=true*/)
{
  ...
  pqCMBLIDARPieceObject *currentObject =
    this->Internal->PieceMainTable->getCurrentObject(false);
  if (currentObject)
    {
...
//-----------------------------------------------------------------------------
void pqCMBPointsBuilderMainWindowCore::onCurrentObjectChanged(
...
void pqCMBPointsBuilderMainWindowCore::updatePointTotals()
{
  ...
  if (!allObjectsUpToDate)
    {
    this->Internal->LIDARPanel->getGUIPanel()->displayPointsLabel->setForegroundRole(QPalette::Highlight);
    }
  else
    {
    this->Internal->LIDARPanel->getGUIPanel()->displayPointsLabel->setForegroundRole(QPalette::WindowText);
    }

  // will be pretty good estimate for Save... unless clip not up-to-date
  if (clipEnabled && !allObjectsClipUpToDate)
    {
    this->Internal->LIDARPanel->getGUIPanel()->savePointsLabel->setForegroundRole(QPalette::Highlight);
    }
  else
    {
    this->Internal->LIDARPanel->getGUIPanel()->savePointsLabel->setForegroundRole(QPalette::WindowText);
    }

  this->Internal->LIDARPanel->getGUIPanel()->displayPointsLabel->setText( QString::number(totalDisplayPoints) );
  this->Internal->LIDARPanel->getGUIPanel()->savePointsLabel->setText( QString::number(totalSavePoints) );
}
...
//-----------------------------------------------------------------------------
void pqCMBPointsBuilderMainWindowCore::onPiecesSelectionChanged(
...
void pqCMBPointsBuilderMainWindowCore::onFilterChanged()
{
  ...
  {
    vtkSMSourceProxy* thresholdSource = vtkSMSourceProxy::SafeDownCast(
      visiblePieces[i]->getThresholdSource()->getProxy());
...
bool pqCMBPointsBuilderMainWindowCore::updateThresholdTransform(pqCMBLIDARPieceObject *dataObj)
{
  ...
  if (dataObj->isPieceTransformed())
    ...
    vtkSMPropertyHelper(dataObj->getThresholdSource()->getProxy(),
    ...
    vtkSMPropertyHelper(dataObj->getContourSource()->getProxy(),
      "Transform").Set(transform->GetMatrix()->Element[0], 16);
    dataObj->getContourSource()->getProxy()->UpdateVTKObjects();

    // set the transform for the elevation filter
    vtkSMPropertyHelper(dataObj->getElevationSource()->getProxy(),
      "Transform").Set(transform->GetMatrix()->Element[0], 16);
...
void pqCMBPointsBuilderMainWindowCore::onTabChanged(int tabIndex)
{
  ...
  if (tabIndex == 0 && this->Internal->LastSelectedObject)
    {
    this->Internal->PieceMainTable->updateWithPieceInfo(
      this->Internal->LastSelectedObject);
...
void pqCMBPointsBuilderMainWindowCore::onRemoveFilter()
{
  ...
  {
    QMessageBox::warning(this->parentWidget(),
      tr("Cannot Remove Filter"),
      tr("You must select a filter first."),
      QMessageBox::Ok);
    return;
  }
...
void pqCMBPointsBuilderMainWindowCore::onActiveFilterChanged()
{
  ...
  for(int i = 0; i < visiblePieces.size(); ++i)
  {
    thresholdSourceProxy = vtkSMSourceProxy::SafeDownCast(
      visiblePieces[i]->getThresholdSource()->getProxy());
    pqSMAdaptor::setElementProperty(
      thresholdSourceProxy->GetProperty("SetActiveFilterIndex"),
      row);
...
void pqCMBPointsBuilderMainWindowCore::onContourChanged()
{
  ...
  QTreeWidgetItem* contourItem =
    this->Internal->ContourTree->onContourChanged(contourWidget);
  if(contourItem)
    {
...
void pqCMBPointsBuilderMainWindowCore::onContourFinished()
{
  ...
  if(contourItem)
    ...
    if(this->Internal->ContourTree->isContourApplied(contourItem))
      ...
      this->onPolygonItemChanged(changedItems,
...
void pqCMBPointsBuilderMainWindowCore::onAddContourWidget()
{
  QList<QTreeWidgetItem*> selItems =
    this->Internal->ContourTree->getSelectedItems();
  if(selItems.size()==0)
    {
  ...
  QObject::connect(
    contourWidget, SIGNAL(contourLoopClosed()),
    this, SLOT(onContourFinished())/*, Qt::QueuedConnection*/);
  QObject::connect(
    contourWidget, SIGNAL(widgetEndInteraction()),
    this, SLOT(onContourChanged())/*, Qt::QueuedConnection*/);
...
void pqCMBPointsBuilderMainWindowCore::updateContourSource(
  vtkSMSourceProxy* contourSource, vtkSMSourceProxy* currentContours,
  bool forceUpdate)
{
  currentContours->UpdatePropertyInformation();
  if (!forceUpdate && pqSMAdaptor::getElementProperty(
    currentContours->GetProperty("NumberOfActivePolygons")).toInt() == 0)
    {
    return;
    }
  ...
  vtkSMProxyProperty* clipFunc = vtkSMProxyProperty::SafeDownCast(
    contourSource->GetProperty("ClipPolygon") );
  int addGroupIndex = 0;
    ...
    if (group->checkState(pqCMBLIDARContourTree::UseFilterCol) == Qt::Checked)
      ...
      for (int j = 0; j < group->childCount(); j++)
        ...
        if (group->child(j)->checkState(pqCMBLIDARContourTree::UseFilterCol) == Qt::Checked)
          ...
          clipFunc->AddProxy( this->Internal->ContourProxyMap[
            this->Internal->ContourTree->getItemObject(group->child(j)) ] );
          ...
          if (addFilterIndex == 0)
            ...
            pqSMAdaptor::setElementProperty(
              contourSource->GetProperty("GroupInvert"), invertGroup);
...
void pqCMBPointsBuilderMainWindowCore::updateThresholdSource(
  vtkSMSourceProxy* thresholdSource, vtkSMSourceProxy* currentThresholds,
  bool forceUpdate)
{
  currentThresholds->UpdatePropertyInformation();
  if (!forceUpdate && pqSMAdaptor::getElementProperty(
    currentThresholds->GetProperty("NumberOfActiveThresholdSets")).toInt() == 0)
    {
    return;
    }

  int numFilters = pqSMAdaptor::getElementProperty(
    currentThresholds->GetProperty("NumberOfThresholdSets")).toInt();
  int addIdx = 0;
  for(int idx=0; idx<numFilters; idx++)
    {
    // only need to do filter if it is "active" ("UseFilter" is true)
    pqSMAdaptor::setElementProperty(
      currentThresholds->GetProperty("SetActiveFilterIndex"),idx);
    currentThresholds->UpdateProperty("SetActiveFilterIndex", true);
    currentThresholds->UpdatePropertyInformation();

    if (!pqSMAdaptor::getElementProperty(
      currentThresholds->GetProperty("GetUseFilter")).toInt())
      {
      continue;
      }
    ...
    pqSMAdaptor::setElementProperty(
      thresholdSource->GetProperty("SetActiveFilterIndex"), addIdx);
    ...
    pqSMAdaptor::setMultipleElementProperty(
      thresholdSource->GetProperty("SetUseFilter"), 1, 1);

    pqSMAdaptor::setElementProperty(
      thresholdSource->GetProperty("SetUseMinX"),
      pqSMAdaptor::getElementProperty(
      currentThresholds->GetProperty("GetUseMinX")).toInt());
    pqSMAdaptor::setElementProperty(
      thresholdSource->GetProperty("SetUseMinY"),
      pqSMAdaptor::getElementProperty(
      currentThresholds->GetProperty("GetUseMinY")).toInt());
    pqSMAdaptor::setElementProperty(
      thresholdSource->GetProperty("SetUseMinZ"),
      pqSMAdaptor::getElementProperty(
      currentThresholds->GetProperty("GetUseMinZ")).toInt());

    pqSMAdaptor::setElementProperty(
      thresholdSource->GetProperty("SetUseMaxX"),
      pqSMAdaptor::getElementProperty(
      currentThresholds->GetProperty("GetUseMaxX")).toInt());
    pqSMAdaptor::setElementProperty(
      thresholdSource->GetProperty("SetUseMaxY"),
      pqSMAdaptor::getElementProperty(
      currentThresholds->GetProperty("GetUseMaxY")).toInt());
    pqSMAdaptor::setElementProperty(
      thresholdSource->GetProperty("SetUseMaxZ"),
      pqSMAdaptor::getElementProperty(
      currentThresholds->GetProperty("GetUseMaxZ")).toInt());

    pqSMAdaptor::setElementProperty(
      thresholdSource->GetProperty("SetUseMinRGB"),
      pqSMAdaptor::getElementProperty(
      currentThresholds->GetProperty("GetUseMinRGB")).toInt());
    pqSMAdaptor::setElementProperty(
      thresholdSource->GetProperty("SetUseMaxRGB"),
      pqSMAdaptor::getElementProperty(
      currentThresholds->GetProperty("GetUseMaxRGB")).toInt());

    pqSMAdaptor::setElementProperty(
      thresholdSource->GetProperty("SetInvert"),
      pqSMAdaptor::getElementProperty(
      currentThresholds->GetProperty("GetInvert")).toInt());

    QList<QVariant> minColorList;
    minColorList.push_back(pqSMAdaptor::getElementProperty(
      currentThresholds->GetProperty("GetMinR")).toInt());
    minColorList.push_back(pqSMAdaptor::getElementProperty(
      currentThresholds->GetProperty("GetMinG")).toInt());
    minColorList.push_back(pqSMAdaptor::getElementProperty(
      currentThresholds->GetProperty("GetMinB")).toInt());
    pqSMAdaptor::setMultipleElementProperty(
      thresholdSource->GetProperty("SetMinRGB"),
      minColorList);

    QList<QVariant> maxColorList;
    maxColorList.push_back(pqSMAdaptor::getElementProperty(
      currentThresholds->GetProperty("GetMaxR")).toInt());
    maxColorList.push_back(pqSMAdaptor::getElementProperty(
      currentThresholds->GetProperty("GetMaxG")).toInt());
    maxColorList.push_back(pqSMAdaptor::getElementProperty(
      currentThresholds->GetProperty("GetMaxB")).toInt());
    pqSMAdaptor::setMultipleElementProperty(
      thresholdSource->GetProperty("SetMaxRGB"),
      maxColorList);

    pqSMAdaptor::setElementProperty(
      thresholdSource->GetProperty("SetMinX"),
      pqSMAdaptor::getElementProperty(
      currentThresholds->GetProperty("GetMinX")).toDouble());
    pqSMAdaptor::setElementProperty(
      thresholdSource->GetProperty("SetMinY"),
      pqSMAdaptor::getElementProperty(
      currentThresholds->GetProperty("GetMinY")).toDouble());
    pqSMAdaptor::setElementProperty(
      thresholdSource->GetProperty("SetMinZ"),
      pqSMAdaptor::getElementProperty(
      currentThresholds->GetProperty("GetMinZ")).toDouble());

    pqSMAdaptor::setElementProperty(
      thresholdSource->GetProperty("SetMaxX"),
      pqSMAdaptor::getElementProperty(
      currentThresholds->GetProperty("GetMaxX")).toDouble());
    pqSMAdaptor::setElementProperty(
      thresholdSource->GetProperty("SetMaxY"),
      pqSMAdaptor::getElementProperty(
      currentThresholds->GetProperty("GetMaxY")).toDouble());
    pqSMAdaptor::setElementProperty(
      thresholdSource->GetProperty("SetMaxZ"),
      pqSMAdaptor::getElementProperty(
      currentThresholds->GetProperty("GetMaxZ")).toDouble());
...
{
  if(col != pqCMBLIDARContourTree::UseFilterCol && col !=
    pqCMBLIDARContourTree::InvertCol)//  && col != ROICol)
    {
    return;
    }
  pqWaitCursor cursor;
...
//-----------------------------------------------------------------------------
void pqCMBPointsBuilderMainWindowCore::onPolygonTableSelectionChanged(
...
{
  QGroupBox* transfromBox = this->getAppearanceEditorContainer()->
  findChild<QGroupBox*>("TransformationGroup");
  if(transfromBox)
    {
...
void pqCMBPointsBuilderMainWindowCore::onElevationMinChanged(double minZ)
{
  this->Internal->LIDARPanel->getGUIPanel()->elevationMaxSpinBox->setMinimum(
    minZ);
  if (this->Internal->ElevationMinZ != minZ || this->Internal->ElevationMaxZ !=
    this->Internal->LIDARPanel->getGUIPanel()->elevationMaxSpinBox->value())
    {
...
void pqCMBPointsBuilderMainWindowCore::onElevationMaxChanged(double maxZ)
{
  this->Internal->LIDARPanel->getGUIPanel()->elevationMinSpinBox->setMaximum(
    maxZ);
  if (this->Internal->ElevationMaxZ != maxZ || this->Internal->ElevationMinZ !=
    this->Internal->LIDARPanel->getGUIPanel()->elevationMinSpinBox->value())
    {
    this->Internal->LIDARPanel->getGUIPanel()->elevationUpdateButton->setEnabled(true);
    }
...
void pqCMBPointsBuilderMainWindowCore::onSaveContour()
{
  ...
  pqFileDialog file_dialog(
    ...
    NULL, tr("Save Contours:"), QString(), filters);
...
void pqCMBPointsBuilderMainWindowCore::onLoadContour()
{
  ...
  pqFileDialog file_dialog(this->getActiveServer(),
    NULL, tr("Load Contours:"), QString(), filters);
...
void pqCMBPointsBuilderMainWindowCore::saveContour(const char* filename)
{
  ...
  vtkSMSourceProxy* contourFilter = vtkSMSourceProxy::SafeDownCast(
    contourSource->getProxy());
  unsigned int numGroups = this->Internal->ContourTree->getNumberOfGroups();
  vtkSMProxyProperty* smpContour = vtkSMProxyProperty::SafeDownCast(
    contourFilter->GetProperty("Contour") );
  for(unsigned int i = 0; i < numGroups; i++)
    {
    smpContour->RemoveAllProxies();
    pqSMAdaptor::setElementProperty(
      contourFilter->GetProperty("ActiveGroupIndex"), i);
    ...
    for (int j = 0; j < group->childCount(); j++)
      ...
      if(this->getContourProjectionPosition(projPos, widget))
        ...
        stream  << vtkClientServerStream::Invoke
          << VTKOBJECT(contourFilter) << "SetContourProjectionPosition"
          << j << projPos
    ...
    int invertGroup =
      (group->checkState(pqCMBLIDARContourTree::InvertCol) == Qt::Unchecked) ? 1 : 0;
    pqSMAdaptor::setElementProperty(
      contourFilter->GetProperty("GroupInvert"), invertGroup);
    contourFilter->UpdateProperty("GroupInvert", true);
    }

  contourFilter->UpdatePipeline();
  pqPipelineSource* writer = builder->createFilter(
    "writers","XMLContourWriter",contourSource);
  pqSMAdaptor::setElementProperty(writer->getProxy()->GetProperty("FileName"),
    filename);
...
void pqCMBPointsBuilderMainWindowCore::loadContour(const char* filename)
{
  ...
  pqPipelineSource* reader = builder->createReader(
    "sources", "XMLMultiBlockDataReader",
    files, this->getActiveServer());
  builder->blockSignals(false);
  if (!reader)
    {
    QMessageBox::warning(this->parentWidget(),
      tr("Loading Contour Warning"),
  ...
  for(int i = 0; i < numBlocks; i++)
    ...
    vtkSMSourceProxy* extractSource = vtkSMSourceProxy::SafeDownCast(
      extractBlock->getProxy());
...
*/
    pqPipelineSource* extractContour = builder->createFilter("filters",
      "CmbExtractContours", extractBlock);
    vtkSMSourceProxy *smContours = vtkSMSourceProxy::SafeDownCast(
      extractContour->getProxy() );
    smContours->UpdateVTKObjects();
    smContours->UpdatePipeline();
    smContours->UpdatePropertyInformation();
    int max =pqSMAdaptor::getElementProperty(
      smContours->GetProperty("NumberOfContoursInfo")).toInt();
/*
...
*/
    double* pRange = NULL;
    int groupInvert = 0;
    vtkPVArrayInformation* groupInfo = contourInfo->GetArrayInformation(
      "GroupInvert");
    ...
    for ( int w = 0; w < max; ++w)
      ...
      if(contourW)
        ...
        item->setBackgroundColor(pqCMBLIDARContourTree::NameCol,
          this->Internal->ContourTree->contourFinishedColor);
        this->addContourFilter(contourW);
        QObject::connect(
          contourW, SIGNAL(widgetEndInteraction()),
