...
namespace
{
  ...
  //----------------------------------------------------------------------------
  bool get_value(const remus::proto::JobSubmission& data,
  ...
  //----------------------------------------------------------------------------
  remus::proto::JobStatus make_jobFailedStatus( const remus::worker::Job &j,
...
//-----------------------------------------------------------------------------
OmicronWorker::OmicronWorker(remus::proto::JobRequirements const& reqs,
...
void OmicronWorker::meshJob()
{
  ...
  if(!settings.valid)
    {
    this->updateStatus(
          make_jobFailedStatus(job,"Unable to read data send from client") );
    return;
    }
...
void OmicronWorker::launchOmicron(omicronSettings& settings)
{
  ...
  boost::filesystem::path executePath =
      boost::filesystem::absolute(settings.executablePath);
