...
class InternalPolygonDetection
  ...
  public:
    ...
    bool FindInnerLoop(const std::deque<vtkIdType> &outerLoop, std::vector< std::deque<vtkIdType> > &innerLoops);

  protected:
    //these are used when finding the outside loop
    bool FindLoop(vtkIdType &startingCellId, std::deque<vtkIdType> &loop, const bool& outerLoopSearch);

    int FindNextCellToTraverse( vtkIdType &currentCellId, vtkIdType &nextCellId, bool &addToBranchStack);
...
//----------------------------------------------------------------------------
bool InternalPolygonDetection::FindOuterLoop(vtkIdType &startingCellId,
...
//----------------------------------------------------------------------------
bool InternalPolygonDetection::FindLoop(vtkIdType &startingCellId, std::deque<vtkIdType> &loop,
              const bool& outerLoopSearch)
{
  ...
  do
    ...
    if (foundNextCellStatus == 1)
      ...
      if ( hasBranches &&
        ( branchCellIds.size() == 0 || branchCellIds.top() != currentCellId ) )
        {
...
bool InternalPolygonDetection::FindInnerLoop(
      const std::deque<vtkIdType> &outerLoop,
      std::vector< std::deque<vtkIdType> > &innerLoops)
{
  ...
  int numPolygonPoints  = (static_cast<int>(outerLoop.size())-1)*3;  //the outerloop will have the same cellId at the starte and end
  ...
  if (polygon[0] == polygon[index-3] &&
      polygon[1] == polygon[index-2] &&
      polygon[2] == polygon[index-1])
    {
    //first id matches second last id
    --numPolygonPoints;
    }
  else if (polygon[0] == polygon[index-6] &&
      polygon[1] == polygon[index-5] &&
      polygon[2] == polygon[index-4])
    {
    //first id matches second last id
    numPolygonPoints -= 2;
    }
  else if (polygon[3] == polygon[index-3] &&
      polygon[4] == polygon[index-2] &&
  ...
  else if (polygon[3] == polygon[index-6] &&
      polygon[4] == polygon[index-5] &&
  ...
  for (arcs->InitTraversal(); arcs->GetNextCell(npts,pts); ++cellId)
    ...
    int result = vtkPolygon::PointInPolygon(this->Arcs->GetPoint(pts[0]),
      numPolygonPoints,polygon,bounds,normal);
     if ( result == -1  )
      {
      continue;
      }
     ...
     if ( isLoop || isLine )
       ...
       for(std::deque<vtkIdType>::const_iterator itIL = innerLoop.begin();
         itIL != innerLoop.end(); itIL++)
         {
         usedIds.insert(*itIL);
         }
...
//----------------------------------------------------------------------------
int vtkCMBSceneGenPolygon::FillInputPortInformation(int /*port*/,
                                                         vtkInformation *info)
{
                                 ...
                                 vtkInformationVector* outputVector)
{
  ...
  bool readyForMeshing = this->PrepForMeshing( inputVector[0],
                                               preMesh.GetPointer());
...
//----------------------------------------------------------------------------
bool vtkCMBSceneGenPolygon::PrepForMeshing(vtkInformationVector* input,
                                           vtkPolyData *mesh)
{
...
//----------------------------------------------------------------------------
void vtkCMBSceneGenPolygon::AppendArcSets(vtkInformationVector* input,
                                          vtkPolyData *mesh)
{
  ...
  if ( outerLoopFound )
    ...
    if ( innerLoopFound )
      ...
      for( unsigned i=0; i < innerLoops.size(); ++i)
        ...
        mapInterface->AddArc(minId*3, ((maxId-minId)+1)*3,1+i,-1,miLoopId,1+i,1+i);
