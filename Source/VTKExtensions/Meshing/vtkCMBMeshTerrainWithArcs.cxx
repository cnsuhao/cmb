...
public:
  ...
  void setGroundElevation(const double& ge) { GroundElevation = ge; }
protected:
  double Radius;
  ...
  vtkInformation*, vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  if (validMesh && mesherResult)
  {
    ...
    this->TerrainInfo =
      new vtkCmbInternalTerrainInfo(inputVector[0], this->ElevationRadius, this->VOIBounds[4]);
...
bool vtkCMBMeshTerrainWithArcs::AssignPolygonIds(vtkPolyData* mesh, const vtkIdType& /*size*/) const
{
  ...
  vtkIdType groundId = this->PolygonInfo->Info[this->PolygonInfo->GroundIndex].Id;
  int asId = 0;
  int endpointId = 0;
  ...
  vtkPolyData* input, vtkPolyData* output, const int& index) const
{
  ...
  {
    if ((input->GetCellType(i) == VTK_LINE || input->GetCellType(i) == VTK_POLY_LINE) &&
...
void vtkCMBMeshTerrainWithArcs::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  os << indent << "VOIBounds: " << this->VOIBounds[0] << ", " << this->VOIBounds[1] << ", "
     << this->VOIBounds[2] << ", " << this->VOIBounds[3] << ", " << this->VOIBounds[4] << ", "
