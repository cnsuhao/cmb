  ...
  const char* xmlname = "CMBNetworkImageSource")
{
  ...
  pqPipelineSource* textureImageSource = builder->createReader(xmlgroup, xmlname, fileList, server);
...
inline void MODELBUILDER_SETUP_CATEGORICAL_CTF(vtkSMProxy* reproxy, vtkSMProxy* lutProxy,
  std::vector<vtkTuple<double, 3> >& rgbColors, const std::vector<std::string>& in_annotations)
  ...
  if (lutProxy && reproxy->GetProperty("LookupTable"))
  {
    ...
    if (in_annotations.size() > 0)
    {
      ...
      vtkSMStringVectorProperty* svp =
        vtkSMStringVectorProperty::SafeDownCast(lutProxy->GetProperty("Annotations"));
      if (svp)
      {
        //        svp->SetElements(new_annotations);
        svp->SetElements(
          (const char**)(&new_annotations[0]), static_cast<unsigned int>(new_annotations.size()));
      }
    }
    ...
    if (rgbColors.size() > 0)
    {
      ...
      int idx = (git != in_annotations.end())
        ? (git - in_annotations.begin())
...
inline void MODELBUILDER_SYNCUP_DISPLAY_LUT(const char* arrayname, vtkSMProxy* lutProxy)
{
  ...
  if (displayLUTProxy)
  {
    vtkSMPropertyHelper(displayLUTProxy, "IndexedColors")
      .Set(&rgbColors[0], static_cast<unsigned int>(rgbColors.size()));
