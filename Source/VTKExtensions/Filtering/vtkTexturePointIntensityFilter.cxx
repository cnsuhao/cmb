  ...
  vtkInformationVector *vtkNotUsed(outputVector))
{
  ...
  vtkDoubleArray *sMap =
    vtkDoubleArray::SafeDownCast( inputPD->GetFieldData()->GetArray("SMap") );
  vtkDoubleArray *tMap =
    vtkDoubleArray::SafeDownCast( inputPD->GetFieldData()->GetArray("TMap") );
...
//    }
  ...
  if (useRegisterPlanarTextureMap)
    ...
    vtkRegisterPlanarTextureMap::ComputeTextureCoordinate(tp,
      sRange->GetTuple2(0), tRange->GetTuple2(0),
      sMap->GetTuple2(0), tMap->GetTuple2(0),
      resultTCoord);
    }
  else
    {
  ...
  switch(imageData->GetScalarType())
    ...
    case VTK_UNSIGNED_CHAR:
      ...
      unsigned char *rgb = static_cast<unsigned char *>(
        imageData->GetScalarPointer( imageCoordinate ) );
      // rgb weighting from search of converting rgb -> intensity
      this->Intensity = 0.2989 * static_cast<double>(rgb[0]) / 255.0 +
        0.5870 * static_cast<double>(rgb[1]) / 255.0 +
        0.1140 * static_cast<double>(rgb[2]) / 255.0;
