...
public:
  ...
  size_t size() const { return this->Set.size(); }

  const_iterator begin() const { return this->Set.begin(); }

  const_iterator end() const { return this->Set.end(); }
  void insert(const_iterator it1, const_iterator it2) { this->Set.insert(it1, it2); }

  const_iterator find(vtkIdType id) const { return this->Set.find(id); }

  std::pair<const_iterator, bool> insert(const vtkIdType& id) { return this->Set.insert(id); }
};
...
public:
  ...
  void Clear() { this->Rows.clear(); }

  bool Empty() { return (this->Rows.size() == 0); }
  ...
  void Add(vtkMergeGroup& group)
  {
    ...
    else
    {
      ...
      {
        if (group.find(iter->Root()) != group.end() || iter->find(group.Root()) != iter->end())
        {
