  ...
  vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  for (i = 0; i < numcells; i++)
  {
    ...
    std::map<vtkIdType, vtkIdList*>::iterator it = modelFaces.find(value);
  ...
  for (it = modelFaces.begin(); it != modelFaces.end(); it++)
  {
    ...
    if (materialMap.find(intmaterialdata->GetValue(cellid)) == materialMap.end())
    {
  ...
  for (it = modelFaces.begin(); it != modelFaces.end(); it++)
  {
    ...
    mbw->CreateModelFace(shellMap[intshelldata->GetValue(cellid)],
      materialMap[intmaterialdata->GetValue(cellid)], rgba, it->second, 0, -1);
    it->second->Delete();
  }
