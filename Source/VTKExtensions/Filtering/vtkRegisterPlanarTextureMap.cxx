  ...
  double xy1[2], double st1[2], double xy2[2], double st2[2])
{
  ...
  {
    this->SMap[1] =
      ((dxy[1] * dst[0]) - (dxy[0] * dst[1])) / ((dxy[0] * dxy[0]) + (dxy[1] * dxy[1]));
  ...
  double xy1[2], double st1[2], double xy2[2], double st2[2], double xy3[2], double st3[2])
{
  ...
  if (dx0 == -1)
  {
    ...
    {
      this->SMap[0] = ((dst[1][0] * dxy[0][1]) - (dst[0][0] * dxy[1][1])) /
        ((dxy[1][0] * dxy[0][1]) - (dxy[0][0] * dxy[1][1]));
      this->SMap[1] = (dst[0][0] - (this->SMap[0] * dxy[0][0])) / dxy[0][1];
      this->TMap[0] = ((dst[1][1] * dxy[0][1]) - (dst[0][1] * dxy[1][1])) /
        ((dxy[1][0] * dxy[0][1]) - (dxy[0][0] * dxy[1][1]));
  ...
  vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  for (i = 0; i < numPts && !abort; i++)
  {
    ...
    tcoords[0] =
      ((this->SMap[0] * p[0]) + (this->SMap[1] * p[1]) + this->SMap[2] - this->SRange[0]) * sSf;
    tcoords[1] =
      ((this->TMap[0] * p[0]) + (this->TMap[1] * p[1]) + this->TMap[2] - this->TRange[0]) * tSf;
...
{
  tCoords[0] =
    ((sMap[0] * pt[0]) + (sMap[1] * pt[1]) + sMap[2] - sRange[0]) / (sRange[1] - sRange[0]);
  tCoords[1] =
...
void vtkRegisterPlanarTextureMap::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  os << indent << "SMap: (" << this->SMap[0] << ", " << this->SMap[1] << ", " << this->SMap[2]
     << " )\n";

  os << indent << "TMap: (" << this->TMap[0] << ", " << this->TMap[1] << ", " << this->TMap[2]
     << " )\n";

  os << indent << "Number Of RegisterPoints: " << this->NumberOfRegisterPoints << "\n";

  int i;
  for (i = 0; i < this->NumberOfRegisterPoints; i++)
  {
    os << indent << "Registration Point " << i << ": (" << this->RegisterXYPoints[i][0] << ", "
       << this->RegisterXYPoints[i][1] << " ) -> (" << this->RegisterSTPoints[i][0] << ", "
  ...
  os << indent << "X Range: (" << this->XRange[0] << ", " << this->XRange[1] << ")\n";

  os << indent << "Y Range: (" << this->YRange[0] << ", " << this->YRange[1] << ")\n";
  os << indent << "Clipping XY: " << (this->ClipXY ? "On\n" : "Off\n");
  os << indent
