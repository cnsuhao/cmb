  ...
  vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  locator->FindClosestPoint(outsideCorner, closestPoint, cell.GetPointer(), cellId, subId, dist2);
  ...
  if (!locator->IntersectWithLine(
        soilPt, exteriorPt, 0.0001, t, x, pCoords, subId, cellId, cell.GetPointer()))
  {
    vtkErrorMacro(
      "Yikes... certainly not expected!  Intersection test of interior soil point failed.");
  ...
  for (int i = 0; i < numShells; i++)
  {
    ...
    if (pointInside) // must not be soil
    {
      ...
      locator->FindClosestPoint(pointInside, closestPoint, cell.GetPointer(), cellId, subId, dist2);
      // Let find the cell's model face and its material id
      modelFace = wrapper->GetModelFaceId(cellId);
      outputMaterialID->InsertNextValue(wrapper->GetModelFaceMaterialId(modelFace));
    }
    else // soil
    {
      tempString = "\"soil\"";
