/*=========================================================================
  ...
  Module:    vtkPolyDataStatsFilter.cxx
...
=========================================================================*/
#include "vtkPolyDataStatsFilter.h"
  ...
  vtkInformationVector *vtkNotUsed(outputVector))
{
  ...
  for (i = 0; i < this->NumberOfPolygons; i++)
    ...
    if (n == 3)
      ...
      area = vtkTriangle::TriangleArea(np,
                                       tPnts->GetPoint(pntIds[0]),
                                       tPnts->GetPoint(pntIds[1]));
       for (j = 1; j < n; j++)
        {
        area += vtkTriangle::TriangleArea(np,
                                          tPnts->GetPoint(pntIds[j]),
                                          tPnts->GetPoint(pntIds[j-1]));
        }
      }
    ...
    if (!i)
      ...
      this->PolygonalSideStats[0] = this->PolygonalSideStats[1] =
...
void vtkPolyDataStatsFilter::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  os << indent << "Polygonal Side Stats (min, ave, max: ("
     << this->PolygonalSideStats[0] << ", "
     << this->PolygonalSideStats[1] << ", "  << this->PolygonalSideStats[2] << ")\n";
  os << indent << "Area Stats (min, ave, max: (" << this->AreaStats[0] << ", "
     << this->AreaStats[1] << ", "  << this->AreaStats[2] << ")\n";
  os << indent << "Geometric Bounds: (" << this->GeometryBounds[0] << ", "
     << this->GeometryBounds[1] << ", "  << this->GeometryBounds[2]
     << this->GeometryBounds[3] << ", "
