...
vtkCMBGlyphPointSource::vtkCMBGlyphPointSource()
{
  ...
  this->DefaultColor[0] = this->DefaultColor[1] = this->DefaultColor[2] = this->DefaultColor[3] =
    1.0;
...
vtkIdType vtkCMBGlyphPointSource::InsertNextPoint(double x, double y, double z)
{
  ...
  this->Color->InsertNextTuple4((255.0 * this->DefaultColor[0]) + 0.5,
    (255.0 * this->DefaultColor[1]) + 0.5, (255.0 * this->DefaultColor[2]) + 0.5,
  ...
  double b, double a, double sx, double sy, double sz, double ox, double oy, double oz, int vis)
{
  ...
  this->Color->InsertNextTuple4(
    (255.0 * r) + 0.5, (255.0 * g) + 0.5, (255.0 * b) + 0.5, (255.0 * a) + 0.5);
...
void vtkCMBGlyphPointSource::SetColor(vtkIdType index, double r, double g, double b, double a)
{
  this->Color->SetTuple4(
    index, (255.0 * r) + 0.5, (255.0 * g) + 0.5, (255.0 * b) + 0.5, (255.0 * a) + 0.5);
...
void vtkCMBGlyphPointSource::ReadFromFile(const char* fname)
{
  ...
  this->Color = vtkUnsignedCharArray::SafeDownCast(pdata->GetArray("Color", index));
  this->Scaling = vtkDoubleArray::SafeDownCast(pdata->GetArray("Scaling", index));
  this->Orientation = vtkDoubleArray::SafeDownCast(pdata->GetArray("Orientation", index));
  this->Visibility = vtkBitArray::SafeDownCast(pdata->GetArray("Visibility", index));
