  ...
  , InvalidValue(invalid)
{
  ...
  vtkUniformGrid* gridInput =
    vtkUniformGrid::SafeDownCast(inInfo->Get(vtkDataObject::DATA_OBJECT()));
  vtkImageData* imageInput = vtkImageData::SafeDownCast(inInfo->Get(vtkDataObject::DATA_OBJECT()));
  if (pd)
  {
...
template <class T>
T vtkCMBApplyBathymetryFilter::vtkCmbInternalTerrainInfo::getElevation(T* point)
{
  double dpoint[3];
...
int vtkCMBApplyBathymetryFilter::FillInputPortInformation(int port, vtkInformation* info)
{
  ...
  vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  vtkPointSet* pdInput = vtkPointSet::SafeDownCast(inInfo->Get(vtkDataObject::DATA_OBJECT()));
