/*=========================================================================
  ...
  Module:    vtkCMBBandedPolyDataContourFilter.cxx
...
int vtkCMBBandedPolyDataContourFilter::ClipEdge(int v1, int v2,
                                             vtkPoints *newPts,
...
inline int vtkCMBBandedPolyDataContourFilter::InsertCell(vtkCellArray *cells,
...
int vtkCMBBandedPolyDataContourFilter::RequestData(
  ...
  vtkInformationVector *outputVector)
{
  ...
  if ( numPolys > 0 || numStrips > 0 )
    ...
    //
    edgeTable->InitEdgeInsertion(numPts,1); //store attributes on edge // FIXME: No reuse of line segment vertices
    intList->Reset();
    ...
    for ( polys->InitTraversal(); polys->GetNextCell(npts,pts) && !abort;
      ...
      for ( intersectionPoint=0, numFullPts=0, i=0; i<npts; i++)
        ...
        if ( generateBoundary )
          ...
          onBdy = ( edgeNeighbors->GetNumberOfIds() == 0 ) && ( ! this->Clipping || this->NumberOfClipValues > 2 );
          }
        ...
        if ( (intLoc=edgeTable->IsEdge(v,vR)) != -1 )
          ...
          for ( ; intsIdx >= 0 && intsIdx < numIntPts; intsIdx += intsInc )
            ...
            if ( onBdy )
              ...
              if (
                ( s[numFullPts] > srange[0] && s[numFullPts] < srange[1] ) ||
        ...
        if ( onBdy )
          ...
          if ( sR >= srange[0] && sR <= srange[1] )
            ...
            if ( start >= 0 )
              ...
              {
              vtkWarningMacro(
                << "Bad boundary edge! cell " << (count-1) << " edge "
                << v << " s " << inScalars->GetTuple1( v ) << " -- ??? -- " << vR << " sR " << sR
      ...
      if ( this->OutputEdges != NO_EDGES )
        ...
        for (i=0; i < numFullPts; i++)
          ...
          if ( isContourValue[i] && isContourValue[(i+1)%numFullPts] &&
               s[i] == s[(i+1)%numFullPts] &&
               s[i] < this->ClipValues[this->NumberOfClipValues-1] &&
               s[i] > this->ClipValues[0] )
            {
            contourEdges->InsertNextCell(2);
      ...
      if (
        this->OutputEdges != NO_EDGES &&
        s[mR] < this->ClipValues[this->NumberOfClipValues-1] &&
...
unsigned long int vtkCMBBandedPolyDataContourFilter::GetMTime()
{
...
static const char* OutputEdgesEnumNames[] = {
  "NoEdges",
  "BandEdges",
  "BandAndBoundaryEdges",
  0
};
...
void vtkCMBBandedPolyDataContourFilter::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  os
    << indent << "ScalarName: "
    << ( this->ScalarName ? this->ScalarName : "NULL" ) << "\n";
}
