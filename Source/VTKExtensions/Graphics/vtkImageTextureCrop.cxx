...
int vtkImageTextureCrop::RequestInformation (
                                       ...
                                       vtkInformationVector *outputVector)
{
  ...
  vtkPolyData *outputPD = vtkPolyData::SafeDownCast(
    outPDInfo->Get(vtkDataObject::DATA_OBJECT()) );
  ...
  if ( inputExtent[0] == outputExtent[0] &&
       inputExtent[1] == outputExtent[1] &&
       inputExtent[2] == outputExtent[2] &&
       inputExtent[3] == outputExtent[3] &&
       inputExtent[4] == outputExtent[4] &&
       inputExtent[5] == outputExtent[5] )
    {
    outputImage->ShallowCopy( inputImage );
    return VTK_OK;
    }

  vtkFloatArray *tCoords = vtkFloatArray::SafeDownCast(
    vtkDataSet::SafeDownCast( inputPD )->GetPointData()->GetTCoords() );
  if (!tCoords)
    {
  ...
  if (!this->Renderer)
    ...
    {
    this->ComputeSAndTRangeBasedOnRenderer(inputPD, tCoords, insidedness,
      computedSRange, computedTRange);
                                                          ...
                                                          double computedTRange[2])
{
  ...
  vtkDoubleArray *normals = vtkDoubleArray::SafeDownCast( this->ExtractFrustum->GetFrustum()->GetNormals() );
  int maxIndex = 0;
  for (int i = 0; i < 6; i++)
    {
    maxIndex = fabs(frustumPlanes[i*4]) > fabs(frustumPlanes[i*4 + 1]) ?
      0 : 1;
    maxIndex = fabs(frustumPlanes[i*4 + maxIndex]) > fabs(frustumPlanes[i*4 + 2]) ?
      maxIndex : 2;
    ...
    normals->SetTuple3(i, -frustumPlanes[i*4], -frustumPlanes[i*4 + 1], -frustumPlanes[i*4 + 2]);
    }

  this->ExtractFrustum->Modified();
  if (this->TransformationMatrix &&
    (this->TransformationMatrix->GetElement(0,0) != 1.0 ||
  ...
  insidedness = vtkSignedCharArray::SafeDownCast(
    vtkDataSet::SafeDownCast( this->ExtractFrustum->GetOutput() )->
    GetPointData()->GetArray( "vtkInsidedness" ) );
...
//----------------------------------------------------------------------------
void vtkImageTextureCrop::DoImageResample(int inputExtents[6],
                                          int outputExtents[6],
                                          vtkImageData *cropImage,
                                          vtkImageData *outputImage)
{
  double magnificationFactor[2];

  magnificationFactor[0] = 1e-5 + static_cast<double>(outputExtents[1]) /
    static_cast<double>(inputExtents[1]);
  magnificationFactor[1] = 1e-5 + static_cast<double>(outputExtents[3]) /
    static_cast<double>(inputExtents[3]);
                                         ...
                                         double *sRange, double *tRange)
{
  ...
  for (int i = 0; i < tCoords->GetNumberOfTuples(); i++)
    ...
    if (!insidedness || insidedness->GetValue(i) > 0)
      ...
      tCoords->SetTuple2(i,
        (origTuple[0] - sRange[0]) / (sRange[1] - sRange[0]),
