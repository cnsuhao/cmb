...
//-----------------------------------------------------------------------------
int vtkCMBPolyReader::RequestData(
...
int vtkCMBPolyReader::Read3DFile(ifstream &fin, int numPts, int numberOfAttributes, int hasBoundaryMarkers,
                                  vtkPolyData *output)
{
...
//-----------------------------------------------------------------------------
int vtkCMBPolyReader::Read2DFile(ifstream &fin, int numPts, int numberOfAttributes, int hasBoundaryMarkers,
                                  vtkPolyData *output)
{
  // if numPts == 0 then we need to read them from a .node file
  double pt[3];
...
//-----------------------------------------------------------------------------
int vtkCMBPolyReader::GetNextLineOfData(ifstream &fin,
                                         std::stringstream &lineStream)
{
...
//----------------------------------------------------------------------------
int vtkCMBPolyReader::RequestInformation(
  vtkInformation *vtkNotUsed(request),
  vtkInformationVector **vtkNotUsed(inputVector),
