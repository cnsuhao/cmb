...
struct vtkCMBMeshWriterInternals
{
  ...
  void InitStructured()
  {
    ...
    if (this->structured->HasAnyBlankPoints())
    {
      ...
      {
        this->ptFileIdArray->SetValue(
          cc, this->structured->IsPointVisible(cc) ? this->nVisPts++ : INVISIBLE_ID);
      }
    }
...
void vtkCMBMeshWriter::WriteData()
{
  ...
  if (!file || !this->WriteHeader(*file) || !this->WriteCells(*file) || !this->WritePoints(*file) ||
  ...
  if (array)
  {
    ...
    switch (array->GetDataType())
    {
      vtkTemplateMacro(
        ...
        return static_cast<unsigned long>(reinterpret_cast<VTK_TT*>(data)[cc * numComps]););
    }
  }
  return 1;
}

bool vtkWriteCell(ostream& fp, vtkIdType npts, vtkIdType* ptIds, vtkIdTypeArray* fileIdArray)
{
...
bool vtkCMBMeshWriter::WriteCells(ostream& fp)
{
  ...
  {
    cellOffset =
      this->Internals->poly->GetNumberOfVerts() + this->Internals->poly->GetNumberOfLines();
...
bool vtkCMBMeshWriter::WritePoints(ostream& fp)
{
  ...
  if (this->Internals->ptFileIdArray)
  {
    for (vtkIdType cc = 0, id = 0; cc < numPts; ++cc)
    {
      ...
      fp << card << SEPARATOR << id + 1 << SEPARATOR << pts[0] << SEPARATOR << pts[1] << SEPARATOR
         << pts[2] << endl;
    }
  }
  else
  {
    ...
    {
      points->GetPoint(cc, pts);
      fp << card << SEPARATOR << cc + 1 << SEPARATOR << pts[0] << SEPARATOR << pts[1] << SEPARATOR
         << pts[2] << endl;
    }
  }
  return true;
...
bool vtkCMBMeshWriter::ValidateFileFormat()
{
  ...
  for (int i = 0; i < numTypes && valid; ++i)
  {
    ...
    if (!valid)
    {
      ...
      else
      {
        ...
        msg << "file format does not support the " << types->GetClassNameFromTypeId(type)
            << " cell type.";
      }
  ...
  if (valid && !this->ValidateDimension)
  {
    switch (this->FileFormat)
    {
      case PT123:
      case WASH123D:
        if ((types->IsType(VTK_QUAD) || types->IsType(VTK_PIXEL)) && types->IsType(VTK_TETRA))
        {
          vtkWarningMacro(
            "The " << vtkCMBMeshFormatStrings[this->FileFormat]
...
void vtkCMBMeshWriter::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  os << indent << "FileFormat: " << vtkCMBMeshFormatStrings[this->FileFormat] << endl;
  os << indent << "MeshDimension: " << MeshDimension << endl;
  os << indent << "ValidateDimension: " << (this->ValidateDimension ? "On" : "Off") << endl;
  os << indent << "WriteMetaInfo: " << (this->WriteMetaInfo ? "On" : "Off") << endl;
  os << indent << "UseScientificNotation: " << (this->UseScientificNotation ? "On" : "Off") << endl;
