...
struct vtkCMBMeshReaderInternals
{
  ...
  vtkCMBMeshReaderInternals() : Stream(0), Line(0) {}
  ...
  vtkInformationVector *outputVector)
{
  ...
  *this->Internals->Line >> card;
  ...
  if (card != "MESH" && // MESH is for backwards compatibility
      card != "MESH1D" && card != "MESH2D" && card != "MESH3D" &&
  ...
  vtkDebugMacro(<< "Mesh information: " << numCells << " elements, " <<
    numPts << " nodes");
  // Empty file or abort
  ...
  if (CreateMeshMaterialIdArray)
    ...
    cellMaterialArray->SetName(RenameMaterialAsRegion ?
                               vtkMultiBlockWrapper::GetShellTagName() : "Mesh Material ID");
    ...
    *this->Internals->Line >> card;
    ...
    if (card == "ND" || card == "GN")
      ...
      this->ReadCell(VTK_TRIANGLE, 3, output, cellMaterialArray, cellIdArray);
             ...
             (card == "GE4" && this->MeshDimension == MESH3D))
      {
      this->ReadCell(VTK_TETRA, 4, output, cellMaterialArray, cellIdArray);
      }
    else if (card == "E4Q" ||
             (card == "GE4" && this->MeshDimension == MESH2D))
      {
      ...
      {
      this->ReadCell(VTK_HEXAHEDRON, 8, output, cellMaterialArray,
        cellIdArray);
      }
      ...
      {
      this->ReadCell(VTK_QUADRATIC_TRIANGLE, 6, output, cellMaterialArray,
        cellIdArray);
      }
    else if (card == "E8Q")
      {
      this->ReadCell(VTK_QUADRATIC_QUAD, 8, output, cellMaterialArray,
        cellIdArray);
      }
    else if (card == "E9Q")
      {
      this->ReadCell(VTK_BIQUADRATIC_QUAD, 9, output, cellMaterialArray,
        cellIdArray);
      }
    else if (card == "E3L")
      {
      this->ReadCell(VTK_QUADRATIC_EDGE, 3, output, cellMaterialArray,
        cellIdArray);
      }
    this->Internals->ReadLine();
    // Check Abort
...
int vtkCMBMeshReader::CanReadFile(const char *fname)
{
  ...
  return card == "MESH" || card == "MESH2D" || card == "MESH3D" ||
         card == "WMS1DM" || card == "WMS2DM" || card == "WMS3DM";
}
...
int vtkCMBMeshReader::PreviewFile(vtkIdType& ncells, vtkIdType& npts)
{
  ...
  while (!this->Internals->Stream->eof())
  {
    ...
    *this->Internals->Line >> card;
    ...
    else if (card == "E3T" || card == "GE3" || card == "E4T" ||
             card == "GE4" || card == "E4Q" || card == "E6W" ||
             card == "GE6" || card == "E5P" || card == "E8H" ||
             card == "GE8" || card == "E6T" || card == "E8Q" ||
             card == "E9Q" || card == "E2L" || card == "GE2" ||
             card == "E3L")
      {
      *this->Internals->Line >> id;
...
void vtkCMBMeshReader::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  os << indent << "MeshElementIDArray: " <<
    (this->CreateMeshElementIdArray ? "On" : "Off") << endl;
  os << indent << "MeshNodeIDArray: " <<
    (this->CreateMeshNodeIdArray ? "On" : "Off") << endl;
  os << indent << "MeshMaterialIDArray: " <<
    (this->CreateMeshMaterialIdArray ? "On" : "Off") << endl;
  os << indent << "RenameMaterialAsRegion: " <<
