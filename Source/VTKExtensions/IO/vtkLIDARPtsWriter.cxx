...
{
  vtkDataArray *scalars = inputPoly->GetPointData() ?
  inputPoly->GetPointData()->GetScalars("Color") : NULL;

  vtkUnsignedCharArray *rgbScalars = scalars ?
  vtkUnsignedCharArray::SafeDownCast(scalars) : NULL;

  vtkFloatArray *intensityArray = inputPoly->GetPointData() ?
    vtkFloatArray::SafeDownCast(inputPoly->GetPointData()->GetArray("Intensity")) : NULL;
  ...
  if(this->OutputIsBinary)
    ...
    requiredPrecision =
      zPrecision > requiredPrecision ? zPrecision : requiredPrecision;
    ofp.precision(requiredPrecision);
    ofp << numPts << endl;
    }

  double pts[3], rgb[3];
  vtkIdType numColorTuples = rgbScalars ?
  rgbScalars->GetNumberOfTuples() : 0;
  for (vtkIdType cc=0; cc < numPts; cc++)
    ...
    if(this->OutputIsBinary)
      ...
      {
      ofp << pts[0] << LIDAR_ASCII_SEPERATOR
        << pts[1] << LIDAR_ASCII_SEPERATOR
        << pts[2];
      if (intensityArray)
        {
      ...
      if(rgbScalars && cc < numColorTuples)
        ...
        ofp << LIDAR_ASCII_SEPERATOR
          << rgb[0] << LIDAR_ASCII_SEPERATOR
          << rgb[1] << LIDAR_ASCII_SEPERATOR
          << rgb[2];
        }
...
//----------------------------------------------------------------------------
int vtkLIDARPtsWriter::FillInputPortInformation(int,
  vtkInformation *info)
{
...
void vtkLIDARPtsWriter::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  os << indent << "Write As Single Piece: "
     << (this->WriteAsSinglePiece ? "On" : "Off") << "\n";
}
//----------------------------------------------------------------------------
