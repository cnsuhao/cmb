...
int vtkCMBICMReader::ReadTemporalData(vtkInformationVector* vtkNotUsed(outputVector))
{
  ...
  if (!this->DataFileName || strcmp(this->DataFileName, "") == 0)
  {
    ...
    this->SetDataFileName(
      (vtksys::SystemTools::GetFilenamePath(this->FileName) + "/" +
        vtksys::SystemTools::GetFilenameWithoutExtension(this->FileName) + "-timedata.dat")
        .c_str());
  }
  ...
  {
    this->SetDataFileName(
      (vtksys::SystemTools::GetFilenamePath(this->FileName) + "/" + this->DataFileName).c_str());
  }
  ...
  while (readNextLine(file, line))
  {
    ...
    for (int i = 0; i < (this->NumberOfCells); i++)
    {
      ...
      {
        vtkErrorMacro("Incorrect number of lines in "
...
int vtkCMBICMReader::ReadGeometryData(vtkInformationVector* vtkNotUsed(outputVector))
{
  ...
  while (readNextLine(file, line))
  {
    for (int count = 0; count < 8; count++)
    {
      if (!readNextLine(file, line))
      {
        vtkErrorMacro("Incorrect file format: " << fileNameStr);
      }
      this->NumberOfPoints++;
    }
  ...
  while (readNextLine(file, line))
  {
    ...
    this->UpdateProgress(
      (static_cast<double>(file.tellg()) + numBytes) / static_cast<double>(numBytes2));
  }
  //Clean up pass 2
  file.close();
  return 1;
  ...
  vtkInformationVector** vtkNotUsed(inputVector), vtkInformationVector* outputVector)
{
    ...
    this->OldDataIsPositiveEast != this->DataIsPositiveEast)
  {
    ...
    this->vectors->SetName(
      vtksys::SystemTools::GetFilenameWithoutExtension(this->FileName).c_str());
