...
//----------------------------------------------------------------------------
int vtkDEMRasterWriter::FillInputPortInformation(int,
                                                vtkInformation *info)
{
...
void vtkDEMRasterWriter::createOutputFile(std::string fname,
                                          vtkPolyData *inputPoly)
{
  if(inputPoly == NULL) return;
  ...
  for (vtkIdType i=0; i < size; ++i)
    ...
    IdToElevation[i] = p[2];
    if(p[2] < min_ele) min_ele = p[2];
    if(p[2] > fabs(max_ele)) max_ele = fabs(p[2]);
    p[2] = 0.0;
  ...
  double space[2] = {(max[0]-min[0])/RasterSize[0],
                     (max[1]-min[1])/RasterSize[1]};
    ...
    for(unsigned int y = 0; y < nobY; ++y)
      ...
      for ( vtkIdType i=0; i < size; ++i)
        ...
        if ( it != IdToElevation.end() )
          ...
          double d = ((p[0]-dpoint[0])*(p[0]-dpoint[0]))/rx2 + ((p[1]-dpoint[1])*(p[1]-dpoint[1]))/ry2;
          if(d >= 1) continue;
          double w = 1 - d;
          if(w > 1) w = 1.0;
          if(w < 0) w = 0.0;
  ...
  poBand->RasterIO( GF_Write, 0, 0, nobX, nobY,
                    matix, nobX, nobY, GDT_Float64, 0, 0 );
  ...
  GDALDataset * poDstDS = demDriver->CreateCopy( fname.c_str(), ds, FALSE,
                                                 demOptions, GDALTermProgress, NULL );
  if( poDstDS != NULL )
    {
