...
int vtkSGXMLBCSWriter::WriteData()
{
  if (!this->Coords || this->Coords->GetNumberOfComponents() != 3 || !this->ModelVertexIds ||
    this->ModelVertexIds->GetNumberOfComponents() != 1)
  {
  ...
  vtkSmartPointer<vtkIdTypeArray> VertexMarker = vtkSmartPointer<vtkIdTypeArray>::New();
  ...
  vtkSmartPointer<vtkXMLDataElement> MeshEdges = vtkSmartPointer<vtkXMLDataElement>::New();
  ...
  MeshEdges->SetAttribute("Description",
    ...
    "points of the cell, the second is the model edge the cell belongs to and the "
    "subsequent numbers are the point ids of the cell.");
  ...
  {
    conn << indent4 << " 2 " << CurrentModelEdgeId << " " << i << " " << i + 1 << endl;
  ...
  conn << indent4 << " 2 " << CurrentModelEdgeId << " " << VertexMarker->GetNumberOfTuples() - 1
       << " 0\n";
  ...
  materials->SetAttribute("Description", "Implicitly ordered materials starting at 0 and numbered "
                                         "consecutively along with a Name of the material that can "
                                         "be set and a Unique Persistent Id of the material.");
  ...
  modelFaces->SetAttribute("Description",
    "Implicitly ordered model faces starting at 0 and numbered consecutively.  A model face is an "
    "aggregation of mesh facets that is used to specify information collectively for the mesh "
    "facets.  The model face has two associated model regions and User specified Name. "
    "ModelRegionId1 corresponds to the side of the model face that the mesh facets' normals point "
    "towards (using a counterclockwise ordering). A -1 indicates that there is no associated model "
    "region for the indicated side of the model face. If the problem is 2D then there are no "
    "associated model regions but there is an associated material instead.");
  ...
  //
  vtkSmartPointer<vtkXMLDataElement> modelEdges = vtkSmartPointer<vtkXMLDataElement>::New();
  ...
  modelEdges->SetAttribute("Description",
    "Implicitly ordered model edges starting at 0 and numbered consecutively.  A model edge is an "
    "aggregation of mesh edges that is used to specify information collectively for the mesh "
    "edges.  The model edge has multiple associated model faces (at most 2 for a 2D problem "
    "though) that are stored in the Data nested XML element and a User specified Name. The model "
    "face ids that are stored are using their implicit numbering.");
  bcsfile->AddNestedElement(modelEdges);
  for (i = 0; i < NumberOfModelEdges; i++)
  {
    vtkSmartPointer<vtkXMLDataElement> modelEdge = vtkSmartPointer<vtkXMLDataElement>::New();
    ...
    vtkSmartPointer<vtkXMLDataElement> EdgeFaces = vtkSmartPointer<vtkXMLDataElement>::New();
