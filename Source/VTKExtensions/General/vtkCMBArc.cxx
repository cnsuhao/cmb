...
void vtkCMBArc::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  for (int i = 0; i < this->GetNumberOfEndNodes(); ++i)
  {
    ...
    cout << indent.GetNextIndent() << " position " << position[0] << "," << position[1] << ","
         << position[2] << endl;
    cout << endl;
  }
...
bool vtkCMBArc::InternalInsertNextPoint(InternalPointList& inPoints, vtkCMBArc::Point& point)
{
...
{
  if (numPoints <= 0 || startIndex < 0 || numPoints > this->GetNumberOfInternalPoints() ||
  ...
  {
    std::advance(this->PointIterator, forwardDirection ? startIndex : startIndex - numPoints + 1);
...
bool vtkCMBArc::GetNextPoint(vtkCMBArc::Point& point)
{
  ...
  if (this->PointIterationForward)
  {
    ...
    {
      std::advance(this->PointIterator, this->NumberOfSpecifiedTraversalPoints - 2);
    }
    if (this->PointIterator == this->Points.end())
    {
      return false;
  ...
  else
  {
    ...
    // but include the two end points
    if (this->InvertTraversalRange &&
      this->CurrentNumberOfTraversedPoints > (numArcPoints - this->TraversalStartIndex) &&
      this->NumberOfSpecifiedTraversalPoints > 2)
    {
      std::advance(this->ReversePointIterator, this->NumberOfSpecifiedTraversalPoints - 2);
    }
    if (this->ReversePointIterator == this->Points.rend())
    {
      return false;
...
bool vtkCMBArc::GetEndNodeDirection(const int& index, double direction[3]) const
{
  ...
  if (this->GetNumberOfInternalPoints() > 0)
  {
    ...
    const vtkCMBArc::Point iteratorPoint =
      (index == 0) ? this->Points.front() : this->Points.back();
...
void vtkCMBArc::UnMarkedForDeletion()
{
  ...
  for (it = this->UndoEndNodes.begin(); it != this->UndoEndNodes.end(); it++, index++)
  {
  ...
  std::list<vtkCMBArc::Point>& newPoints, bool includeEnds)
{
  ...
  vtkIdType numPoints =
    endIndex > startIndex ? (endIndex - startIndex + 1) : (startIndex - endIndex + 1);
  ...
  {
    this->InternalInsertNextPoint(subPoints, forwardDir ? newPoints.front() : newPoints.back());
    if (forwardDir)
    {
      newPoints.pop_front();
    }
  ...
  vtkIdType startId, vtkIdType endId, vtkIdType numArcPoints, bool closedArc)
{
  ...
  {
    numRequestedArcPoints = (endId >= startId) ? (endId - startId + 1) : (startId - endId + 1);
  }

  return ((numRequestedArcPoints == numArcPoints && !closedArc) || (closedArc && startId == endId));
}
