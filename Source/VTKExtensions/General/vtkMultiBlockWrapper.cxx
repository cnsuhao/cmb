...
int vtkMultiBlockWrapper::GetModelFaceId(vtkPolyData* poly)
{
  ...
  vtkIntArray* array = vtkIntArray::SafeDownCast(field->GetArray(ModelFaceDataString));
  if (array)
  {
...
void vtkMultiBlockWrapper::SetModelFaceUse1(int modelFaceId, int shellId)
{
  ...
  vtkIntArray* data = vtkIntArray::SafeDownCast(
    master->GetFieldData()->GetArray(vtkMultiBlockWrapper::GetModelFaceUse1String()));
  if (!data)
  {
  ...
  {
    data = vtkIntArray::SafeDownCast(this->PerformNeededDeepCopy(data, master->GetFieldData()));
...
vtkMultiBlockDataSet* vtkMultiBlockWrapper::GetModelFaceRootBlock()
{
  ...
  {
    mfData = vtkMultiBlockDataSet::SafeDownCast(this->mb->GetBlock(ModelFaceRootIndex));
  }
  return mfData;
}
...
vtkMultiBlockDataSet* vtkMultiBlockWrapper::GetNodalGroupRootBlock()
{
  ...
  {
    ngData = vtkMultiBlockDataSet::SafeDownCast(this->mb->GetBlock(NodalGroupRootIndex));
  }
  return ngData;
}
...
void vtkMultiBlockWrapper::GetModelFaceIds(vtkIdList* ids)
{
  ...
  if (this->mb)
  {
    ...
    {
      vtkIntArray* modelFaceInfo = vtkIntArray::SafeDownCast(
...
void vtkMultiBlockWrapper::GetBCSIds(vtkIdList* ids)
{
  ...
  if (this->mb)
  {
    ...
    vtkFloatArray* rgba =
      vtkFloatArray::SafeDownCast(poly->GetFieldData()->GetArray(BCSColorsString));
    if (!rgba)
    {
      return;
    }
...
void vtkMultiBlockWrapper::GetMaterialIds(vtkIdList* ids)
{
  ...
  if (this->mb)
  {
    vtkPolyData* poly = this->GetMasterPolyData();
    vtkFloatArray* rgba =
      vtkFloatArray::SafeDownCast(poly->GetFieldData()->GetArray(MaterialColorsString));
    for (int i = 0; i < rgba->GetNumberOfTuples(); i++)
    {
      if (rgba->GetValue(i * 4 + 3) >= -1)
      {
...
void vtkMultiBlockWrapper::GetShellIds(vtkIdList* ids)
{
  ...
  if (this->mb)
  {
    vtkPolyData* poly = this->GetMasterPolyData();
    vtkFloatArray* rgba =
      vtkFloatArray::SafeDownCast(poly->GetFieldData()->GetArray(ShellColorsString));
    for (int i = 0; i < rgba->GetNumberOfTuples(); i++)
    {
      if (rgba->GetValue(i * 4 + 3) >= -1)
      {
...
double* vtkMultiBlockWrapper::GetShellTranslationPoint(vtkIdType ShellId)
{
  ...
  vtkDoubleArray* array =
    vtkDoubleArray::SafeDownCast(master->GetFieldData()->GetArray(ShellTranslationPointString));
...
void vtkMultiBlockWrapper::SetShellTranslationPoint(vtkIdType ShellId, double* translationPoint)
{
  ...
  if (ShellId < array->GetNumberOfTuples())
  {
    if (translationPoint)
    {
      double vals[4] = { translationPoint[0], translationPoint[1], translationPoint[2], 1 };
...
void vtkMultiBlockWrapper::ProcessForWriting(vtkPolyData* poly)
{
  ...
  vtkIntArray* version = vtkIntArray::SafeDownCast(
    this->PerformNeededDeepCopy(fieldData->GetArray(CMBFileVersionString), fieldData));
  if (!version)
  {
...
int vtkMultiBlockWrapper::CreateModelFace(vtkIdList* cellIds)
{
  ...
  this->PerformNeededShallowCopy(this->GetMasterPolyData(), MasterPolyRootIndex, this->mb);
  // first do shallow copy on model face multiblock
  this->PerformNeededShallowCopy(this->GetModelFaceRootBlock(), ModelFaceRootIndex, this->mb);
  ...
  oldModelFace = vtkPolyData::SafeDownCast(
    this->PerformNeededDeepCopy(oldModelFace, blockIndex, this->GetModelFaceRootBlock()));

  vtkIntArray* rcinfo =
    vtkIntArray::SafeDownCast(oldModelFace->GetCellData()->GetArray(RCNameString));
  rcinfo =
    vtkIntArray::SafeDownCast(this->PerformNeededDeepCopy(rcinfo, oldModelFace->GetCellData()));
  vtkIdType i;
  for (i = 0; i < cellIds->GetNumberOfIds(); i++)
  {
    ...
    this->SetModelFaceCellId(
      rcinfo->GetValue(lastCellId), this->GetModelFaceCellId(cellIds->GetId(i)));
    rcinfo->SetValue(this->GetModelFaceCellId(cellIds->GetId(i)), rcinfo->GetValue(lastCellId));
    oldModelFace->DeleteCell(lastCellId);
    this->needToRemoveDeletedCells = true;
  }

  vtkIntArray* modelFaceData = vtkIntArray::SafeDownCast(this->PerformNeededDeepCopy(
    oldModelFace->GetFieldData()->GetArray(ModelFaceDataString), oldModelFace->GetFieldData()));
  ...
  vtkFloatArray* floatArray =
    vtkFloatArray::SafeDownCast(fieldData->GetArray(ModelFaceColorsString));
  float rgba[4];
  floatArray->GetTypedTuple(oldModelFaceId, rgba);
  // get any existing modelfaceuse1 information
  vtkAbstractArray* usearray = fieldData->GetArray(vtkMultiBlockWrapper::GetModelFaceUse1String());
  int modelFaceUse1 = -1;
  if (usearray)
  {
...
int vtkMultiBlockWrapper::MergeModelFaces(vtkIdType targetFaceId, vtkIdList* faceIds)
{
  ...
  vtkPolyData* targetFace = vtkPolyData::SafeDownCast(this->PerformNeededDeepCopy(
    origTargetFace, this->GetModelFaceBlockIndex(targetFaceId), this->GetModelFaceRootBlock()));

  vtkIntArray* targetCellIdArray = vtkIntArray::SafeDownCast(this->PerformNeededDeepCopy(
    targetFace->GetCellData()->GetArray(RCNameString), targetFace->GetCellData()));
  /*
...
*/
  ...
  for (int i = 0; i < faceIds->GetNumberOfIds(); i++)
  {
    ...
    if (facePoly)
    {
      vtkIntArray* cellIdArray =
        vtkIntArray::SafeDownCast(facePoly->GetCellData()->GetArray(RCNameString));
...
void vtkMultiBlockWrapper::CreateModelFaces(vtkIntArray* markedCells, vtkIntArray* modelFaceIds)
{
  ...
  for (i = 0; i < numCells; i++)
  {
    if (markedCells->GetValue(i) != 0)
    {
      ...
      std::map<int, vtkIdList*>::iterator it = newModelFaces.find(modelFaceId);
  ...
  for (std::map<int, vtkIdList*>::iterator it = newModelFaces.begin(); it != newModelFaces.end();
       it++)
  {
  ...
  vtkIntArray* modelFaceData, int isLoadingFile)
{
  ...
  this->PerformNeededDeepCopy(modelFaceCellIds->GetArray(ModelFaceCellIdsString), modelFaceCellIds);
  ...
  if (numBCSIds)
  {
    ...
    for (i = 3; i < modelFaceData->GetNumberOfTuples(); i++)
    {
      ...
      vtkIntArray* bcs = vtkIntArray::SafeDownCast(this->PerformNeededDeepCopy(
        polyFieldData->GetArray(this->GetBCSNameFromId(bcsid)), polyFieldData));
      int j = 0;
      bool insert = 1;
  ...
  int data[3] = { modelFaceData->GetValue(shellIdArrayLoc),
    modelFaceData->GetValue(materialIdArrayLoc), modelFaceId };
  ...
  vtkFloatArray* RGBA = vtkFloatArray::SafeDownCast(this->PerformNeededDeepCopy(
    poly->GetFieldData()->GetArray(ModelFaceColorsString), poly->GetFieldData()));
  if (!RGBA)
  {
...
vtkPolyData* vtkMultiBlockWrapper::GetModelFaceWithIndex(int mfIndex)
{
  if (this->GetModelFaceRootBlock() && mfIndex >= 0 &&
    mfIndex < static_cast<int>(this->GetModelFaceRootBlock()->GetNumberOfBlocks()))
  {
    return vtkPolyData::SafeDownCast(this->GetModelFaceRootBlock()->GetBlock(mfIndex));
  }
...
vtkIdType vtkMultiBlockWrapper::GetCellIdOnMasterPolyData(vtkIdType cellId, int modelFaceId)
{
  ...
  if (poly)
  {
    vtkIntArray* rcArray = vtkIntArray::SafeDownCast(poly->GetCellData()->GetArray(RCNameString));
...
{
  vtkPolyData* poly = this->GetMasterPolyData();
  vtkStringArray* userNames =
    vtkStringArray::SafeDownCast(poly->GetFieldData()->GetAbstractArray(MaterialUserNamesString));
  materialUserName = userNames->GetValue(materialId);
  vtkIntArray* userIds =
    vtkIntArray::SafeDownCast(poly->GetFieldData()->GetArray(MaterialUserIdsString));
...
void vtkMultiBlockWrapper::GetBCSUserData(int BCSId, int& BCSUserId, const char*& BCSUserName)
{
  vtkPolyData* poly = this->GetMasterPolyData();
  vtkStringArray* userNames =
    vtkStringArray::SafeDownCast(poly->GetFieldData()->GetAbstractArray(BCSUserNamesString));
  BCSUserName = userNames->GetValue(BCSId);
  vtkIntArray* userIds =
    vtkIntArray::SafeDownCast(poly->GetFieldData()->GetArray(BCSUserIdsString));
...
const char* vtkMultiBlockWrapper::GetModelFaceUserName(int modelFaceId)
{
  vtkPolyData* poly = this->GetMasterPolyData();
  vtkStringArray* userNames =
    vtkStringArray::SafeDownCast(poly->GetFieldData()->GetAbstractArray(ModelFaceUserNamesString));
...
const char* vtkMultiBlockWrapper::GetShellUserName(int shellId)
{
  vtkPolyData* poly = this->GetMasterPolyData();
  vtkStringArray* userNames =
    vtkStringArray::SafeDownCast(poly->GetFieldData()->GetAbstractArray(ShellUserNamesString));
...
void vtkMultiBlockWrapper::GetModelFaceBCSIds(int modelFaceId, vtkIdList* list)
{
  ...
  vtkIntArray* BCSIds = vtkIntArray::SafeDownCast(field->GetArray(ModelFaceDataString));
...
int vtkMultiBlockWrapper::GetModelFaceMaterialId(int modelFaceId)
{
  ...
  vtkIntArray* modelFaceData =
    vtkIntArray::SafeDownCast(field->GetArray(ModelFaceConvenientArrayName));
  int vals[3];
...
unsigned int vtkMultiBlockWrapper::GetModelFaceBlockIndex(int modelFaceId)
{
  ...
  for (int i = 0; i < this->GetNumberOfModelFaces(); i++)
  {
    if (this->GetModelFaceWithIndex(i))
    {
      ...
      vtkIntArray* modelFaceData = vtkIntArray::SafeDownCast(field->GetArray(ModelFaceDataString));
...
void vtkMultiBlockWrapper::ChangeMaterialIdOfShell(int shellId, int newMaterialId)
{
  ...
  vtkIntArray* array = vtkIntArray::SafeDownCast(
    this->PerformNeededDeepCopy(field->GetArray(ShellMaterialIdString), field));
  ...
  vtkIntArray* MFData = vtkIntArray::SafeDownCast(
    this->PerformNeededDeepCopy(field->GetArray(ModelFaceConvenientArrayName), field));
  if (MFData)
  {
    int vals[3];
  ...
  for (int ui = 0; ui < this->GetNumberOfModelFaces(); ui++)
  {
    ...
    array = vtkIntArray::SafeDownCast(field->GetArray(ModelFaceDataString));
    if (shellId == array->GetValue(shellIdArrayLoc))
    {
      if (array->GetValue(materialIdArrayLoc) != newMaterialId)
      {
        this->PerformNeededShallowCopy(
          this->GetModelFaceWithIndex(ui), ui, this->GetModelFaceRootBlock());
        field = this->GetModelFaceWithIndex(ui)->GetFieldData();
...
void vtkMultiBlockWrapper::ChangeUserNameOfMaterial(int materialId, const char* userName)
{
  ...
  vtkStringArray* matNames = vtkStringArray::SafeDownCast(
    this->PerformNeededDeepCopy(field->GetAbstractArray(MaterialUserNamesString), field));
...
void vtkMultiBlockWrapper::ChangeUserNameOfShell(int shellId, const char* userName)
{
  ...
  vtkStringArray* shellNames = vtkStringArray::SafeDownCast(
    this->PerformNeededDeepCopy(field->GetAbstractArray(ShellUserNamesString), field));
...
void vtkMultiBlockWrapper::ChangeUserNameOfBCS(int BCSId, const char* userName)
{
  ...
  vtkStringArray* BCSNames = vtkStringArray::SafeDownCast(
    this->PerformNeededDeepCopy(field->GetAbstractArray(BCSUserNamesString), field));
...
void vtkMultiBlockWrapper::ChangeUserNameOfModelFace(int modelFaceId, const char* userName)
{
  ...
  vtkStringArray* ModelFaceNames = vtkStringArray::SafeDownCast(
    this->PerformNeededDeepCopy(field->GetAbstractArray(ModelFaceUserNamesString), field));
  ...
  int BCSUserId, const char* BCSUserName, float* colors, vtkIdList* modelFaceIds)
{
  ...
  for (int i = 0; i < modelFaceIds->GetNumberOfIds(); i++)
  {
    ...
    unsigned int blockIndex = this->GetModelFaceBlockIndex(modelFaceIds->GetId(i));
    this->PerformNeededShallowCopy(
      this->GetModelFaceWithIndex(blockIndex), blockIndex, this->GetModelFaceRootBlock());
    array->SetValue(i, modelFaceIds->GetId(i));
    // need to add this BCS to the model face info
    vtkFieldData* modelFaceFieldData = this->GetModelFaceWithIndex(blockIndex)->GetFieldData();
    // there better be field data already associated with the model face
    vtkIntArray* dataArray = vtkIntArray::SafeDownCast(this->PerformNeededDeepCopy(
      modelFaceFieldData->GetArray(ModelFaceDataString), modelFaceFieldData));
  ...
  vtkFloatArray* colorArray = vtkFloatArray::SafeDownCast(
  ...
  vtkStringArray* nameArray = vtkStringArray::SafeDownCast(
    this->PerformNeededDeepCopy(fieldData->GetAbstractArray(BCSUserNamesString), fieldData));
  vtkIntArray* idArray = vtkIntArray::SafeDownCast(
    this->PerformNeededDeepCopy(fieldData->GetArray(BCSUserIdsString), fieldData));
  if (!colorArray)
  {
...
void vtkMultiBlockWrapper::DeleteBCS(const char* name)
{
  ...
  for (vtkIdType id = 0; id < array->GetNumberOfTuples(); id++)
  {
    ...
    for (vtkIdType id2 = modelFaceArray->GetNumberOfTuples() - 1; id2 > 2;
         id2--) // first three are shell, material id and model face ids
    {
  ...
  int MaterialUserId, const char* MaterialUserName, float* colors)
{
  ...
  vtkFloatArray* colorArray = vtkFloatArray::SafeDownCast(
    this->PerformNeededDeepCopy(fieldData->GetArray(MaterialColorsString), fieldData));
  vtkStringArray* nameArray = vtkStringArray::SafeDownCast(
    this->PerformNeededDeepCopy(fieldData->GetAbstractArray(MaterialUserNamesString), fieldData));
  vtkIntArray* idArray = vtkIntArray::SafeDownCast(
    this->PerformNeededDeepCopy(fieldData->GetArray(MaterialUserIdsString), fieldData));
  if (!colorArray)
  {
    // assume that all of the arrays need to be created
    colorArray = vtkFloatArray::New();
...
void vtkMultiBlockWrapper::AddBCSToModelFace(int modelFaceId, int BCSId, const char* BCSName)
{
  vtkIntArray* BCSModelFaces =
    vtkIntArray::SafeDownCast(this->GetMasterPolyData()->GetFieldData()->GetArray(BCSName));
  if (!BCSModelFaces)
  {
  ...
  BCSModelFaces = vtkIntArray::SafeDownCast(
    this->PerformNeededDeepCopy(BCSModelFaces, this->GetMasterPolyData()->GetFieldData()));
...
void vtkMultiBlockWrapper::RemoveBCSFromModelFace(int modelFaceId, int BCSId, const char* BCSName)
{
  ...
  vtkIntArray* modelFaceBCSs =
    vtkIntArray::SafeDownCast(data->GetFieldData()->GetArray(ModelFaceDataString));
  for (i = modelFaceBCSs->GetNumberOfTuples() - 1; i > 2; i--)
  {
    if (BCSId == modelFaceBCSs->GetValue(i))
    {
      modelFaceBCSs =
        vtkIntArray::SafeDownCast(this->PerformNeededDeepCopy(modelFaceBCSs, data->GetFieldData()));
  ...
  const char* ShellUserName, float* RGBA, int materialId, double* translationPoint)
{
  ...
  vtkFloatArray* colorArray = vtkFloatArray::SafeDownCast(
    this->PerformNeededDeepCopy(fieldData->GetArray(ShellColorsString), fieldData));
  vtkStringArray* nameArray = vtkStringArray::SafeDownCast(
    this->PerformNeededDeepCopy(fieldData->GetAbstractArray(ShellUserNamesString), fieldData));

  vtkIntArray* shellMaterialArray = vtkIntArray::SafeDownCast(
    this->PerformNeededDeepCopy(fieldData->GetArray(ShellMaterialIdString), fieldData));

  vtkDoubleArray* translationPointArray = vtkDoubleArray::SafeDownCast(
    this->PerformNeededDeepCopy(fieldData->GetArray(ShellTranslationPointString), fieldData));
  if (!colorArray)
  {
    // assume that all of the arrays need to be created
    colorArray = vtkFloatArray::New();
...
int vtkMultiBlockWrapper::GetModelFaceId(int cellId)
{
  ...
  vtkIntArray* array = vtkIntArray::SafeDownCast(cellData->GetArray(ModelFaceIdsString));
...
void vtkMultiBlockWrapper::SetModelFaceId(int cellId, int modelFaceId)
{
  vtkCellData* cellData = this->GetMasterPolyData()->GetCellData();
  vtkIntArray* array = vtkIntArray::SafeDownCast(cellData->GetArray(ModelFaceIdsString));
  array = vtkIntArray::SafeDownCast(this->PerformNeededDeepCopy(array, cellData));
...
{
  vtkCellData* cellData = this->GetMasterPolyData()->GetCellData();
  vtkIntArray* array = vtkIntArray::SafeDownCast(cellData->GetArray(ModelFaceCellIdsString));
  return array->GetValue(cellId);
}
  ...
  vtkIntArray* ptIds, vtkIntArray* /*modelFaceIds*/, float* rgba, int isLoadingFile)
{
  ...
  // now add in rgba data
  vtkFloatArray* RGBA = vtkFloatArray::SafeDownCast(
    this->PerformNeededDeepCopy(fieldData->GetArray(NodeGroupColorsString), fieldData));
