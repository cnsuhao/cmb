...
int main(int argc, char* argv[])
{
  ...
  vtkSmartPointer<vtkCMBGlyphPointSource> points = vtkSmartPointer<vtkCMBGlyphPointSource>::New();
  ...
  for (j = 0, y = start; j < n; j++, y += delta)
  {
    for (i = 0, x = start; i < n; i++, x += delta)
    {
      ...
      zrot = b * 360.0;
      s = 4.0 * a / 2.49e+03;
      points->InsertNextPoint(x, y, 0.0, r, g, b, a, s, s, s, 0.0, 00.0, zrot, vis);
  ...
  vtkSmartPointer<vtkDiscreteLookupTable> lut = vtkSmartPointer<vtkDiscreteLookupTable>::New();
