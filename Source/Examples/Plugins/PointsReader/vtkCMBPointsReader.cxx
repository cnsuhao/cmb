...
vtkStandardNewMacro(vtkCMBPointsReader);
  ...
  vtkInformationVector** vtkNotUsed(inputVector), vtkInformationVector* outputVector)
{
  ...
  if (this->FileType == VTK_ASCII && (this->ValuesPerLine == 7 || this->ValuesPerLine == 4))
  {
...
int vtkCMBPointsReader::GetPointInfo(ifstream& fin)
{
  ...
  while (!fin.eof() && numPts <= 0)
  {
    if (this->FileType == VTK_ASCII)
    {
      ...
      do
      {
        ...
        {
          vtkErrorMacro(<< "Invalid Pts Format (missing number of points) in the file:"
                        << this->FileName);
          return VTK_ERROR;
        }
        else
  ...
  if (this->BytesPerPoint <= 0 || (this->FileType == VTK_ASCII && this->ValuesPerLine <= 0))
  {
  ...
  vtkFloatArray* intensityArray, vtkUnsignedCharArray* pieceIndexArray)
{
  ...
  for (long i = 0; i < numPts; i++)
  {
    ...
    if (i % onRatio == 0)
    {
      if (this->FileType == VTK_ASCII)
      {
        sscanf(buffer, "%lf %lf %lf %f %lf %lf %lf", pt, pt + 1, pt + 2, &intensity, rgb, rgb + 1,
          rgb + 2);
      }
      else if (onRatio > 1)
      {
