...
#ifndef rtvl_vote_txx
#define rtvl_vote_txx
...
public:
  rtvl_vote_internal(vnl_vector_fixed<double, N> const& voter_location,
    ...
    vnl_vector_fixed<double, N> const (&voter_basis)[N], rtvl_weight<N>& w);
...
{
  rtvl_vote_internal<N> vi(voter.location(), votee.location(), voter.basis(), w);
...
#define RTVL_VOTE_INSTANTIATE(N)                                                                   \
  ...
  template void rtvl_vote<N>(rtvl_voter<N>&, rtvl_votee<N>&, rtvl_weight<N>&, bool);               \
  template void rtvl_vote_component<N>(rtvl_vote_internal<N>&, vnl_matrix_fixed<double, N, N>&);   \
