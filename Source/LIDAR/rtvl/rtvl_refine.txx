...
template <unsigned int N> class rtvl_refine_quadtree;
...
public:
  ...
  rtvl_refine_quadtree<N> *quad;  // quad that contains this refine_level
...
public:
  ...
  ~rtvl_refine_quadtree() 
    {
    typename vcl_vector< rtvl_refine_level<N>* >::const_iterator levels_iter = this->levels.begin();
  ...
  void add_selections_for_next_level(double scale, double scale_multiplier,
    int processLevel, vcl_vector< rtvl_refine_level<N>* > &levels_subset);
    
  rtvl_refine_quadtree<N> *find_quad_with_level(int level);
  void get_lengths( double *lengths )
    {
    lengths[0] = this->bounds[1] - this->bounds[0];
    lengths[1] = this->bounds[3] - this->bounds[2];
    }
  rtvl_refine_quadtree<N> *get_quad(unsigned int depth, unsigned int i,
    unsigned int j);
  ...
  unsigned int gather_points(int level, double padLength,
    ...
    rgtl_object_array_points<N> &points, rtvl_refine_level<N> **(&pointsLevels),
    bool countOnly);
  void gather_levels(int level, vcl_vector< rtvl_refine_level<N>* > &levels_vector);
  void gather_levels_within_bounds(int level,
    vcl_vector< rtvl_refine_level<N>* > &levels_vector, double required_bounds[4]);
  bool within_bounds(double required_bounds[4]);
  void build_quad(vcl_list< rtvl_refine_quadtree<N>* > &quad_stack);

  rtvl_refine_quadtree<N> *parent;
  // 2 3
  // 0 1
  char position;
  rtvl_refine_quadtree<N> *child;
  double bounds[4]; // of this quad
  int tree_depth;
  rtvl_refine_quadtree<N> *quadRight;
  rtvl_refine_quadtree<N> *quadUp;
...
{
  vcl_list< rtvl_refine_quadtree<N>* > quad_stack;
...
void rtvl_refine_quadtree<N>::build_quad(vcl_list< rtvl_refine_quadtree<N>* > &quad_stack)
{
  if (this->tree_depth > 0)
    {
    this->child = new rtvl_refine_quadtree<N> [4];
...
//----------------------------------------------------------------------------
template <unsigned int N>
void rtvl_refine_quadtree<N>::initialize_points(int depth, 
                                                unsigned int i,
                                                unsigned int j,
                                                double scale,
                                                ...
                                                rgtl_object_array_points<N> &points)
{
  if (depth != 0)
    ...
    this->child[((i & mask) ? 1 : 0) + ((j & mask) ? 2 : 0)].initialize_points(depth, i, j, scale, indices, points);
    return;
    }

  rtvl_refine_level<N> *level = new rtvl_refine_level<N>;
...
//----------------------------------------------------------------------------
template <unsigned int N>
void rtvl_refine_quadtree<N>::add_selections_for_next_level(double scale,
                                                            double scale_multiplier,
                                                            int processLevel,
                                                            vcl_vector< rtvl_refine_level<N>* > &levels_subset)
{
  typename vcl_vector< rtvl_refine_level<N>* >::const_iterator levels_subset_iter = levels_subset.begin();
  ...
  rtvl_refine_level<N> *next_level = new rtvl_refine_level<N>;
  ...
  for (levels_subset_iter = levels_subset.begin(); 
    levels_subset_iter != levels_subset.end(); levels_subset_iter++)
    {    
    rtvl_refine_level<N> *this_level = (*levels_subset_iter);
...
//    unsigned int n = static_cast<unsigned int>((*levels_subset_iter)->selection.size());
    ...
    for(unsigned int i = 0; i < n; i++)
      ...
      next_level->calculate_tokens->at(new_points_index++).next_scale(scale_multiplier, max_saliency);
      }
...
{
  typename vcl_vector< rtvl_refine_level<N>* >::iterator level_iter = this->levels.begin();
...
//----------------------------------------------------------------------------
template <unsigned int N>
rtvl_refine_quadtree<N> *rtvl_refine_quadtree<N>::get_quad(unsigned int depth, 
                                                           unsigned int i,
                                                           unsigned int j)
{
  if (depth == 0)
...
//----------------------------------------------------------------------------
template <unsigned int N>
void rtvl_refine_quadtree<N>::gather_levels(int level,
                                            vcl_vector< rtvl_refine_level<N>* > &levels_vector)
{
...
//----------------------------------------------------------------------------
template <unsigned int N>
void rtvl_refine_quadtree<N>::gather_levels_within_bounds(int level,
                                                          vcl_vector< rtvl_refine_level<N>* > &levels_vector,
...
unsigned int rtvl_refine_quadtree<N>::gather_points(int level, double padLength,
                                                    vcl_vector< rtvl_refine_level<N>* > &levels_vector,
                                                    rgtl_object_array_points<N> &points,
                                                    rtvl_refine_level<N> **(&pointsLevels),
                                                    bool countOnly)
{
  ...
  rtvl_refine_quadtree *tempQuadRight = this->quadRight;
  ...
  rtvl_refine_quadtree *tempQuadUp = this->quadUp;
  ...
  typename vcl_vector< rtvl_refine_level<N>* >::iterator level_iter = levels_vector.begin();
  unsigned int total_points = 0;
  ...
  // now add the points... but from index of point need to be able to get to rtvl_refine_level
  ...
  pointsLevels = new rtvl_refine_level<N>* [total_points];
...
bool rtvl_refine_quadtree<N>::within_bounds(double required_bounds[4])
{
  ...
  for (i = 0; i < 4; i+=2)
    {
    if ((required_bounds[i] >= this->bounds[i]) &&
        (required_bounds[i] <= this->bounds[i+1]))
      {
      continue;
      }
    if ((this->bounds[i] >= required_bounds[i]) &&
        (this->bounds[i] <= required_bounds[i+1]))
      {
      continue;
      }
...
private:
  void setup_level();
  void refine_block(rgtl_object_array_points<N> &blockPoints,
    rtvl_refine_level<N> **(&pointsLevels), double refine_bounds[4]);
  void select_block_samples(vcl_vector< rtvl_refine_level<N>* > &levels_vector,
    rgtl_object_array_points<N> &blockPoints,
    rtvl_refine_level<N> **(&pointsLevels), double selection_bounds[4]);

  rtvl_refine<N>* external;
  double scale_multiplier;
  double mask_size;
  vcl_auto_ptr< rtvl_refine_quadtree<N> > tree;
  ...
  rtvl_refine_quadtree<N> *refine_quad_base;
  rtvl_refine_quadtree<N> *refine_quad_next;
  rgtl_object_array_points<N> points;
...
void rtvl_refine_internal<N>::build_tree()
{
  ...
  tree.reset( new rtvl_refine_quadtree<N> );
  ...
  int numberPerLeaf =  (int)(1.1 * this->points.get_number_of_points() / (numberOfDivisions * numberOfDivisions));  
                                                     ...
                                                     int requested_min_depth)
{
  ...
  rtvl_refine_quadtree<N> *quad_with_level_data = this->tree->find_quad_with_level( this->current_level );
  ...
  rtvl_refine_quadtree<N> *levelStartQuad = quad_with_level_data;
  unsigned int max_count = 0, current_count;

  vcl_vector< rtvl_refine_level<N>* > levels_vector;
  rgtl_object_array_points<N> blockPoints;
  rtvl_refine_level<N> **pointsLevels;
  while (levelStartQuad && 
    (this->tree->tree_depth - levelStartQuad->tree_depth) > 1 &&
    max_count < max_number_of_points)
    {
    this->refine_quad_base = levelStartQuad;
    rtvl_refine_quadtree<N> *rowStartQuad = levelStartQuad;
    rtvl_refine_quadtree<N> *currentQuad = rowStartQuad; 
    while (currentQuad)
      {
...
int rtvl_refine_internal<N>::refine_next_block(const char *base_filename)
{
  ...
  vcl_vector< rtvl_refine_level<N>* > levels_vector;
  rgtl_object_array_points<N> blockPoints;
  rtvl_refine_level<N> **pointsLevels = 0;
  this->refine_quad_next->gather_points(this->current_level, 
    6.0 * this->current_scale, levels_vector, blockPoints, pointsLevels, false);
  ...
  vcl_vector< rtvl_refine_level<N>* > refine_quad_levels_vector;
  this->refine_quad_next->gather_levels( this->current_level, refine_quad_levels_vector);
  typename vcl_vector< rtvl_refine_level<N>* >::const_iterator levels_iter = refine_quad_levels_vector.begin();
  ...
  if (!this->is_last_level)
    ...
    if (this->refine_quad_next->bounds[0] == this->data_bounds[0])
      ...
      {
      selection_bounds[0] = this->refine_quad_next->bounds[0] + this->current_scale * this->mask_size;
      }
    ...
    if (this->refine_quad_next->bounds[2] == this->data_bounds[2])
      ...
      {
      selection_bounds[2] = this->refine_quad_next->bounds[2] + this->current_scale * this->mask_size;
      }
                                           ...
                                           double refine_bounds[4])
{
  ...
  for(unsigned int i=0; i < n; ++i)
    ...
    if (votee_location[0] < refine_bounds[0] || 
        votee_location[0] > refine_bounds[1] ||
        votee_location[1] < refine_bounds[2] || 
        votee_location[1] > refine_bounds[3])
      {
      continue;
      }
    ...
    int num_voters = objects->query_sphere(votee_location.data_block(),
                                           3*this->current_scale, voter_ids);
    ...
    for(int vi=0; vi < num_voters; vi++)
      ...
      rtvl_voter<N> voter(voter_location, pointsLevels[j]->calculate_tokens->at(j - pointsLevels[j]->offset));
                                                   ...
                                                   double selection_bounds[4])
{
  ...
  for (; level_iter != levels_vector.end(); level_iter++)
    ...
    for (; selection_iter != (*level_iter)->selection.end(); selection_iter++)
      ...
      int num_masked = objects->query_sphere(p, 
        this->current_scale * this->mask_size, masked_ids);
  ...
  while(!saliency_map.empty() && saliency_map.begin()->first >= 0)
    ...
    if (p[0] >= selection_bounds[0] && p[0] <= selection_bounds[1] &&
        p[1] >= selection_bounds[2] && p[1] <= selection_bounds[3])
      {
  ...
  for (; levels_iter != refine_quad_levels_vector.end(); levels_iter++)
    ...
    rtvl_refine_quadtree<N> *next_level_quad = (*levels_iter)->quad;
    if (this->save_next_level_samples_in_parent)
      ...
      levels_subset.push_back( *(++levels_iter) );
      // temporary check
      //rtvl_refine_quadtree<N> *parent = next_level_quad->parent;
      //vcl_vector< rtvl_refine_level<N>* >::const_iterator levels_subset_iter = levels_subset.begin();
      //for (; levels_subset_iter != levels_subset.end(); levels_subset_iter++)
      //  {
    ...
    next_level_quad->add_selections_for_next_level(
      this->current_scale * this->scale_multiplier, this->scale_multiplier,
      this->current_level + 1, levels_subset);
    }
}
...
//----------------------------------------------------------------------------
template <unsigned int N>
int rtvl_refine<N>::initialize_refine_level(unsigned int memory_limit,
