...
public:
  ...
  rtvl_refine_quadtree<N>* get_quad(unsigned int depth, unsigned int i, unsigned int j);
  ...
  unsigned int gather_points(int level, double padLength,
    ...
    rtvl_refine_level<N>**(&pointsLevels), bool countOnly);
...
{
  typename vcl_vector<rtvl_refine_level<N>*>::const_iterator levels_subset_iter =
    levels_subset.begin();
...
private:
  void setup_level();
  void refine_block(rgtl_object_array_points<N>& blockPoints, rtvl_refine_level<N>**(&pointsLevels),
  ...
  unsigned int /*memory_limit*/, int requested_min_depth)
{
  ...
  rtvl_refine_quadtree<N>* quad_with_level_data =
...
int rtvl_refine_internal<N>::refine_next_block(const char* base_filename)
{
  ...
  typename vcl_vector<rtvl_refine_level<N>*>::const_iterator levels_iter =
